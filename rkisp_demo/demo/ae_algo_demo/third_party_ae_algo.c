                      ...
                      ae_pfnAe_results_t* pstAeResult)
{
    ...
    if(pstAeInfo != NULL) {
        float MeanLuma[2] = {0.0f};

        for(int i = 0; i < 225; i++) {
            MeanLuma[0] += (float)(pstAeInfo->pHwEnt0->mainWin.hw_ae_meanBayerGrGb_val[i] >> 4); //channelg is 12bit, channelr/channelb is 10bit
            MeanLuma[1] += (float)(pstAeInfo->pHwEnt3->mainWin.hw_ae_meanBayerGrGb_val[i] >> 4); //channelg is 12bit, channelr/channelb is 10bit
        }
        ...
        printf("custom ae test: %s lin gain=%f,time=%f,meanluma=%f, tmoluma=%f\n", __func__, pstAeInfo->cisRkExp.linear_exp.exp_real_params.analog_gain,
               pstAeInfo->cisRkExp.linear_exp.exp_real_params.integration_time, MeanLuma[0], MeanLuma[1]);
        printf("custom ae test: %s hdr sgain=%f,stime=%f,mgain=%f,mtime=%f,smeanluma = %f,mmeanluma=%f\n", __func__,
               pstAeInfo->cisRkExp.hdr_exp[0].exp_real_params.analog_gain, pstAeInfo->cisRkExp.hdr_exp[0].exp_real_params.integration_time,
               pstAeInfo->cisRkExp.hdr_exp[1].exp_real_params.analog_gain, pstAeInfo->cisRkExp.hdr_exp[1].exp_real_params.integration_time,
               MeanLuma[0], MeanLuma[1]);

    }

    pstAeResult->isRkExpValid = true;
    static uint32_t frmid = 0;

    if(frmid % 20 < 10) {
        //0) HDR
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.analog_gain = 1.0f;
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.integration_time = 0.003f;
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.digital_gain = 1.0f;
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.isp_dgain = 1.0f;
        //pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.dcg_mode = 0;

        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.analog_gain = 2.0f;
        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.integration_time = 0.01f;
        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.digital_gain = 1.0f;
        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.isp_dgain = 1.0f;
        //pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.dcg_mode = 0;

        //1) Linear
        pstAeResult->cisRkExp.linear_exp.exp_real_params.analog_gain = 2.0f;
        pstAeResult->cisRkExp.linear_exp.exp_real_params.integration_time = 0.02f;
        pstAeResult->cisRkExp.linear_exp.exp_real_params.digital_gain = 1.0f;
        pstAeResult->cisRkExp.linear_exp.exp_real_params.isp_dgain = 1.0f;
        //pstAeResult->cisRkExp.linear_exp.exp_real_params.dcg_mode = 0;
        ...
        //0) HDR
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.analog_gain = 6.0f;
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.integration_time = 0.003f;
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.digital_gain = 1.0f;
        pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.isp_dgain = 1.0f;
        //pstAeResult->cisRkExp.hdr_exp[0].exp_real_params.dcg_mode = 1;

        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.analog_gain = 40.0f;
        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.integration_time = 0.01f;
        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.digital_gain = 1.0f;
        pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.isp_dgain = 1.0f;
        //pstAeResult->cisRkExp.hdr_exp[1].exp_real_params.dcg_mode = 1;

        //1) Linear
        pstAeResult->cisRkExp.linear_exp.exp_real_params.analog_gain = 40.0f;
        pstAeResult->cisRkExp.linear_exp.exp_real_params.integration_time = 0.03f;
        pstAeResult->cisRkExp.linear_exp.exp_real_params.digital_gain = 1.0f;
        pstAeResult->cisRkExp.linear_exp.exp_real_params.isp_dgain = 1.0f;
        //pstAeResult->cisRkExp.linear_exp.exp_real_params.dcg_mode = 1;
