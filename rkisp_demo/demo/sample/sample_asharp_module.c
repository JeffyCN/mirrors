...
XCamReturn sample_sharp_setStrength_v34(const rk_aiq_sys_ctx_t* ctx, rk_aiq_uapi_mode_sync_e sync_mode, float fStrength)
{
    ...
    rk_aiq_uapi2_setSharpness(ctx, g_level);
    printf("Set asharp v4 set streangth ret:%d strength:%d \n\n", ret, g_level);
...
static void get_auto_attr(sharp_api_attrib_t* attr) {
    sharp_param_auto_t* stAuto = &attr->stAuto;
    for (int i = 0;i < 13;i++) {
    }
}

static void get_manual_attr(sharp_api_attrib_t* attr) {
