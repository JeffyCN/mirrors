...
static void get_manual_attr(gamma_api_attrib_t* attr) {
...
static void sample_gamma_tuningtool_test(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    const char* json_gamma_status_str = " \n\
        [{ \n\
            \"op\":\"get\", \n\
    ...
    rkaiq_uapi_unified_ctl((rk_aiq_sys_ctx_t*)(ctx),
                           json_gamma_status_str, &ret_str, RKAIQUAPI_OPMODE_GET);
    ...
    ret_str = NULL;
    rkaiq_uapi_unified_ctl((rk_aiq_sys_ctx_t*)(ctx),
                           json_gamma_str, &ret_str, RKAIQUAPI_OPMODE_SET);
