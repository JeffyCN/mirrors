...
static void sample_blc_tuningtool_test(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    const char* json_blc_status_str = " \n\
        [{ \n\
            \"op\":\"get\", \n\
    ...
    rkaiq_uapi_unified_ctl((rk_aiq_sys_ctx_t*)(ctx),
                           json_blc_status_str, &ret_str, RKAIQUAPI_OPMODE_GET);
    ...
    ret_str = NULL;
    rkaiq_uapi_unified_ctl((rk_aiq_sys_ctx_t*)(ctx),
                           json_blc_str, &ret_str, RKAIQUAPI_OPMODE_SET);
...
static void get_auto_attr(blc_api_attrib_t* attr) {
...
static void get_manual_attr(blc_api_attrib_t* attr) {
