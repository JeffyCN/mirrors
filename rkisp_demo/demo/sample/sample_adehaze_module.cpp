...
static void sample_dehaze_tuningtool_test(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    std::string json_dehaze_status_str = " \n\
        [{ \n\
            \"op\":\"get\", \n\
    ...
    rkaiq_uapi_unified_ctl(const_cast<rk_aiq_sys_ctx_t*>(ctx),
                           const_cast<char*>(json_dehaze_status_str.c_str()), &ret_str, RKAIQUAPI_OPMODE_GET);
    ...
    printf("dehaze json_cmd_str: %s\n", json_dehaze_str.c_str());
    ret_str = NULL;
    rkaiq_uapi_unified_ctl(const_cast<rk_aiq_sys_ctx_t*>(ctx),
                           const_cast<char*>(json_dehaze_str.c_str()), &ret_str, RKAIQUAPI_OPMODE_SET);
