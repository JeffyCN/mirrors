...
static void sample_cp_tuningtool_test(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    rkaiq_uapi_unified_ctl((rk_aiq_sys_ctx_t*)(ctx),
                           json_cp_status_str, &ret_str, RKAIQUAPI_OPMODE_GET);
    ...
    ret_str = NULL;
    rkaiq_uapi_unified_ctl((rk_aiq_sys_ctx_t*)(ctx),
                           json_cp_str, &ret_str, RKAIQUAPI_OPMODE_SET);
...
static void get_auto_attr(cp_api_attrib_t* attr) {
    cp_param_auto_t* stAuto = &attr->stAuto;
    for (int i = 0;i < 13;i++) {
    }
}

static void get_manual_attr(cp_api_attrib_t* attr) {
