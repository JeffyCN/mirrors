...
int32_t custom_awb_run(void* ctx, const void* vpstAwbInfo,  void* vpstAwbResult)
{
    ...
    if (pstAwbInfo == NULL){
        ...
        rk_aiq_wb_gain_t wbgain = {1.9,1.0,1.0,1.6};
        pstAwbResult->awb_gain_algo=wbgain;
    ...
    if (cam_type == RK_AIQ_CAM_TYPE_GROUP){
        if(1/* for other cameras are different with camera0*/){
            if(pstAwbResult->next ==NULL){//for camera1
                ...
                rk_aiq_wb_gain_t gain = {1.8,1.0,1.0,1.6};
                pstAwbResult->next->awb_gain_algo = gain;
                //to do more paras
            }
            if(pstAwbResult->next->next == NULL){//for camera2
                ...
                rk_aiq_wb_gain_t gain = {1.9,1.0,1.0,1.6};
                pstAwbResult->next->next->awb_gain_algo = gain;
                //to do more paras
            }
