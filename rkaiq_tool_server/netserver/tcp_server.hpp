...
template <uint16_t BUFFER_SIZE = AS_DEFAULT_BUFFER_SIZE> class NewTCPServer : public BaseSocket
{
  public:
    // Event Listeners:
    std::function<void(TCPSocket<BUFFER_SIZE>*)> onNewConnection = [](TCPSocket<BUFFER_SIZE>* sock) {
        FDR_UNUSED(sock)
    };

    explicit NewTCPServer(FDR_ON_ERROR) : BaseSocket(onError, SocketType::TCP)
    {
        int opt = 1;
  ...
  private:
    static void Accept(NewTCPServer<BUFFER_SIZE>* server, FDR_ON_ERROR)
    {
