...
template <uint16_t BUFFER_SIZE = AS_DEFAULT_BUFFER_SIZE> class TCPSocket : public BaseSocket
{
  public:
    // Event Listeners:
    ...
    // Send raw bytes
    ssize_t Send(const char* bytes, size_t byteslength)
    {
    ...
    void Connect(
        uint32_t ipv4, uint16_t port, std::function<void()> onConnected = []() {}, FDR_ON_ERROR)
    {
    ...
    void Connect(
        const char* host, uint16_t port, std::function<void()> onConnected = []() {}, FDR_ON_ERROR)
    {
    ...
    void Connect(
        const std::string& host, uint16_t port, std::function<void()> onConnected = []() {}, FDR_ON_ERROR)
    {
