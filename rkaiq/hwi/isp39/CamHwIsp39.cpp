...
CamHwIsp39::CamHwIsp39() : CamHwIsp3x() {
...
CamHwIsp39::~CamHwIsp39() {}

XCamReturn CamHwIsp39::init(const char* sns_ent_name) {
    XCamReturn ret = CamHwIsp3x::init(sns_ent_name);
    return ret;
}

XCamReturn CamHwIsp39::stop() {
    XCamReturn ret = CamHwIsp3x::stop();
    return ret;
}

void
CamHwIsp39::updateEffParams(void* params, void* ori_params)
{
...
#define UPDATE_EFF_PARAMS() \
        ...
        if (mIsMultiIspMode) {
            dst_isp_params = &_effecting_ispparam_map[effFrmId]->data()->result.isp_params_v39[0];
            if (is_got_latest_params)
                lates_isp_params_ptr = &latestIspParams.isp_params_v39[0];
            new_isp_params = (struct isp39_isp_params_cfg*)ori_params;
            UPDATE_EFF_PARAMS();

            dst_isp_params = &_effecting_ispparam_map[effFrmId]->data()->result.isp_params_v39[1];
            if (is_got_latest_params)
                lates_isp_params_ptr = &latestIspParams.isp_params_v39[1];
            new_isp_params = (struct isp39_isp_params_cfg*)ori_params;
            UPDATE_EFF_PARAMS();
        }
#endif
...
bool
CamHwIsp39::processTb(void* params)
{
#if defined(ISP_HW_V39)
    struct isp39_isp_params_cfg* isp_params = (struct isp39_isp_params_cfg*)params;
    if (mTbInfo.is_fastboot) {
        if (isp_params->frame_id == 0 && _not_skip_first) {
            ...
            if (!_first_awb_cfg) {
                ...
                *((struct isp39_rawawb_meas_cfg*)_first_awb_cfg) = isp_params->meas.rawawb;
            }
            if (!_skipped_params) {
            ...
            if (isp_params->frame_id < 10) {
                if (_first_awb_cfg) {
                    struct isp39_rawawb_meas_cfg* first_awb_cfg = (struct isp39_rawawb_meas_cfg*)_first_awb_cfg;
            ...
            struct isp39_isp_params_cfg* skip_param = (struct isp39_isp_params_cfg*)_skipped_params;
