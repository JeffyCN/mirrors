...
XCamReturn RkAiqCamGroupAynrV22HandleInt::updateConfig(bool needSync) {
    ...
    if (updateAtt) {
        ...
        mCurAtt   = mNewAtt;
        rk_aiq_uapi_camgroup_aynrV22_SetAttrib(mAlgoCtx, &mCurAtt, false);
    ...
    if (updateStrength) {
        ...
        rk_aiq_uapi_camgroup_aynrV22_SetLumaSFStrength(mAlgoCtx, &mCurStrength);
...
XCamReturn RkAiqCamGroupAynrV22HandleInt::setAttrib(const rk_aiq_ynr_attrib_v22_t* att) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
...
XCamReturn RkAiqCamGroupAynrV22HandleInt::getAttrib(rk_aiq_ynr_attrib_v22_t* att) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
    ...
    if(att->sync.sync_mode == RK_AIQ_UAPI_MODE_SYNC) {
        mCfgMutex.lock();
        rk_aiq_uapi_camgroup_aynrV22_GetAttrib(mAlgoCtx, att);
        att->sync.done = true;
        mCfgMutex.unlock();
    } else {
        if(updateAtt) {
...
XCamReturn RkAiqCamGroupAynrV22HandleInt::setStrength(const rk_aiq_ynr_strength_v22_t *pStrength) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
...
XCamReturn RkAiqCamGroupAynrV22HandleInt::getStrength(rk_aiq_ynr_strength_v22_t *pStrength) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ...
    if(pStrength->sync.sync_mode == RK_AIQ_UAPI_MODE_SYNC) {
        mCfgMutex.lock();
        rk_aiq_uapi_camgroup_aynrV22_GetLumaSFStrength(mAlgoCtx, pStrength);
...
XCamReturn RkAiqCamGroupAynrV22HandleInt::getInfo(rk_aiq_ynr_info_v22_t *pInfo) {
    ENTER_ANALYZER_FUNCTION();
