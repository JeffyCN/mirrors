...
XCamReturn RkAiqCamGroupAcnrV2HandleInt::updateConfig(bool needSync) {
    ...
    if (updateAtt) {
        ...
        mCurAtt   = mNewAtt;
        rk_aiq_uapi_camgroup_acnrV2_SetAttrib(mAlgoCtx, &mCurAtt, false);
...
XCamReturn RkAiqCamGroupAcnrV2HandleInt::setAttrib(rk_aiq_cnr_attrib_v2_t* att) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
...
XCamReturn RkAiqCamGroupAcnrV2HandleInt::getAttrib(rk_aiq_cnr_attrib_v2_t* att) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
    ...
    if(att->sync.sync_mode == RK_AIQ_UAPI_MODE_SYNC) {
        mCfgMutex.lock();
        rk_aiq_uapi_camgroup_acnrV2_GetAttrib(mAlgoCtx, att);
        att->sync.done = true;
        mCfgMutex.unlock();
    } else {
        if(updateAtt) {
...
XCamReturn RkAiqCamGroupAcnrV2HandleInt::setStrength(rk_aiq_cnr_strength_v2_t *pStrength) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
...
XCamReturn RkAiqCamGroupAcnrV2HandleInt::getStrength(rk_aiq_cnr_strength_v2_t *pStrength) {
    ENTER_ANALYZER_FUNCTION();
    LOGD_ANR("%s:%d\n", __FUNCTION__, __LINE__);
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
