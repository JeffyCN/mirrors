...
XCamReturn RkAiqResourceTranslatorV39::translateAecStats(const SmartPtr<VideoBuffer>& from,
    ...
    statsInt->aec_stats_v25.ae_data.hw_ae_entityGroup_mode = _ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode;

    if(_ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
            _ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity0_wkOnly_mode) {
    ...
    if(_ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
            _ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity3_wkOnly_mode) {
    ...
    if(Bnr20bitEn > 0 && _ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_coWk_mode) {
        ...
        rawhist_mode = _ispParams.ae_cfg_v39.entityGroup.coWkEntity03.hist.hw_aeCfg_pix_mode;
    ...
    if(Bnr20bitEn) {
        ...
        if(_ispParams.ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_coWk_mode) {
    ...
    // expsoure params
    if (_expParams.ptr()) {
        statsInt->aec_stats_v25.ae_exp = _expParams->data()->aecExpInfo;
    ...
    if (irisParams.ptr()) {
        ...
        */
        ...
        if (sof_time < end_time + frm_intval)
            statsInt->aec_stats_v25.ae_exp.Iris.PIris.step = irisParams->data()->PIris.laststep;
        else
            statsInt->aec_stats_v25.ae_exp.Iris.PIris.step = irisParams->data()->PIris.step;
    }
...
XCamReturn RkAiqResourceTranslatorV39::translateAecStats(const SmartPtr<VideoBuffer>& from,
    ...
    if (run_flag) {
        ...
        if(Bnr20bitEn > 0) {
            ...
            //for 20bit input, default is 3frame,and clip LBS[0:9] as Lframe
            switch (_aeAlgoStatsCfg.RawStatsChnSel) {
            case RAWSTATS_CHN_R_EN:
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    statsInt->aec_stats.ae_data.chn[2].rawae_big.channelr_xy[i] = stats->stat.rawae0.blk_y[i / 15].blk_x[i % 15].r;
                    statsInt->aec_stats.ae_data.extra.rawae_big.channelr_xy[i] = stats->stat.rawae3.blk_y[i / 15].blk_x[i % 15].r;
                }

                Aec20BitGirdClip(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelr_xy, statsInt->aec_stats.ae_data.extra.rawae_big.channelr_xy, 10);
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    sum_xy += ((statsInt->aec_stats.ae_data.chn[2].rawae_big.channelr_xy[i] >> 2) * _aeAlgoStatsCfg.BigWeight[i]);
                    sum_weight += _aeAlgoStatsCfg.BigWeight[i];
                }
                statsInt->aec_stats.ae_data.raw_mean[2] = round(256.0f * (float)sum_xy / (float)sum_weight);
                break;

            case RAWSTATS_CHN_G_EN:
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    statsInt->aec_stats.ae_data.chn[2].rawae_big.channelg_xy[i] = stats->stat.rawae0.blk_y[i / 15].blk_x[i % 15].g;
                    statsInt->aec_stats.ae_data.extra.rawae_big.channelg_xy[i] = stats->stat.rawae3.blk_y[i / 15].blk_x[i % 15].g;
                }
                Aec20BitGirdClip(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelg_xy, statsInt->aec_stats.ae_data.extra.rawae_big.channelg_xy, 12);
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    sum_xy += ((statsInt->aec_stats.ae_data.chn[2].rawae_big.channelg_xy[i] >> 4) * _aeAlgoStatsCfg.BigWeight[i]);
                    sum_weight += _aeAlgoStatsCfg.BigWeight[i];
                }
                statsInt->aec_stats.ae_data.raw_mean[2] = round(256.0f * (float)sum_xy / (float)sum_weight);
                break;
            ...
            case RAWSTATS_CHN_B_EN:
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    statsInt->aec_stats.ae_data.chn[2].rawae_big.channelb_xy[i] = stats->stat.rawae0.blk_y[i / 15].blk_x[i % 15].b;
                    statsInt->aec_stats.ae_data.extra.rawae_big.channelb_xy[i] = stats->stat.rawae3.blk_y[i / 15].blk_x[i % 15].b;
                }
                Aec20BitGirdClip(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelb_xy, statsInt->aec_stats.ae_data.extra.rawae_big.channelb_xy, 10);
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    sum_xy += ((statsInt->aec_stats.ae_data.chn[2].rawae_big.channelb_xy[i] >> 2) * _aeAlgoStatsCfg.BigWeight[i]);
                    sum_weight += _aeAlgoStatsCfg.BigWeight[i];
                }
                statsInt->aec_stats.ae_data.raw_mean[2] = round(256.0f * (float)sum_xy / (float)sum_weight);
                break;
            ...
            default:
                ...
                Aec20BitGirdClip(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelb_xy, statsInt->aec_stats.ae_data.extra.rawae_big.channelb_xy, 10);
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    statsInt->aec_stats.ae_data.chn[2].rawae_big.channely_xy[i] = CLIP(round(
                                rcc * (float)(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelr_xy[i] >> 2) +
                                gcc * (float)(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelg_xy[i] >> 4) +
                                bcc * (float)(statsInt->aec_stats.ae_data.chn[2].rawae_big.channelb_xy[i] >> 2) + off), 0, MAX_8BITS);
                    sum_xy += (statsInt->aec_stats.ae_data.chn[2].rawae_big.channely_xy[i] * _aeAlgoStatsCfg.BigWeight[i]);
                    sum_weight += _aeAlgoStatsCfg.BigWeight[i];
                }
                statsInt->aec_stats.ae_data.raw_mean[2] = round(256.0f * (float)sum_xy / (float)sum_weight);
                break;
            ...
            for(int i = 0; i < ISP3X_HIST_BIN_N_MAX; i++) {
                statsInt->aec_stats.ae_data.chn[2].rawhist_big.bins[i] = stats->stat.rawhist0.bin[i];
                statsInt->aec_stats.ae_data.extra.rawhist_big.bins[i] = stats->stat.rawhist3.bin[i];
                sumpixel += statsInt->aec_stats.ae_data.extra.rawhist_big.bins[i];
            }

            statsInt->aec_stats.ae_data.chn[2].rawhist_big.bins[ISP3X_HIST_BIN_N_MAX - 1] += sumpixel;
    ...
    if(Bnr20bitEn) {
        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 15; j++) {
                printf("rawae0[%d,%d]:r 0x%x, g 0x%x, b 0x%x\n", i, j,
                       stats->stat.rawae0.blk_y[i].blk_x[j].r,
                       stats->stat.rawae0.blk_y[i].blk_x[j].g,
        ...
        for (int i = 0; i < 256; i++)
            printf("rawae0 bin[%d]= 0x%08x\n", i, stats->stat.rawhist0.bin[i]);

        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 15; j++) {
                printf("rawae3[%d,%d]:r 0x%x, g 0x%x, b 0x%x\n", i, j,
                       stats->stat.rawae3.blk_y[i].blk_x[j].r,
                       stats->stat.rawae3.blk_y[i].blk_x[j].g,
...
XCamReturn RkAiqResourceTranslatorV39::translateAwbStats(const SmartPtr<VideoBuffer>& from,
    ...
    memcpy(statsInt->awb_stats_v39.awb_cfg_effect.IllIndxSetCurrent, _ispParams.awb_cfg_v39.IllIndxSetCurrent,
