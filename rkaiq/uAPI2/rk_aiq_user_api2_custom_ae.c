...
(
    ...
    if(pConfig->stSensorInfo.Gain2Reg.GainMode == EXPGAIN_MODE_LINEAR) {
        ...
        int *revert_gain_array = (int *)aiq_mallocz((size / 7 * 2) * sizeof(int));
        ...
        if(revert_gain_array != NULL) {
            aiq_free(revert_gain_array);
            revert_gain_array = NULL;
        }
    } else if(pConfig->stSensorInfo.Gain2Reg.GainMode == EXPGAIN_MODE_NONLINEAR_DB) {
        float dbUnit = pConfig->stSensorInfo.Gain2Reg.GainRegDBUnit;
        realGain = pow(10, (float)sensorGain * dbUnit / 20.0f);
    }
...
(
    ...
    if(pConfig->stSensorInfo.Gain2Reg.GainMode == EXPGAIN_MODE_LINEAR) {
        ...
        *regGain = (int)(C1 * pow(ag, M0) - C0 + 0.5f);
        ...
        float dbUnit = pConfig->stSensorInfo.Gain2Reg.GainRegDBUnit;
        *regGain = (int)(20.0f * log10(ag) / dbUnit + 0.5f);
    }
    ...
    float IntegrationTimeLine = timeC0 * vts + timeC1 + timeC2 * ((SplitIntegrationTime * pclk * 1000000 / hts) + timeC3);
    *regIntegrationTime = (int)(IntegrationTimeLine / pConfig->stSensorInfo.CISTimeSet.Linear.CISTimeLinePerReg);
    int Index = (*regIntegrationTime - (int)CISTimeRegOdevity[1]) / (int)CISTimeRegOdevity[0];
    *regIntegrationTime = (int)CISTimeRegOdevity[0] * Index + (int)CISTimeRegOdevity[1];
        ...
        rk_aiq_singlecam_3a_result_t ** rk_aiq_singlecam_3a_result)
{
    ...
    for(int i = 0; i < camnum; i++) {
        ...
        if (!rk_aiq_singlecam_3a_result[i]->aec.aec_stats) {
        ...
        if(i > 0) {
            if(customAeStats->next == nullptr)
                customAeStats->next = (rk_aiq_customAe_stats_t*)aiq_mallocz(1 * sizeof(rk_aiq_customAe_stats_t));
            next_customAeStats = customAeStats->next;
            customAeStats = next_customAeStats;
        }

        _rkAeStats2CustomAeStats(pConfig, customAeStats, rk_aiq_singlecam_3a_result[i]->aec.aec_stats);
    }
...
static XCamReturn AeDemoProcessing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
    ...
    if(!inparams->u.proc.init) { // init=ture, stats=null
        ...
        rk_aiq_isp_aec_stats_t* xAecStats = AeProcParams->aecStatsBuf;
        if (!xAecStats) {
            LOGE_AEC("aec stats is null");
            return(XCAM_RETURN_ERROR_FAILED);
        }
