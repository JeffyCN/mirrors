...
XCamReturn rk_aiq_user_api2_alsc_SetAttrib(const rk_aiq_sys_ctx_t *sys_ctx,
  ...
  if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
    ...
    RkAiqCamGroupAlscHandleInt *algo_handle =
        camgroupAlgoHandle<RkAiqCamGroupAlscHandleInt>(sys_ctx,
    ...
    if (algo_handle) {
      ...
      for (auto camCtx : camgroup_ctx->cam_ctxs_array) {
        ...
        RkAiqAlscHandleInt *singleCam_algo_handle =
            algoHandle<RkAiqAlscHandleInt>(camCtx, RK_AIQ_ALGO_TYPE_ALSC);
    ...
    if (algo_handle) {
      ...
      for (auto camCtx : camgroup_ctx->cam_ctxs_array) {
        ...
        RkAiqAlscHandleInt *singleCam_algo_handle =
            algoHandle<RkAiqAlscHandleInt>(camCtx, RK_AIQ_ALGO_TYPE_ALSC);
        if (singleCam_algo_handle)
          return singleCam_algo_handle->queryLscInfo(lsc_querry_info);
      }
    }
#else
    return XCAM_RETURN_ERROR_FAILED;
