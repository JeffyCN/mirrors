...
#include "algo_handlers/newStruct/RkAiqYmeHandler.h"
...
static XCamReturn
_yme_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, yme_api_attrib_t* attr)
{
    ...
    params.type = RESULT_TYPE_MOTION_PARAM;
    params.man_param_size = sizeof(yme_param_t);
    params.man_param_ptr = &attr->stMan;
    params.aut_param_size = sizeof(yme_param_auto_t);
...
rk_aiq_user_api2_yme_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, yme_api_attrib_t* attr)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        if (camgroup_ctx->cam_ctxs_array[0])
            return _yme_SetAttrib(camgroup_ctx->cam_ctxs_array[0], attr);
        else
        ...
        return _yme_SetAttrib(sys_ctx, attr);
    }
#else
...
static XCamReturn
_yme_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, yme_api_attrib_t* attr)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    RkAiqYmeHandleInt* algo_handle =
        algoHandle<RkAiqYmeHandleInt>(sys_ctx, RK_AIQ_ALGO_TYPE_AMD);
    if (algo_handle) {
...
rk_aiq_user_api2_yme_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, yme_api_attrib_t* attr)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        const rk_aiq_camgroup_ctx_t* camgroup_ctx = (rk_aiq_camgroup_ctx_t *)sys_ctx;
        if (camgroup_ctx->cam_ctxs_array[0])
            return _yme_GetAttrib(camgroup_ctx->cam_ctxs_array[0], attr);
        else
            return XCAM_RETURN_ERROR_FAILED;
#else
        return XCAM_RETURN_ERROR_FAILED;
        ...
        return _yme_GetAttrib(sys_ctx, attr);
    }
#else
    return XCAM_RETURN_ERROR_UNKNOWN;
#endif
...
static XCamReturn
_yme_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, yme_status_t* status)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    if (sys_ctx->_rkAiqManager->getGlobalParamsManager()->isManual(RESULT_TYPE_MOTION_PARAM)) {
        // get cur manual params
        rk_aiq_global_params_wrap_t params;
        params.type = RESULT_TYPE_MOTION_PARAM;
        params.man_param_size = sizeof(yme_param_t);
...
rk_aiq_user_api2_yme_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, yme_status_t* status)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        const rk_aiq_camgroup_ctx_t* camgroup_ctx = (rk_aiq_camgroup_ctx_t *)sys_ctx;
        if (camgroup_ctx->cam_ctxs_array[0])
            return _yme_QueryStatus(camgroup_ctx->cam_ctxs_array[0], status);
        else
            return XCAM_RETURN_ERROR_FAILED;
#endif
    } else {
        return _yme_QueryStatus(sys_ctx, status);
    }
#else
    return XCAM_RETURN_ERROR_UNKNOWN;
#endif
