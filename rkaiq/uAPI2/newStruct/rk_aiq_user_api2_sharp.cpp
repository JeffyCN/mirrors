...
#include "algo_handlers/newStruct/RkAiqSharpHandler.h"
...
_sharp_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, sharp_api_attrib_t* attr)
{
    ...
    params.type = RESULT_TYPE_SHARPEN_PARAM;
    params.man_param_size = sizeof(sharp_param_t);
...
rk_aiq_user_api2_sharp_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, sharp_api_attrib_t* attr)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        if (camgroup_ctx->cam_ctxs_array[0])
            return _sharp_SetAttrib(camgroup_ctx->cam_ctxs_array[0], attr);
        else
        ...
        return _sharp_SetAttrib(sys_ctx, attr);
    }
#else
...
static XCamReturn
_sharp_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, sharp_api_attrib_t* attr)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    RkAiqSharpHandleInt* algo_handle =
        algoHandle<RkAiqSharpHandleInt>(sys_ctx, RK_AIQ_ALGO_TYPE_ASHARP);
    if (algo_handle) {
...
rk_aiq_user_api2_sharp_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, sharp_api_attrib_t* attr)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        const rk_aiq_camgroup_ctx_t* camgroup_ctx = (rk_aiq_camgroup_ctx_t *)sys_ctx;
        if (camgroup_ctx->cam_ctxs_array[0])
            return _sharp_GetAttrib(camgroup_ctx->cam_ctxs_array[0], attr);
        else
            return XCAM_RETURN_ERROR_FAILED;
#else
        return XCAM_RETURN_ERROR_FAILED;
        ...
        return _sharp_GetAttrib(sys_ctx, attr);
    }
#else
    return XCAM_RETURN_ERROR_UNKNOWN;
#endif
...
static XCamReturn
_sharp_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, sharp_status_t* status)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    if (sys_ctx->_rkAiqManager->getGlobalParamsManager()->isManual(RESULT_TYPE_SHARPEN_PARAM)) {
        // get cur manual params
        rk_aiq_global_params_wrap_t params;
        params.type = RESULT_TYPE_SHARPEN_PARAM;
        params.man_param_size = sizeof(sharp_param_t);
...
rk_aiq_user_api2_sharp_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, sharp_status_t* status)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        const rk_aiq_camgroup_ctx_t* camgroup_ctx = (rk_aiq_camgroup_ctx_t *)sys_ctx;
        if (camgroup_ctx->cam_ctxs_array[0])
            return _sharp_QueryStatus(camgroup_ctx->cam_ctxs_array[0], status);
        else
            return XCAM_RETURN_ERROR_FAILED;
#endif
    } else {
        return _sharp_QueryStatus(sys_ctx, status);
    }
#else
    return XCAM_RETURN_ERROR_UNKNOWN;
#endif
