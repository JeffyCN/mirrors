...
#include "algo_handlers/newStruct/RkAiqMergeHandler.h"
...
static XCamReturn
_merge_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, mge_api_attrib_t* attr)
{
    ...
    params.en = attr->en;
    params.type = RESULT_TYPE_MERGE_PARAM;
    params.man_param_size = sizeof(mge_param_t);
    params.man_param_ptr = &attr->stMan;
    params.aut_param_size = sizeof(mge_param_auto_t);
...
rk_aiq_user_api2_merge_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, mge_api_attrib_t* attr)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        for (auto camCtx : camgroup_ctx->cam_ctxs_array) {
            if (!camCtx)
                continue;
            ret = _merge_SetAttrib(camCtx, attr);
        }
        return ret;
#else
        ...
        return _merge_SetAttrib(sys_ctx, attr);
    }
#else
...
static XCamReturn
_merge_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, mge_api_attrib_t* attr)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    RkAiqMergeHandleInt* algo_handle =
        algoHandle<RkAiqMergeHandleInt>(sys_ctx, RK_AIQ_ALGO_TYPE_AMERGE);
...
rk_aiq_user_api2_merge_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, mge_api_attrib_t* attr)
{
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        for (auto camCtx : camgroup_ctx->cam_ctxs_array) {
            ...
            return _merge_GetAttrib(camCtx , attr);
        }
#else
        return XCAM_RETURN_ERROR_FAILED;
#endif
    } else {
        return _merge_GetAttrib(sys_ctx, attr);
    }
#else
    return XCAM_RETURN_ERROR_UNKNOWN;
#endif
...
static XCamReturn
_merge_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, mge_status_t* status)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    if (sys_ctx->_rkAiqManager->getGlobalParamsManager()->isManual(RESULT_TYPE_MERGE_PARAM)) {
        // get cur manual params
        rk_aiq_global_params_wrap_t params;
        params.type = RESULT_TYPE_MERGE_PARAM;
        params.man_param_size = sizeof(mge_param_t);
    ...
    if (sys_ctx->cam_type == RK_AIQ_CAM_TYPE_GROUP) {
        ...
        for (auto camCtx : camgroup_ctx->cam_ctxs_array) {
            if (!camCtx)
                continue;
            // return the first cam's attrib
            return _merge_QueryStatus(camCtx , status);
        }
#else
        return XCAM_RETURN_ERROR_FAILED;
#endif
    } else {
        return _merge_QueryStatus(sys_ctx, status);
    }
#else
    return XCAM_RETURN_ERROR_UNKNOWN;
#endif
