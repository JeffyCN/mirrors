...
void
argparse_usage_string(struct argparse *self, char *usage)
{
    if (self->usages) {
        const char *const *usages = self->usages;
        sprintf(usage + strlen(usage), "Usage: %s\n", *usages++);
        while (*usages && **usages)
            sprintf(usage + strlen(usage), "   or: %s\n", *usages++);
    } else {
        sprintf(usage + strlen(usage), "Usage:\n");
    }
    ...
    for (; options->type != ARGPARSE_OPT_END; options++) {
        ...
        pos = sprintf(usage + strlen(usage), "    ");
        if (options->short_name) {
            pos += sprintf(usage + strlen(usage), "-%c", options->short_name);
        }
        if (options->long_name && options->short_name) {
            pos += sprintf(usage + strlen(usage), ", ");
        }
        if (options->long_name) {
            pos += sprintf(usage + strlen(usage), "--%s", options->long_name);
        }
        if (options->type == ARGPARSE_OPT_INTEGER) {
            pos += sprintf(usage + strlen(usage), "=<int>");
        } else if (options->type == ARGPARSE_OPT_FLOAT) {
            pos += sprintf(usage + strlen(usage), "=<flt>");
        } else if (options->type == ARGPARSE_OPT_STRING) {
            pos += sprintf(usage + strlen(usage), "=<str>");
        }
        if (pos <= usage_opts_width) {
            pad = usage_opts_width - pos;
        } else {
        ...
        sprintf(usage + strlen(usage), "%*s%s\n", (int)pad + 2, "", options->help);
    }
