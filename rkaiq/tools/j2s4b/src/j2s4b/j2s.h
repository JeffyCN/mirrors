...
/* Apply cJSON to root bin */
#define j2s_json_to_bin_root(ctx, json, ptr, ptr_size, ofname)                 \
  j2s_json_to_bin(ctx, json, NULL, ptr, ptr_size, ofname)
...
extern struct_map_t *struct_map_create(const char *fpath);
extern int struct_map_close(struct_map_t *struct_map);
