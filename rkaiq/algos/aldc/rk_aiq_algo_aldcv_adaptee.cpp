...
LdcvAlgoAdaptee::~LdcvAlgoAdaptee() {
    cached_lut_.clear();
    lut_manger_ = nullptr;
};

XCamReturn LdcvAlgoAdaptee::UpdateMesh(const uint8_t level, const int32_t fd) {
    ENTER_ALDC_FUNCTION();

    LOGD_ALDC_SUBM(ALDC_LDCV_SUBM, "LDCV update mesh fd: correct level %d, cached lut size %d",
    ...
    if (cached_lut_.size() > 0) {
        ...
        LOGD_ALDC_SUBM(ALDC_LDCV_SUBM, "LDCV update chip_in_use_lut_fd_ %d",
...
XCamReturn LdcvAlgoAdaptee::SetApiAttr(const rk_aiq_ldc_ldcv_attrib_t* attr) {
    ENTER_ALDC_FUNCTION();
    if (!attr_) attr_ = std::unique_ptr<rk_aiq_ldc_ldcv_attrib_t>(new rk_aiq_ldc_ldcv_attrib_t);
...
XCamReturn LdcvAlgoAdaptee::GetApiAttr(rk_aiq_ldc_ldcv_attrib_t* attr) {
    ENTER_ALDC_FUNCTION();
...
XCamReturn LdcvAlgoAdaptee::Config(const AlgoCtxInstanceCfg* config, const int32_t conf_type,
    ...
    ctx_config_ = config;
    calib_      = calib;
    enable_     = calib != nullptr ? calib->param.ldcv_en : false;
...
XCamReturn LdcvAlgoAdaptee::Prepare(const RkAiqAlgoConfigAldc* config) {
    ...
    char file_name[600] = {};
    sprintf(file_name, "%s/%s/ldcv_mesh.bin", config->iqpath, calib_->param.ldcv_meshfile);
    LOGD_ALDC_SUBM(ALDC_LDCV_SUBM, "%s : try reading mesh from %s", __FUNCTION__, file_name);
    if (access(file_name, F_OK) == 0) {
        int32_t buf_fd = 0;
        void* lut_buf  = GetFreeLutBuf(0, &buf_fd);
        if (!lut_buf) {
            LOGE_ALDC_SUBM(ALDC_LDCV_SUBM, "%s : Failed to get lut buffer, disable LDCV",
                           __FUNCTION__);
            enable_ = false;
        }
        ...
        if (ReadMeshFromFile(file_name, lut_buf, cached_lut_[0]->Config) < 0) {
            LOGW_ALDC_SUBM(ALDC_LDCV_SUBM, "%s : Failed to read mesh from %s, disable LDCV",
...
void LdcvAlgoAdaptee::OnFrameEvent(const RkAiqAlgoProcAldc* input, RkAiqAlgoProcResAldc* output) {
    ...
    isReCal_        = false;
    output->ldcv_en = enable_;

    auto* result   = &output->config->ldcv;
    ...
    if (chip_in_use_lut_fd_.size() > 0 && chip_in_use_lut_fd_[0] >= 0) {
        ...
        LOGD_ALDC_SUBM(ALDC_LDCV_SUBM, "mesh buf fd is invalid, disable LDCV");
    ...
    result->map13p3_en   = 0;
    result->thumb_mode   = 0;
    result->out_vsize    = height;

    result->hsize = (((width + 15) / 16 + 1) + 1) / 2;
    result->vsize = (height + 15) / 16 + 2;

    memcpy(result->bicubic, ldcv_default_bic_table, sizeof(ldcv_default_bic_table));

    LOGD_ALDC_SUBM(ALDC_LDCV_SUBM, "LDCV result mesh level %d, buf_fd %d, h/vsize: %dx%d",
                   correct_level_, result->buf_fd, result->hsize, result->vsize);
}

void* LdcvAlgoAdaptee::GetFreeLutBuf(int8_t isp_id, int32_t* out_fd) {
    if (lut_manger_ == nullptr) {
        ...
        lut_manger_->ImportHwBuffers(0, MEM_TYPE_LDCV);
    }
    ...
    LOGD_ALDC_SUBM(ALDC_LDCV_SUBM, "LDCV get lut buf fd %d, cached lut size %d", buf->Fd,
                   cached_lut_.size());

    return cached_lut_.size() > 0 ? cached_lut_[0]->Addr : nullptr;
}

XCamReturn LdcvAlgoAdaptee::ReadMeshFromFile(const char* file_name, const void* addr,
    ...
    if (ofp != NULL) {
        ...
        LOGE_ALDC("%s size %d", file_name, file_stat.st_size);
...
#ifdef HAVE_HEADINFO
        if (num != lut_size) {
            LOGE_ALDC("mismatched the size of mesh");
            return XCAM_RETURN_ERROR_FAILED;
        }
        ...
        LOGE_ALDC("lut file %s not exist", file_name);
        return XCAM_RETURN_ERROR_FAILED;
    }
