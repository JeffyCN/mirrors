...
#ifndef _RK_AIQ_ALGO_ALDCH_ALGO_H_
#define _RK_AIQ_ALGO_ALDCH_ALGO_H_
...
class LdchAlgoAdaptee {
 public:
    LdchAlgoAdaptee() = default;
    virtual ~LdchAlgoAdaptee();
    LdchAlgoAdaptee(const LdchAlgoAdaptee&) = delete;
    const LdchAlgoAdaptee& operator=(const LdchAlgoAdaptee&) = delete;

    XCamReturn Config(const AlgoCtxInstanceCfg* config, const int32_t conf_type,
                      const CalibDbV2_LDC_t* calib);
    XCamReturn SetApiAttr(const rk_aiq_ldc_ldch_attrib_t* attr);
    XCamReturn GetApiAttr(rk_aiq_ldc_ldch_attrib_t* attr);
    ...
    rk_aiq_ldc_ldch_attrib_t* GetCurAttrib() const { return attr_ ? attr_.get() : nullptr; }
    ...
    void SetGenMeshState(const LdcGenMeshState state) {
        ...
        printf("set state %d", (int32_t)gen_mesh_state_.load(std::memory_order_acquire));
    }
 ...
 private:
    ...
    std::unique_ptr<rk_aiq_ldc_ldch_attrib_t> attr_ = nullptr;
...
#endif  //_RK_AIQ_ALGO_ALDCH_ALGO_H_
