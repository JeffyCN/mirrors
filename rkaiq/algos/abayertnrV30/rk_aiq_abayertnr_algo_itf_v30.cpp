...
typedef struct _RkAiqAlgoContext {
    Abayertnr_Context_V30_t AbayertnrCtx;
} RkAiqAlgoContext;
...
#if 1
    Abayertnr_Context_V30_t* pAbayertnrCtx = NULL;
#if (ABAYERTNR_USE_JSON_FILE_V30)
    Abayertnr_result_V30_t ret = Abayertnr_Init_V30(&pAbayertnrCtx, cfg->calibv2);
#endif
    if(ret != ABAYERTNRV30_RET_SUCCESS) {
...
#if 1
    Abayertnr_Context_V30_t* pAbayertnrCtx = (Abayertnr_Context_V30_t*)context;
    Abayertnr_result_V30_t ret = Abayertnr_Release_V30(pAbayertnrCtx);
    if(ret != ABAYERTNRV30_RET_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
prepare(RkAiqAlgoCom* params)
{
    ...
    Abayertnr_Context_V30_t* pAbayertnrCtx = (Abayertnr_Context_V30_t *)params->ctx;
    RkAiqAlgoConfigAbayertnrV30* pCfgParam = (RkAiqAlgoConfigAbayertnrV30*)params;
    pAbayertnrCtx->prepare_type = params->u.prepare.conf_type;

    pAbayertnrCtx->stTransPrarms.rawWidth = pCfgParam->stAbayertnrConfig.rawWidth;
    pAbayertnrCtx->stTransPrarms.rawHeight = pCfgParam->stAbayertnrConfig.rawHeight;

    if(!!(params->u.prepare.conf_type & RK_AIQ_ALGO_CONFTYPE_UPDATECALIB )) {
#if ABAYERTNR_USE_JSON_FILE_V30
        void* pCalibDbV2 = (void*)(pCfgParam->com.u.prepare.calibv2);
        CalibDbV2_BayerTnrV30_t *bayertnr_v30 = (CalibDbV2_BayerTnrV30_t*)(CALIBDBV2_GET_MODULE_PTR((void*)pCalibDbV2, bayertnr_v30));
        pAbayertnrCtx->bayertnr_v30 = *bayertnr_v30;
        pAbayertnrCtx->isIQParaUpdate = true;
        pAbayertnrCtx->isReCalculate |= 1;
#endif
    }

    Abayertnr_result_V30_t ret = Abayertnr_Prepare_V30(pAbayertnrCtx, &pCfgParam->stAbayertnrConfig);
    if(ret != ABAYERTNRV30_RET_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
pre_process(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
    ...
    LOGI_ANR("%s: (enter)\n", __FUNCTION__ );
    Abayertnr_Context_V30_t* pAbayertnrCtx = (Abayertnr_Context_V30_t *)inparams->ctx;

    RkAiqAlgoPreAbayertnrV30* pAbayertnrPreParams = (RkAiqAlgoPreAbayertnrV30*)inparams;
    ...
    Abayertnr_result_V30_t ret = Abayertnr_PreProcess_V30(pAbayertnrCtx);
    if(ret != ABAYERTNRV30_RET_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
processing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
...
#if 1
    RkAiqAlgoProcAbayertnrV30* pAbayertnrProcParams = (RkAiqAlgoProcAbayertnrV30*)inparams;
    RkAiqAlgoProcResAbayertnrV30* pAbayertnrProcResParams = (RkAiqAlgoProcResAbayertnrV30*)outparams;
    Abayertnr_Context_V30_t* pAbayertnrCtx = (Abayertnr_Context_V30_t *)inparams->ctx;
    Abayertnr_ExpInfo_V30_t stExpInfo;
    memset(&stExpInfo, 0x00, sizeof(Abayertnr_ExpInfo_V30_t));
    ...
    stExpInfo.isFirstFrame = pAbayertnrCtx->isFirstFrame;
    stExpInfo.gray_mode = pAbayertnrCtx->isGrayMode;
    ...
    if(pAbayertnrProcParams != NULL) {
        ...
        LOGD_ANR("bayertnrv30 predgain:%f \n",
    ...
    if(delta_iso > ABAYERTNRV30_RECALCULATE_DELTA_ISO) {
        pAbayertnrCtx->isReCalculate |= 1;
    }
    ...
    if(pAbayertnrProcParams && pAbayertnrProcParams->stTnr_stats.isValid) {
        if(memcmp(&pAbayertnrProcParams->stTnr_stats, &pAbayertnrCtx->stStats, sizeof(pAbayertnrCtx->stStats))) {
            memcpy(&pAbayertnrCtx->stStats, &pAbayertnrProcParams->stTnr_stats, sizeof(pAbayertnrCtx->stStats));
            Abayertnr_Stats_V30_t *bay3dStat = &pAbayertnrProcParams->stTnr_stats;
            pAbayertnrCtx->isReCalculate |= 1;
        }
    } else {
        LOGD_ANR("%s,%d: stats is valid:%d, frameid:%d\n", __FUNCTION__, __LINE__,
                 pAbayertnrProcParams->stTnr_stats.isValid,
                 pAbayertnrProcParams->stTnr_stats.frame_id);
    }
    pAbayertnrCtx->isReCalculate |= 1;
    if(pAbayertnrCtx->isReCalculate) {
        Abayertnr_result_V30_t ret = Abayertnr_Process_V30(pAbayertnrCtx, &stExpInfo);
        if(ret != ABAYERTNRV30_RET_SUCCESS) {
            result = XCAM_RETURN_ERROR_FAILED;
            LOGE_ANR("%s: processing ANR failed (%d)\n", __FUNCTION__, ret);
        }

        Abayertnr_GetProcResult_V30(pAbayertnrCtx, &pAbayertnrProcResParams->stAbayertnrProcResult);
    ...
    pAbayertnrCtx->isFirstFrame = false;
#endif
...
RkAiqAlgoDescription g_RkIspAlgoDescAbayertnrV30 = {
    .common = {
        .version = RKISP_ALGO_ABAYERTNR_VERSION_V30,
        .vendor  = RKISP_ALGO_ABAYERTNR_VENDOR_V30,
        .description = RKISP_ALGO_ABAYERTNR_DESCRIPTION_V30,
