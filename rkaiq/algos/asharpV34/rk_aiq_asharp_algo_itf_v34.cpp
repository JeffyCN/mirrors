/*
 * rk_aiq_asharp_algo_itf_v34.cpp
 *
...
typedef struct _RkAiqAlgoContext {
    Asharp_Context_V34_t AsharpCtx;
} RkAiqAlgoContext;
...
static XCamReturn create_context(RkAiqAlgoContext** context, const AlgoCtxInstanceCfg* cfg) {
    ...
    Asharp_Context_V34_t* pAsharpCtx = NULL;
#if (ASHARP_USE_JSON_FILE_V34)
    Asharp_result_V34_t ret = Asharp_Init_V34(&pAsharpCtx, cfg->calibv2);
#else
    Asharp_result_V34_t ret = Asharp_Init_V34(&pAsharpCtx, cfg->calib);
#endif

    if (ret != ASHARP_V34_RET_SUCCESS) {
...
static XCamReturn destroy_context(RkAiqAlgoContext* context) {
    ...
    LOG1_ASHARP("%s: oyyf (enter)\n", __FUNCTION__);

    Asharp_Context_V34_t* pAsharpCtx = (Asharp_Context_V34_t*)context;
    Asharp_result_V34_t ret          = Asharp_Release_V34(pAsharpCtx);
    if (ret != ASHARP_V34_RET_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
static XCamReturn prepare(RkAiqAlgoCom* params) {
    ...
    Asharp_Context_V34_t* pAsharpCtx    = (Asharp_Context_V34_t*)params->ctx;
    RkAiqAlgoConfigAsharpV34* pCfgParam = (RkAiqAlgoConfigAsharpV34*)params;
    pAsharpCtx->prepare_type            = params->u.prepare.conf_type;

    if (!!(params->u.prepare.conf_type & RK_AIQ_ALGO_CONFTYPE_UPDATECALIB)) {
#if (ASHARP_USE_JSON_FILE_V34)
#if RKAIQ_HAVE_SHARP_V34
        CalibDbV2_SharpV34_t* calibv2_sharp = (CalibDbV2_SharpV34_t*)(CALIBDBV2_GET_MODULE_PTR(
            pCfgParam->com.u.prepare.calibv2, sharp_v34));
#endif
        // just update calib ptr
        if (params->u.prepare.conf_type & RK_AIQ_ALGO_CONFTYPE_UPDATECALIB_PTR) {
            return XCAM_RETURN_NO_ERROR;
        }
        pAsharpCtx->sharp_v34 = *calibv2_sharp;
#endif
        pAsharpCtx->isIQParaUpdate = true;
        pAsharpCtx->isReCalculate |= 1;
    }

    Asharp_result_V34_t ret = Asharp_Prepare_V34(pAsharpCtx, &pCfgParam->stAsharpConfig);
    if (ret != ASHARP_V34_RET_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
static XCamReturn processing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams) {
    ...
    RkAiqAlgoProcAsharpV34* pAsharpProcParams       = (RkAiqAlgoProcAsharpV34*)inparams;
    RkAiqAlgoProcResAsharpV34* pAsharpProcResParams = (RkAiqAlgoProcResAsharpV34*)outparams;
    Asharp_Context_V34_t* pAsharpCtx                = (Asharp_Context_V34_t*)inparams->ctx;
    Asharp_ExpInfo_V34_t stExpInfo;
    memset(&stExpInfo, 0x00, sizeof(Asharp_ExpInfo_V34_t));

    LOGD_ASHARP("%s:%d frame_id:%d init:%d hdr mode:%d  \n", __FUNCTION__, __LINE__,
                inparams->frame_id, inparams->u.proc.init, pAsharpProcParams->hdr_mode);
    ...
    Asharp_result_V34_t ret = Asharp_PreProcess_V34(pAsharpCtx);
    if (ret != ASHARP_V34_RET_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
    ...
    for (int i = 0; i < 3; i++) {
        stExpInfo.arIso[i]   = 50.0f;
        stExpInfo.arAGain[i] = 1.0f;
        stExpInfo.arDGain[i] = 1.0f;
        stExpInfo.arTime[i]  = 0.01f;
    }
    ...
    if (curExp != NULL) {
        ...
        if (pAsharpProcParams->hdr_mode == RK_AIQ_WORKING_MODE_NORMAL) {
            ...
            if (stExpInfo.blc_ob_predgain < 1.0f) {
                stExpInfo.blc_ob_predgain = 1.0f;
            }
            ...
            LOGD_ASHARP("new snr mode:%d old:%d  gain:%f isp_dgain:%f iso:%d time:%f\n",
                        stExpInfo.snr_mode, pAsharpCtx->stExpInfo.snr_mode, stExpInfo.arDGain[0],
                        stExpInfo.isp_dgain[0], stExpInfo.arIso[0], stExpInfo.arTime[0]);
        } else {
            for (int i = 0; i < 3; i++) {
                if (curExp->HdrExp[i].exp_real_params.analog_gain < 1.0f) {
                    stExpInfo.arAGain[i] = 1.0f;
                    LOGW_ASHARP("hdr mode again is wrong, use 1.0 instead\n");
                } else {
                    stExpInfo.arAGain[i] = curExp->HdrExp[i].exp_real_params.analog_gain;
                }
                if (curExp->HdrExp[i].exp_real_params.digital_gain < 1.0f) {
                    stExpInfo.arDGain[i] = 1.0f;
                    LOGW_ASHARP("hdr mode dgain is wrong, use 1.0 instead\n");
                } else {
                    stExpInfo.arDGain[i] = curExp->HdrExp[i].exp_real_params.digital_gain;
                }
                if (curExp->HdrExp[i].exp_real_params.isp_dgain < 1.0f) {
                    stExpInfo.isp_dgain[i] = 1.0f;
                ...
                stExpInfo.arIso[i] =
                    stExpInfo.arAGain[i] * stExpInfo.arDGain[i] * 50.0f * stExpInfo.arDGain[i];
                ...
                LOGD_ASHARP(
                    ...
                    stExpInfo.arDGain[i], stExpInfo.arTime[i], stExpInfo.arIso[i],
                    stExpInfo.hdr_mode);
            }
        }
    } else {
    ...
    if (pAsharpCtx->isReCalculate) {
        Asharp_result_V34_t ret = Asharp_Process_V34(pAsharpCtx, &stExpInfo);
        if (ret != ASHARP_V34_RET_SUCCESS) {
            result = XCAM_RETURN_ERROR_FAILED;
            LOGE_ASHARP("%s: processing ANR failed (%d)\n", __FUNCTION__, ret);
        }

        Asharp_GetProcResult_V34(pAsharpCtx, &pAsharpProcResParams->stAsharpProcResult);
...
RkAiqAlgoDescription g_RkIspAlgoDescAsharpV34 = {
    .common =
        {
            .version         = RKISP_ALGO_ASHARP_VERSION_V34,
            .vendor          = RKISP_ALGO_ASHARP_VENDOR_V34,
            .description     = RKISP_ALGO_ASHARP_DESCRIPTION_V34,
