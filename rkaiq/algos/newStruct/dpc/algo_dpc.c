...
create_context(RkAiqAlgoContext** context, const AlgoCtxInstanceCfg* cfg)
{
    ...
    DpcContext_t* ctx = aiq_mallocz(sizeof(DpcContext_t));
    if (ctx == NULL) {
...
processing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
    ...
    if (pDpcCtx->isReCal_) {
        DpcSelectParam(pDpcCtx, outparams->algoRes, iso);
...
XCamReturn DpcSelectParam(DpcContext_t* pDpcCtx, dpc_param_t* out, int iso) {
    ...
    for (int i = 0; i < DPC_PDAF_POINT_NUM; i++) {
        out->sta.spc.spInZone.hw_dpcCfg_spcEnInZone_en[i] =
            paut->sta.spc.spInZone.hw_dpcCfg_spcEnInZone_en[i];
        out->sta.spc.spInZone.hw_dpcCfg_spInZoneCoord_x[i] =
            paut->sta.spc.spInZone.hw_dpcCfg_spInZoneCoord_x[i];
        out->sta.spc.spInZone.hw_dpcCfg_spInZoneCoord_y[i] =
            paut->sta.spc.spInZone.hw_dpcCfg_spInZoneCoord_y[i];
    }
