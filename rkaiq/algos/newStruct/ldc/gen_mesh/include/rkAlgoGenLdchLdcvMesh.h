...
#include <opencv2/opencv.hpp> /* 在Windows C++ 环境上仿真验证效果 */
#endif
...
typedef struct RKALGO_LDCH_LDCV_INFO {
    ...
    uint32_t ldcvAlignMeshW;     /* mesh表的宽做2对齐 */
...
/*
    ...
    [input/output]  : pu16MeshX, pu16MeshY: 输出的LDCH/LDCV的定点化表, 根据buffer的指针是否为空,
   有以下输出的组合，适配不同需求: (1)NULL!=pf32MapxNew && NULL!=pu16MeshX:
   则输出用于LDCH的定点化表; (2)NULL!=pf32MapyNew && NULL!=pu16MeshY: 则输出用于LDCV的定点化表;
                                (3)以上(1)和(2)可以同时输出。
*/
int32_t genLdchLdcvMeshFromFloatMesh(const RKALGO_LDCH_LDCV_INFO_S* pstLdchLdcvInfo,
...
*/
int32_t genLdchLdcvMeshFromBigMap(const RKALGO_LDCH_LDCV_INFO_S* pstLdchLdcvInfo,
...
/*
   ...
   则输出用于LDCH的定点化表; (2)NULL!=pf32MapyNew && NULL!=pu16MeshY: 则输出用于LDCV的定点化表;
                                (3)以上(1)和(2)可以同时输出。
*/
int32_t genLdchLdcvMeshFromFloatMesh_old(
    const RKALGO_LDCH_LDCV_INFO_S* pstLdchLdcvInfo, const uint32_t ldchFloatMeshW,
    const uint32_t ldchFloatMeshH, const uint32_t ldcvFloatMeshW, const uint32_t ldcvFloatMeshH,
    const float* pf32MapxNew, const float* pf32MapyNew, uint16_t* pu16MeshX, uint16_t* pu16MeshY);
