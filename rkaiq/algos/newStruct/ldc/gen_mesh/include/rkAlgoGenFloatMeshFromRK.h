...
typedef struct RKALGO_CAM_COEFF_INFO {
    ...
    double invPolyTanCoeff0[INV_POLY_COEFF_NUM]; /* level = 0时的rho-tanTheta多项式系数,
                                                    最高次数(INV_POLY_COEFF_NUM-1)次 */

    int invPolyCotNum0;                          /* level = 0时的rho-cotTheta多项式的系数个数 */
    double invPolyCotCoeff0[INV_POLY_COEFF_NUM]; /* level = 0时的rho-cotTheta多项式系数,
                                                    最高次数(INV_POLY_COEFF_NUM-1)次 */

    int invPolyTanNum255;                          /* level = 255时的rho-tanTheta多项式的系数个数 */
    double invPolyTanCoeff255[INV_POLY_COEFF_NUM]; /* level = 255时的rho-tanTheta多项式系数,
                                                      最高次数(INV_POLY_COEFF_NUM-1)次 */

    int invPolyCotNum255;                          /* level = 255时的rho-cotTheta多项式的系数个数 */
    double invPolyCotCoeff255[INV_POLY_COEFF_NUM]; /* level = 255时的rho-cotTheta多项式系数,
                                                      最高次数(INV_POLY_COEFF_NUM-1)次 */
...
typedef struct RKALGO_CAM_COEFF_ONE_LEVEL_INFO {
    ...
    double invPolyTanCoeff[INV_POLY_COEFF_NUM]; /* level = 0时的rho-tanTheta多项式系数,
                                                   最高次数(INV_POLY_COEFF_NUM-1)次 */
    int invPolyCotNum;                          /* rho-cotTheta多项式的系数个数 */
    double invPolyCotCoeff[INV_POLY_COEFF_NUM]; /* level = 0时的rho-cotTheta多项式系数,
                                                   最高次数(INV_POLY_COEFF_NUM-1)次 */
...
typedef struct RKALGO_GEN_FLOAT_MESH_INFO {
    ...
    RKALGO_LDCH_VERSION_E
        enLdchVersion; /* 硬件LDCH版本(后级没有LDCV)，例如rv1109，rv1126，rk356x，rk3588，1106，rk3562
                        */
...
int32_t genFloatMeshNLevelInit(RKALGO_CAM_COEFF_INFO_S* camCoeff,
                               RKALGO_GEN_FLOAT_MESH_INFO_S* genFloatMeshInfo);

/* 外部接口：生成不同校正程度的下采样浮点表，采样步长可配 */
int32_t genFloatMeshNLevel(const RKALGO_CAM_COEFF_INFO_S* camCoeff,
                           const RKALGO_GEN_FLOAT_MESH_INFO_S* genFloatMeshInfo, uint32_t level,
