...
*/
EXPORT int32_t calcSeparateMeshInfo(const uint32_t srcW, const uint32_t srcH, const uint32_t dstW,
                                    const uint32_t dstH,
...
*/
EXPORT int32_t genSeparateMeshFromOriMap(const RKALGO_SEPARATE_MESH_INFO_S* pstMapSepaInfo,
                                         const float* pf32MapxOri, const float* pf32MapyOri,
                                         float* pf32MapxNew, float* pf32MapyNew);

#if defined(__cplusplus) && (defined(WIN32) || defined(_WIN32))
int32_t genSeparateBigMapFromOriMap(const uint32_t bigMapWid, const uint32_t bigMapHgt,
                                    const cv::Mat& oriBigMapX, const cv::Mat& oriBigMapY,
                                    cv::Mat& newBigMapX, cv::Mat& newBigMapY);
