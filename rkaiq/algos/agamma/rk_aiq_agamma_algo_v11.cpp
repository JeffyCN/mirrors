...
void AgammaSetProcRes(AgammaProcRes_t* AgammaProcRes, AgammaHandle_t* pGammaCtx, bool bypass) {
    ...
    AgammaProcRes->Gamma_v11.gamma_en = pGammaCtx->agamma_config.gamma_en;
    if (AgammaProcRes->update) {
        ...
        AgammaProcRes->Gamma_v11.equ_segm = pGammaCtx->agamma_config.gamma_out_segnum > 1
                                                ? ISP3X_SEGNUM_LOG_45
                                                : pGammaCtx->agamma_config.gamma_out_segnum;
        AgammaProcRes->Gamma_v11.offset = pGammaCtx->agamma_config.gamma_out_offset;
        for (int i = 0; i < CALIBDB_AGAMMA_KNOTS_NUM_V11; i++)
            AgammaProcRes->Gamma_v11.gamma_y[i] = pGammaCtx->agamma_config.gamma_table[i];
    }
