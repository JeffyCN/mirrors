...
#if 1
    Abayernr_Context_V1_t* pAbayernrCtx = (Abayernr_Context_V1_t*)context;
    Abayernr_result_v1_t ret = Abayernr_Release_V1(pAbayernrCtx);
    if(ret != ABAYERNR_RET_V1_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
prepare(RkAiqAlgoCom* params)
{
	...
	Abayernr_result_v1_t ret = Abayernr_Prepare_V1(pAbayernrCtx, &pCfgParam->stArawnrConfig);	
    if(ret != ABAYERNR_RET_V1_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
pre_process(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
    ...
    Abayernr_result_v1_t ret = Abayernr_PreProcess_V1(pAbayernrCtx);
    if(ret != ABAYERNR_RET_V1_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
...
processing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
    ...
    Abayernr_result_v1_t ret = Abayernr_Process_V1(pAbayernrCtx, &stExpInfo);
    if(ret != ABAYERNR_RET_V1_SUCCESS) {
        result = XCAM_RETURN_ERROR_FAILED;
        LOGE_ANR("%s: processing ANR failed (%d)\n", __FUNCTION__, ret);
    }

    Abayernr_GetProcResult_V1(pAbayernrCtx, &pAnrProcResParams->stArawnrProcResult);
#endif
...
RkAiqAlgoDescription g_RkIspAlgoDescArawnr = {
    .common = {
        .version = RKISP_ALGO_ABAYERNR_VERSION_V1,
        .vendor  = RKISP_ALGO_ABAYERNR_VENDOR_V1,
        .description = RKISP_ALGO_ABAYERNR_DESCRIPTION_V1,
