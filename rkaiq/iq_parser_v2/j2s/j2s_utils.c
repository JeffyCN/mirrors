...
int j2s_alloc_map_record(j2s_ctx *ctx, void *dst, void *ptr, size_t len) {
    ...
    if (!j2s_pool->maps_list) {
        j2s_pool->maps_list = (map_index_t *)aiq_mallocz(1 * sizeof(map_index_t));
        j2s_pool->map_len = 1;
    } else {
        j2s_pool->map_len++;
...
void *j2s_alloc_data2(j2s_ctx *ctx, size_t size, size_t* real_size) {
    ...
    if (!j2s_pool) {
        ctx->priv = aiq_mallocz(sizeof(j2s_pool_t));
    ...
    if (!j2s_pool->data) {
        j2s_pool->data = (uint8_t *)aiq_mallocz(J2S_POOL_SIZE);
...
int j2s_add_data(j2s_ctx* ctx, void* ptr, bool freeable)
{
    ...
    if (!ctx->priv)
        ctx->priv = aiq_mallocz(1 * sizeof(j2s_priv_data));
    ...
    void* auth_data, int auth_size)
{
    ...
    if (auth_data && auth_size) {
        void* buf = aiq_mallocz(auth_size);
        if (!buf)
            goto out;
