...
CamCalibDbV2Context_t* calibdbV2_ctx_new() {
    ...
    calib_scene = aiq_mallocz(sizeof(CamCalibDbV2ContextIsp20_t));
#elif defined(ISP_HW_V21)
    calib_scene = aiq_mallocz(sizeof(CamCalibDbV2ContextIsp21_t));
#elif defined(ISP_HW_V30)
    calib_scene = aiq_mallocz(sizeof(CamCalibDbV2ContextIsp30_t));
#elif defined(ISP_HW_V32) || defined(ISP_HW_V32_LITE)
    calib_scene = aiq_mallocz(sizeof(CamCalibDbV2ContextIsp32_t));
    ...
    CamCalibDbV2Context_t* calib_ctx = aiq_mallocz(sizeof(CamCalibDbV2Context_t));
    calib_ctx->calib_scene = (char*)calib_scene;

    calib_ctx->sensor_info = aiq_mallocz(sizeof(CalibDb_Sensor_ParaV2_t));

    calib_ctx->module_info = aiq_mallocz(sizeof(CalibDb_Module_ParaV2_t));
...
void calibdbV2_ctx_delete(CamCalibDbV2Context_t* calib_ctx) {
    if (calib_ctx->calib_scene)
        aiq_free(calib_ctx->calib_scene);
    if (calib_ctx->sensor_info)
        aiq_free(calib_ctx->sensor_info);
...
static CamCalibDbMap gCalibDbsMap[CAMCALIBDB_MAP_LEN];
static int gCalibDbsMapCnt = 0;
static AiqMutex_t gCamCalibDbMutex = PTHREAD_MUTEX_INITIALIZER;
...
CamCalibDbProj_t *CamCalibDbProjAlloc() {
    CamCalibDbProj_t *calibproj =
        (CamCalibDbProj_t *)aiq_mallocz(sizeof(CamCalibDbProj_t));
    if (!calibproj) {
        XCAM_LOG_ERROR("malloc for new calibv2 failed.");
        return NULL;
    }
...
CamCalibDbProj_t *CamCalibDbJson2calibprojFromFile(const char *jsfile) {
    ...
    base_json = RkCam_cJSON_Parse(json_buff);
    if (!base_json) {
        aiq_free(json_buff);
        return NULL;
    }
    ...
    if (!RkAiqSceneManagerMergeMultiSceneIQ(base_json)) {
...
#ifdef IQ_DEBUG
    CamCalibDbCalibproj2json("/tmp/iq_dump.json", calibproj);
#endif
...
CamCalibDbProj_t *CamCalibDbJson2calibprojFromStr(const char *jstr, size_t len) {
    ...
    base_json = RkCam_cJSON_Parse(jstr);
    if (!base_json) {
        return NULL;
    }
...
void CamCalibDbBin2calib(void *bin_buff, void *struct_ptr) {
    ...
    for (int i = 0; i < header->block_len; i++) {
        ...
        if (strcmp("CalibDb_Sensor_ParaV2_t", block_list[i].name) == 0)
            sns_ptr = (CalibDb_Sensor_ParaV2_t *)CamCalibDbBin2CalibprojFromBuf(buf_start + block_list[i].offset, block_list[i].size);
        else if (strcmp("CalibDb_Aec_ParaV2_t", block_list[i].name) == 0)
            ae_ptr = (CalibDb_Aec_ParaV2_t *)CamCalibDbBin2CalibprojFromBuf(buf_start + block_list[i].offset, block_list[i].size);
        else if (strcmp("CalibDbV2_Bayer2dnrV23_t", block_list[i].name) == 0)
            bayer2dnr_v23_ptr = (CalibDbV2_Bayer2dnrV23_t *)CamCalibDbBin2CalibprojFromBuf(buf_start + block_list[i].offset, block_list[i].size);
        else if (strcmp("CalibDbV2_BayerTnrV23_t", block_list[i].name) == 0)
            bayertnr_v23_ptr = (CalibDbV2_BayerTnrV23_t *)CamCalibDbBin2CalibprojFromBuf(buf_start + block_list[i].offset, block_list[i].size);
        else if (strcmp("CalibDbV2_Wb_Para_V32_t", block_list[i].name) == 0)
            wb_v32 = (CalibDbV2_Wb_Para_V32_t *)CamCalibDbBin2CalibprojFromBuf(buf_start + block_list[i].offset, block_list[i].size);
        else if (strcmp("CalibDbV2_Blc_V32_t", block_list[i].name) == 0)
            ablcV32_calib = (CalibDbV2_Blc_V32_t *)CamCalibDbBin2CalibprojFromBuf(buf_start + block_list[i].offset, block_list[i].size);
    }
...
CamCalibDbProj_t *CamCalibDbBin2CalibprojFromFile(const char *binfile) {
    ...
    ret = CamCalibDbCheckBinVersion((uint8_t*)bin_buff, bin_size);
    if (ret) {
...
CamCalibDbProj_t *CamCalibDbBin2CalibprojFromBuf(const void *bin_buff, size_t len) {
    ...
    ret = CamCalibDbCheckBinVersion((uint8_t*)bin_buff, len);
    if (ret) {
        LOGW("iq bin version is no current");
    }

    ret = CamCalibDbParseBinStructMap((uint8_t*)bin_buff, len);
    if (ret) {
        return NULL;
    }
...
CamCalibDbCamgroup_t* CamCalibDbCreateCalibDbCamgroup(const char *jsfile) {
    ...
    CamCalibDbCamgroup_t* calib_camgroup =
        (CamCalibDbCamgroup_t*)aiq_mallocz(1 * sizeof(CamCalibDbCamgroup_t));
...
{
    if (calib_camgroup)
        aiq_free(calib_camgroup);
    return 0;
}

int CamCalibDbCalibproj2json(const char *jsfile,
                                   CamCalibDbProj_t *calibproj) {
    char *json_buff = NULL;
    j2s_ctx ctx;
    FILE* fp;
...
CamCalibDbProj_t *CamCalibDbCreateCalibDbProjFromFile(int sns_id, const char *jsfile) {
    ...
    CamCalibDbProj_t *calibproj = NULL;
    char binFile[CAMCALIBDB_JSFILE_LEN+4];
    char* subStr;
    int find_idx = -1;

    aiqMutex_lock(&gCamCalibDbMutex);
    for (int i = 0; i < gCalibDbsMapCnt; i++) {
        CamCalibDbProj_t *calibproj = gCalibDbsMap[i].calibproj;
		/* first 2 chars are: <sns_id>- */
        if (atoi(gCalibDbsMap[i].jsFileName) == sns_id &&
			strcmp(gCalibDbsMap[i].jsFileName + 2, jsfile) == 0) {
            find_idx = i;
            break;
        }
    }
    ...
    if (find_idx != -1) {
        ...
        aiqMutex_unlock(&gCamCalibDbMutex);
        return gCalibDbsMap[find_idx].calibproj;
    } else {
        binFile[sizeof(binFile)-1] = 0;
        strncpy(binFile, jsfile, sizeof(binFile)-1);
        subStr = strrchr(binFile, '.');
        if (subStr != NULL) {
            strcpy(subStr, ".bin");
        }
        if (0 == access(jsfile, F_OK)) {
            calibproj = CamCalibDbJson2calibprojFromFile(jsfile);
        } else if (0 == access(binFile, F_OK)) {
            calibproj = CamCalibDbBin2CalibprojFromFile(binFile);
        } else {
            XCAM_LOG_ERROR("access %s && %s failed!", jsfile, binFile);
            aiqMutex_unlock(&gCamCalibDbMutex);
            return NULL;
        }

        if (calibproj) {
			sprintf(gCalibDbsMap[gCalibDbsMapCnt].jsFileName, "%1d-",sns_id);
            strcat(gCalibDbsMap[gCalibDbsMapCnt].jsFileName, jsfile);
            gCalibDbsMap[gCalibDbsMapCnt].calibproj = calibproj;
            gCalibDbsMapCnt++;
...
CamCalibDbProj_t *CamCalibDbCreateCalibDbProjFromBuf(int sns_id, const void *bin_buff, size_t len) {
    ...
    aiqMutex_lock(&gCamCalibDbMutex);
    calibproj = CamCalibDbBin2CalibprojFromBuf(bin_buff, len);
...
CamCalibDbToDefaultCalibDb(CamCalibDbProj_t *calibproj) {
...
CamCalibDbV2Context_t *
CamCalibDbApplyPatchFromcJSON(const CamCalibDbV2Context_t *calib, cJSON *patch) {
    cJSON *base_json = NULL;
    int ret = -1;
    ...
    base_json = CamCalibDbCalib2cjson(calib);
    ...
    ret = RkCam_cJSONUtils_ApplyPatches(base_json, patch);
    if (0 != ret) {
        XCAM_LOG_ERROR("%s apply patch failed %d!", __func__, ret);
        return NULL;
    }

    return CamCalibDbCjson2calib(base_json);
}

CamCalibDbV2Context_t *
CamCalibDbApplyPatch(const CamCalibDbV2Context_t *calib, const char *patch_str) {
    cJSON *patch_json = NULL;

    patch_json = RkCam_cJSON_Parse(patch_str);

    return CamCalibDbApplyPatchFromcJSON(calib, patch_json);
}

static char *find_first_not_of(const char *s, const char ch)
{
    if (NULL == s)
        return NULL;
...
static char *find_last_not_of(char *str, char c)
{
    char *p = NULL;

    for (char *px=str+strlen(str); px >= str; px--)
    {
        if (*px != c)
        {
            p = px;
...
CamCalibDbV2Context_t *
CamCalibDbApplyPatch2(const CamCalibDbV2Context_t *calib, cJSON *patch) {
    //const std::lock_guard<std::mutex> lock(RkAiqCalibDbV2::calib_mutex);
    ...
    aiqMutex_lock(&gCamCalibDbMutex);
    if (!calib || !patch) {
        LOGE_IPC("%s input invalied!", __func__);
        aiqMutex_unlock(&gCamCalibDbMutex);
        return NULL;
    }

    change_sum = RkCam_cJSON_GetArraySize(patch);

    if (change_sum <= 0) {
        LOGE_IPC("[Tuning]: analyz change failed!\n");
        aiqMutex_unlock(&gCamCalibDbMutex);
        return NULL;
    }
    ...
    for (int i = 0; i <= (change_sum - 1); ++i) {
        if (arr_item) {
            char module_name[CAMCALIBDB_MODULE_NAME_LEN];
            char *path_str = RkCam_cJSON_GetObjectItem(arr_item, JSON_PATCH_PATH)->valuestring;
            char *first_p = find_first_not_of(path_str, '/');
            char *last_p = find_last_not_of(path_str, '/');
            char *tmp = (char *)aiq_mallocz(strlen(path_str)+16);
            ...
            memcpy(tmp, first_p, last_p - first_p);
            last_p = strchr(tmp, '/');
            memset(module_name, 0, sizeof(module_name));
            if (last_p)
                memcpy(module_name, tmp, last_p - tmp);
            else
            ...
            sprintf(tmp, "/calib_scene%s", path_str);
            RkCam_cJSON_ReplaceItemInObject(arr_item, JSON_PATCH_PATH,
                                      RkCam_cJSON_CreateString(tmp));
            aiq_free(tmp);
        }
    ...
    LOGD_IPC("[Tuning]: calibdbV2_to_tuningdb");
    ...
    if (!base_json) {
        LOGE_IPC("create CamCalibDbV2Tuning_t failed.");
        goto patch_failed;
    }

    ret = RkCam_cJSONUtils_ApplyPatches(base_json, patch);
    if (0 != ret) {
        LOGE_IPC("%s apply patch failed %d!", __func__, ret);
        goto patch_failed;
    }
    ...
    new_calib = CamCalibDbCalibV2Alloc();
...
CamCalibDbApplyPatchFile(const CamCalibDbV2Context_t *calib, const char *patch_file) {
    ...
    return CamCalibDbApplyPatch(calib, json_str);
}

RkAiqAlgoType_t CamCalibDbString2algostype(const char *str) {
    typedef struct algoTable_s {
        char algoName[32];
        RkAiqAlgoType_t algoType;
    } algoTable;

    static algoTable const table[] = {
    ...
    for (unsigned int i = 0; i < sizeof(table)/sizeof(algoTable); i++) {
        if (strcmp(table[i].algoName, str) == 0) {
            ret = table[i].algoType;
        }
    }
...
int CamCalibDbAnalyzChange(cJSON *patch, TuningCalib *tuning_calib) {
    ...
    LOGD_IPC("[Tuning]: %d changes in this cmd!\n", change_sum);
    if (change_sum <= 0) {
        LOGE_IPC("[Tuning]: analyz change failed!\n");
        return -1;
    }
    ...
    tuning_calib->moduleNamesSize = 0;
    for (int i = 0; i <= (change_sum - 1); ++i) {
        if (arr_item) {
            char module_name[CAMCALIBDB_MODULE_NAME_LEN];
            char *path_str = RkCam_cJSON_GetObjectItem(arr_item, JSON_PATCH_PATH)->valuestring;
            ...
            bool find_flg = false;
            for (int k = 0; k < tuning_calib->moduleNamesSize; k++) {
                if (strcmp(tuning_calib->moduleNames[k], module_name) == 0) {
                    find_flg = true;
                    break;
                }
            }
            ...
            if (!find_flg) {
                LOGD_IPC("[Tuning]: %s changed!\n", module_name);
                strncpy(tuning_calib->moduleNames[tuning_calib->moduleNamesSize], module_name, CAMCALIBDB_MODULE_NAME_LEN);
                tuning_calib->moduleNamesSize++;
            }
        }
        arr_item = arr_item->next;
    }
...
int
CamCalibDbAnalyzTuningCalibFromcJson(const CamCalibDbV2Context_t *calib,
                                  cJSON *patch, TuningCalib *tuning_calib) {
    CamCalibDbAnalyzChange(patch, tuning_calib);
    tuning_calib->calib = CamCalibDbApplyPatch2(calib, patch);

    if (patch)
        RkCam_cJSON_Delete(patch);

    return 0;
}

int
CamCalibDbAnalyzTuningCalibFromStr(const CamCalibDbV2Context_t *calib,
                                  const char *patch_str, TuningCalib *tuning_calib) {
    cJSON *patch_json = NULL;

    patch_json = RkCam_cJSON_Parse(patch_str);

    return CamCalibDbAnalyzTuningCalibFromcJson(calib, patch_json, tuning_calib);
}
...
cJSON *CamCalibDbReadIQNode(const CamCalibDbV2Context_t *calib,
    ...
    cJSON *ret_json = NULL;
    char module_name[CAMCALIBDB_MODULE_NAME_LEN];
    char *path_str = (char *)node_path;
    char *first_p = find_first_not_of(path_str, '/');
    char *last_p = find_last_not_of(path_str, '/');
    char *tmp = (char *)aiq_mallocz(strlen(path_str)+16);

    if (tmp == NULL) {
        XCAM_LOG_INFO("%s: malloc failed!\n", __func__);
        return NULL;
    }
    ...
    if (strcmp(module_name, "sensor_calib") == 0 ||
        strcmp(module_name, "module_calib") == 0 ||
        strcmp(module_name, "sys_static_cfg") == 0) {
        j2s_ctx ctx;
        ctx.format_json = true;
        CamCalibDbV2Tuning_t tuning_base;
    ...
    cJSON *node_json = RkCam_cJSONUtils_GetPointer(base_json, node_path);
    if (node_json) {
        ret_json = RkCam_cJSON_Duplicate(node_json, 1);
    }
...
char* CamCalibDbReadIQNodeStr(const CamCalibDbV2Context_t* calib,
                                    const char* node_path) {
    cJSON* node_json = CamCalibDbReadIQNode(calib, node_path);
    if (!node_json) {
        XCAM_LOG_ERROR("%s find json node failed!", __func__);
        return NULL;
    }

    return RkCam_cJSON_Print(node_json);
}
...
char* CamCalibDbReadIQNodeStrFromJstr(const CamCalibDbV2Context_t* calib,
    ...
    request_json = RkCam_cJSON_Parse(json_str);
    request_sum = RkCam_cJSON_GetArraySize(request_json);
    ...
    for (int i = 0; i <= (request_sum - 1); ++i) {
        if (arr_item) {
            char* path_str = RkCam_cJSON_GetObjectItem(arr_item, JSON_PATCH_PATH)->valuestring;
            cJSON* ret_node = CamCalibDbReadIQNode(calib, path_str);
            if (ret_node) {
                cJSON* ret_item = RkCam_cJSON_CreateObject();
                RkCam_cJSON_AddStringToObject(ret_item, JSON_PATCH_PATH, path_str);
                RkCam_cJSON_AddItemToObject(ret_item, JSON_PATCH_VALUE, ret_node);
    ...
    RkCam_cJSON_Delete(request_json);
...
int CamCalibDbFreeSceneCtx(void* scene_ctx)
{
    return 0;
}
...
#endif
#if RKAIQ_HAVE_GIC_V21 && (USE_NEWSTRUCT==0)
