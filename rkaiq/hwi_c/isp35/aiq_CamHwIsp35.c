...
static void CamHwIsp35_updateEffParams(AiqCamHwBase_t* pCamHw, void* params, void* ori_params) {
#if defined(ISP_HW_V35)
    struct isp35_isp_params_cfg* isp_params = (struct isp35_isp_params_cfg*)params;
    ...
    struct isp35_isp_params_cfg* lates_isp_params_ptr = NULL;
    struct isp35_isp_params_cfg* new_isp_params       = NULL;
...
#define UPDATE_EFF_PARAMS()                                                                       \
    ...
    if (g_mIsMultiIspMode) {
        dst_isp_params = &pDstEff->isp_params_v35[0];
        if (is_got_latest_params) lates_isp_params_ptr = &latestIspParams->isp_params_v35[0];
        new_isp_params = (struct isp35_isp_params_cfg*)ori_params;
        UPDATE_EFF_PARAMS();

        dst_isp_params = &pDstEff->isp_params_v35[1];
        if (is_got_latest_params) lates_isp_params_ptr = &latestIspParams->isp_params_v35[1];
        new_isp_params = (struct isp35_isp_params_cfg*)ori_params;
        UPDATE_EFF_PARAMS();
    }
#endif
...
bool CamHwIsp35_processTb(AiqCamHwBase_t* pCamHw, void* params) {
#if defined(ISP_HW_V35)
    struct isp35_isp_params_cfg* isp_params = (struct isp35_isp_params_cfg*)params;
        ...
        if (pDstEff) {
            ...
            if (pCamHw->_mIspParamsCvt->mAwbParams && pCamHw->_first_awb_param) {
                ...
                *awb_meas_cfg        = *first_awb_param;
                pDstEff->awb_cfg_v35 = *awb_meas_cfg;
            }
            LOGK_CAMHW("run here %s:%d %s", __FUNCTION__, __LINE__, awb_meas_cfg->timeSign);
        }
    ...
    if (pCamHw->mTbInfo.is_fastboot) {
        if (isp_params->frame_id == 0 && pCamHw->_not_skip_first) {
            ...
            if (!pCamHw->_first_awb_cfg) {
                pCamHw->_first_awb_cfg = aiq_mallocz(sizeof(struct isp35_rawawb_meas_cfg));
                *((struct isp35_rawawb_meas_cfg*)pCamHw->_first_awb_cfg) = isp_params->meas.rawawb;
            }
            if (!pCamHw->_skipped_params) {
                pCamHw->_skipped_params = aiq_mallocz(sizeof(struct isp35_isp_params_cfg));
                *((struct isp35_isp_params_cfg*)pCamHw->_skipped_params) = *isp_params;
            ...
            if (isp_params->frame_id < 10) {
                if (pCamHw->_first_awb_cfg) {
                    struct isp35_rawawb_meas_cfg* first_awb_cfg =
                        (struct isp35_rawawb_meas_cfg*)pCamHw->_first_awb_cfg;
                ...
                aiq_isp_effect_params_t* pDstEff =
                    AiqCamHw_getParamsForEffMap(pCamHw, isp_params->frame_id);
                if (pDstEff) {
                    rk_aiq_isp_awb_meas_cfg_v35_t* awb_meas_cfg =
                        (rk_aiq_isp_awb_meas_cfg_v35_t*)pCamHw->_mIspParamsCvt->mAwbParams->_data;
                    if (pCamHw->_mIspParamsCvt->mAwbParams && pCamHw->_first_awb_param) {
                        rk_aiq_isp_awb_meas_cfg_v35_t* first_awb_param =
                            (rk_aiq_isp_awb_meas_cfg_v35_t*)pCamHw->_first_awb_param;
                        memcpy(first_awb_param->preWbgainSw, awb_meas_cfg->preWbgainSw,
                               4 * sizeof(float));
                        *awb_meas_cfg        = *first_awb_param;
                        pDstEff->awb_cfg_v35 = *awb_meas_cfg;
            ...
            struct isp35_isp_params_cfg* skip_param =
                (struct isp35_isp_params_cfg*)pCamHw->_skipped_params;
            struct isp35_isp_params_cfg* new_param  = (struct isp35_isp_params_cfg*)params;
            ...
            new_param->module_en_update &= ~ISP35_MODULE_BAY3D;
            new_param->module_cfg_update &= ~ISP35_MODULE_BAY3D;
...
XCamReturn CamHwIsp35_saveInfoForStartAgainTb(AiqCamHwBase_t* pCamHw) {
#if defined(ISP_HW_V35)
    ...
    fwrite(&ispParams->awb_cfg_v35, sizeof(awbStats_cfg_priv_t), 1, fp);
    fclose(fp);

    LOGK_CAMHW("<TB> sync to file, pclock %f, awb time(%s)",
        pCamHw->mTbInfo.pixel_clock_freq_mhz, ispParams->awb_cfg_v35.timeSign);
...
XCamReturn CamHwIsp35_getLastEffectParamTb(AiqCamHwBase_t* pCamHw) {
#if defined(ISP_HW_V35)
    char fastboot_info_path[32];
    snprintf(fastboot_info_path, 32, "%s%d", AIQCAM_FASTBOOT_FILE_PREFIX, pCamHw->mCamPhyId);
...
XCamReturn AiqCamHwIsp35_init(AiqCamHwBase_t* pCamHw, const char* sns_ent_name) {
    XCamReturn ret                    = AiqCamHwBase_init(pCamHw, sns_ent_name);
    pCamHw->mVicapIspPhyLinkSupported = true;

    pCamHw->updateEffParams      = CamHwIsp35_updateEffParams;
    pCamHw->processTb            = CamHwIsp35_processTb;
    pCamHw->getLastEffectParamTb = CamHwIsp35_getLastEffectParamTb;
    pCamHw->saveInfotoFileTb     = CamHwIsp35_saveInfoForStartAgainTb;
