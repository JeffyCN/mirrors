...
static void clipWbgain(rk_aiq_wb_gain_v32_t* awb_gain)
{
    float maxg1 = 0;
...
static void convertAiqAwbGainToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    pCvt->awb_gain_final.applyPosition =
        ...
        : IN_AWBGAIN0;
    // 2) //otp
    rk_aiq_wb_gain_v32_t awb_gain_otp = {1, 1, 1, 1, 1};
    AwbGainOtpAdjust(&awb_gain_otp, &pCvt->mCommonCvtInfo.otp_awb);
    ...
    if (pCvt->awb_gain_final.applyPosition == IN_AWBGAIN0) {
        //apply awb_gain_otp in wb0, apply awb_gain_final in wb0
        // or IS_SENSOR_BUILTIN apply awb_gain_otp in sensor, apply awb_gain_final in sensor
        rk_aiq_wb_gain_v32_t awb_gain2 = {1, 1, 1, 1, 1};
        awb_gain2.rgain  = pCvt->awb_gain_final.rgain * awb_gain_otp.rgain;
        awb_gain2.bgain  = pCvt->awb_gain_final.bgain * awb_gain_otp.bgain;
        awb_gain2.grgain = pCvt->awb_gain_final.grgain * awb_gain_otp.grgain;
        awb_gain2.gbgain = pCvt->awb_gain_final.gbgain * awb_gain_otp.gbgain;
        clipWbgain(&awb_gain2);
        uint16_t R  = (uint16_t)(0.5 + awb_gain2.rgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
        uint16_t B  = (uint16_t)(0.5 + awb_gain2.bgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
        uint16_t Gr = (uint16_t)(0.5 + awb_gain2.grgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
        uint16_t Gb = (uint16_t)(0.5 + awb_gain2.gbgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
        ...
        LOGD_AWB("%s wb0 gain  = [%f, %f, %f, %f] ,wb1 gain=[1,1,1,1]\n", __FUNCTION__, awb_gain2.rgain, awb_gain2.grgain,
                 awb_gain2.gbgain, awb_gain2.bgain);
    } else {
        if(! pCvt->mCommonCvtInfo._airms_en) {
            //apply awb_gain_otp in wb0, apply awb_gain_final in wb1
            uint16_t R  = (uint16_t)(0.5 + awb_gain_otp.rgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            uint16_t B  = (uint16_t)(0.5 + awb_gain_otp.bgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            uint16_t Gr = (uint16_t)(0.5 + awb_gain_otp.grgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            uint16_t Gb = (uint16_t)(0.5 + awb_gain_otp.gbgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            cfg->gain0_red     = R > max_wb_gain ? max_wb_gain : R;
            cfg->gain0_blue    = B > max_wb_gain ? max_wb_gain : B;
            cfg->gain0_green_r = Gr > max_wb_gain ? max_wb_gain : Gr;
            cfg->gain0_green_b = Gb > max_wb_gain ? max_wb_gain : Gb;
            ...
            cfg->gain2_green_r = Gr > max_wb_gain ? max_wb_gain : Gr;
            cfg->gain2_green_b = Gb > max_wb_gain ? max_wb_gain : Gb;
            clipWbgain(&pCvt->awb_gain_final);
            R  = (uint16_t)(0.5 + pCvt->awb_gain_final.rgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            B  = (uint16_t)(0.5 + pCvt->awb_gain_final.bgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            Gr = (uint16_t)(0.5 + pCvt->awb_gain_final.grgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            Gb = (uint16_t)(0.5 + pCvt->awb_gain_final.gbgain * (1 << ISP2X_WBGAIN_FIXSCALE_BIT));
            ...
            LOGD_AWB("%s wb0 gain  = [%f, %f, %f, %f] ,wb1 gain=[%f,%f,%f,%f]\n", __FUNCTION__, awb_gain_otp.rgain, awb_gain_otp.grgain,
                     awb_gain_otp.gbgain, awb_gain_otp.bgain, pCvt->awb_gain_final.rgain,
                     pCvt->awb_gain_final.grgain, pCvt->awb_gain_final.gbgain, pCvt->awb_gain_final.bgain);
        } else {
            //_airms_en=1+ linear
            ...
            //apply awb_gain_otp in sensor, apply awb_gain_final in sensor
            rk_aiq_wb_gain_v32_t awb_gain2 = {1, 1, 1, 1, 1};
            awb_gain2.rgain  = pCvt->awb_gain_final.rgain * awb_gain_otp.rgain;
            awb_gain2.bgain  = pCvt->awb_gain_final.bgain * awb_gain_otp.bgain;
            awb_gain2.grgain = pCvt->awb_gain_final.grgain * awb_gain_otp.grgain;
            ...
            cfg->awb1_gain_gb  = Gb > max_wb_gain ? max_wb_gain : Gb;
            (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            LOGD_AWB("%s wb0 gain  = [1,1,1,1] ,wb1 gain= [%f, %f, %f, %f]\n", __FUNCTION__, awb_gain2.rgain, awb_gain2.grgain,
                     awb_gain2.gbgain, awb_gain2.bgain);
        }
    }

    pCvt->mLatestWbGainCfg = *cfg; //mLatestWbGainCfg for sensor
...
static void WriteDataForIcCmodel(struct isp35_rawawb_meas_cfg* wpDetectPara)
{
    ...
    if(fp != NULL)
    {
        ...
        fprintf(fp, "g_awb_para.bnr_be_sel =%d;\n", wpDetectPara->bnr_be_sel);
...
static void WriteAwbReg(struct isp35_rawawb_meas_cfg* awb_cfg_v35) {
#if 0
    static int count = 0;
    if(count > 20) return;
    char fName[100];
    ...
    //fprintf(fp, "\t\tsw_rawawb_en = 0x%0x (%d)\n",                              awb_cfg_v35->en                            ,awb_cfg_v35->en);
    fprintf(fp, "\t\tsw_drc2awb_sel = 0x%0x (%d)\n",                                  awb_cfg_v35->drc2awb_sel, awb_cfg_v35->drc2awb_sel);
    fprintf(fp, "\t\tsw_bnr2awb_sel = 0x%0x (%d)\n",                                  awb_cfg_v35->bnr2awb_sel, awb_cfg_v35->bnr2awb_sel);
    fprintf(fp, "\t\tsw_bnr_be_sel = 0x%0x (%d)\n",                                  awb_cfg_v35->bnr_be_sel, awb_cfg_v35->bnr_be_sel);
    fprintf(fp, "\t\tsw_rawawb_sel = 0x%0x (%d)\n",                                   awb_cfg_v35->rawawb_sel, awb_cfg_v35->rawawb_sel);
    fprintf(fp, "\t\tsw_rawawb_uv_en0 = 0x%0x (%d)\n",                            awb_cfg_v35->uv_en0, awb_cfg_v35->uv_en0);
    fprintf(fp, "\t\tsw_rawawb_xy_en0 = 0x%0x (%d)\n",                            awb_cfg_v35->xy_en0, awb_cfg_v35->xy_en0);
    fprintf(fp, "\t\tsw_rawawb_in_rshift_to_12bit_en = 0x%0x (%d)\n",             awb_cfg_v35->in_rshift_to_12bit_en, awb_cfg_v35->in_rshift_to_12bit_en);
    fprintf(fp, "\t\tsw_rawawb_in_overexposure_check_en = 0x%0x (%d)\n",          awb_cfg_v35->in_overexposure_check_en, awb_cfg_v35->in_overexposure_check_en);
    fprintf(fp, "\t\tsw_rawawb_wind_size = 0x%0x (%d)\n",                         awb_cfg_v35->wind_size, awb_cfg_v35->wind_size);
    fprintf(fp, "\t\tsw_rawlsc_bypass_en = 0x%0x (%d)\n",                         awb_cfg_v35->rawlsc_bypass_en, awb_cfg_v35->rawlsc_bypass_en);
    fprintf(fp, "\t\tsw_rawawb_light_num = 0x%0x (%d)\n",                         awb_cfg_v35->light_num, awb_cfg_v35->light_num);
    //fprintf(fp, "\t\tsw_rawawb_2ddr_path_en = 0x%0x (%d)\n",                      awb_cfg_v35->2ddr_path_en                  ,awb_cfg_v35->2ddr_path_en);
    fprintf(fp, "\t\tsw_rawawb_uv_en1 = 0x%0x (%d)\n",                            awb_cfg_v35->uv_en1, awb_cfg_v35->uv_en1);
    fprintf(fp, "\t\tsw_rawawb_xy_en1 = 0x%0x (%d)\n",                            awb_cfg_v35->xy_en1, awb_cfg_v35->xy_en1);
    //fprintf(fp, "\t\tsw_rawawb_2ddr_path_sel = 0x%0x (%d)\n",                     awb_cfg_v35->2ddr_path_sel                 ,awb_cfg_v35->2ddr_path_sel);
    fprintf(fp, "\t\tsw_rawawbin_low12bit_val = 0x%0x (%d)\n",                    awb_cfg_v35->low12bit_val, awb_cfg_v35->low12bit_val);
    fprintf(fp, "\t\tsw_rawawb_blk_measure_en = 0x%0x (%d)\n",                    awb_cfg_v35->blk_measure_enable, awb_cfg_v35->blk_measure_enable);
    fprintf(fp, "\t\tsw_rawawb_blk_measure_mode = 0x%0x (%d)\n",                  awb_cfg_v35->blk_measure_mode, awb_cfg_v35->blk_measure_mode);
    fprintf(fp, "\t\tsw_rawawb_blk_measure_xytype = 0x%0x (%d)\n",                awb_cfg_v35->blk_measure_xytype, awb_cfg_v35->blk_measure_xytype);
    fprintf(fp, "\t\tsw_rawawb_blk_rtdw_measure_en = 0x%0x (%d)\n",               awb_cfg_v35->blk_rtdw_measure_en, awb_cfg_v35->blk_rtdw_measure_en);
    fprintf(fp, "\t\tsw_rawawb_blk_measure_illu_idx = 0x%0x (%d)\n",              awb_cfg_v35->blk_measure_illu_idx, awb_cfg_v35->blk_measure_illu_idx);
    fprintf(fp, "\t\tsw_rawawb_ds16x8_mode_en = 0x%0x (%d)\n",                    awb_cfg_v35->ds16x8_mode_en, awb_cfg_v35->ds16x8_mode_en);
    fprintf(fp, "\t\tsw_rawawb_blk_with_luma_wei_en = 0x%0x (%d)\n",              awb_cfg_v35->blk_with_luma_wei_en, awb_cfg_v35->blk_with_luma_wei_en);
    fprintf(fp, "\t\tsw_rawawb_in_overexposure_threshold = 0x%0x (%d)\n",         awb_cfg_v35->in_overexposure_threshold, awb_cfg_v35->in_overexposure_threshold);
    fprintf(fp, "\t\tsw_rawawb_h_offs = 0x%0x (%d)\n",                            awb_cfg_v35->h_offs, awb_cfg_v35->h_offs);
    fprintf(fp, "\t\tsw_rawawb_v_offs = 0x%0x (%d)\n",                            awb_cfg_v35->v_offs, awb_cfg_v35->v_offs);
    fprintf(fp, "\t\tsw_rawawb_h_size = 0x%0x (%d)\n",                            awb_cfg_v35->h_size, awb_cfg_v35->h_size);
    fprintf(fp, "\t\tsw_rawawb_v_size = 0x%0x (%d)\n",                            awb_cfg_v35->v_size, awb_cfg_v35->v_size);
    fprintf(fp, "\t\tsw_rawawb_r_max = 0x%0x (%d)\n",                             awb_cfg_v35->r_max, awb_cfg_v35->r_max);
    fprintf(fp, "\t\tsw_rawawb_g_max = 0x%0x (%d)\n",                             awb_cfg_v35->g_max, awb_cfg_v35->g_max);
    fprintf(fp, "\t\tsw_rawawb_b_max = 0x%0x (%d)\n",                             awb_cfg_v35->b_max, awb_cfg_v35->b_max);
    fprintf(fp, "\t\tsw_rawawb_y_max = 0x%0x (%d)\n",                             awb_cfg_v35->y_max, awb_cfg_v35->y_max);
    fprintf(fp, "\t\tsw_rawawb_r_min = 0x%0x (%d)\n",                             awb_cfg_v35->r_min, awb_cfg_v35->r_min);
    fprintf(fp, "\t\tsw_rawawb_g_min = 0x%0x (%d)\n",                             awb_cfg_v35->g_min, awb_cfg_v35->g_min);
    fprintf(fp, "\t\tsw_rawawb_b_min = 0x%0x (%d)\n",                             awb_cfg_v35->b_min, awb_cfg_v35->b_min);
    fprintf(fp, "\t\tsw_rawawb_y_min = 0x%0x (%d)\n",                             awb_cfg_v35->y_min, awb_cfg_v35->y_min);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_wei_en0 = 0x%0x (%d)\n",                   awb_cfg_v35->wp_luma_wei_en0, awb_cfg_v35->wp_luma_wei_en0);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_wei_en1 = 0x%0x (%d)\n",                   awb_cfg_v35->wp_luma_wei_en1, awb_cfg_v35->wp_luma_wei_en1);
    fprintf(fp, "\t\tsw_rawawb_wp_blk_wei_en0 = 0x%0x (%d)\n",                    awb_cfg_v35->wp_blk_wei_en0, awb_cfg_v35->wp_blk_wei_en0);
    fprintf(fp, "\t\tsw_rawawb_wp_blk_wei_en1 = 0x%0x (%d)\n",                    awb_cfg_v35->wp_blk_wei_en1, awb_cfg_v35->wp_blk_wei_en1);
    fprintf(fp, "\t\tsw_rawawb_wp_hist_xytype = 0x%0x (%d)\n",                    awb_cfg_v35->wp_hist_xytype, awb_cfg_v35->wp_hist_xytype);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y0 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y0, awb_cfg_v35->wp_luma_weicurve_y0);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y1 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y1, awb_cfg_v35->wp_luma_weicurve_y1);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y2 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y2, awb_cfg_v35->wp_luma_weicurve_y2);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y3 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y3, awb_cfg_v35->wp_luma_weicurve_y3);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y4 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y4, awb_cfg_v35->wp_luma_weicurve_y4);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y5 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y5, awb_cfg_v35->wp_luma_weicurve_y5);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y6 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y6, awb_cfg_v35->wp_luma_weicurve_y6);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y7 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y7, awb_cfg_v35->wp_luma_weicurve_y7);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_y8 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_y8, awb_cfg_v35->wp_luma_weicurve_y8);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w0 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w0, awb_cfg_v35->wp_luma_weicurve_w0);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w1 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w1, awb_cfg_v35->wp_luma_weicurve_w1);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w2 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w2, awb_cfg_v35->wp_luma_weicurve_w2);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w3 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w3, awb_cfg_v35->wp_luma_weicurve_w3);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w4 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w4, awb_cfg_v35->wp_luma_weicurve_w4);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w5 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w5, awb_cfg_v35->wp_luma_weicurve_w5);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w6 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w6, awb_cfg_v35->wp_luma_weicurve_w6);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w7 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w7, awb_cfg_v35->wp_luma_weicurve_w7);
    fprintf(fp, "\t\tsw_rawawb_wp_luma_weicurve_w8 = 0x%0x (%d)\n",               awb_cfg_v35->wp_luma_weicurve_w8, awb_cfg_v35->wp_luma_weicurve_w8);
    fprintf(fp, "\t\tsw_ccm_coeff0_r = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff0_r, awb_cfg_v35->ccm_coeff0_r);
    fprintf(fp, "\t\tsw_ccm_coeff1_r = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff1_r, awb_cfg_v35->ccm_coeff1_r);
    fprintf(fp, "\t\tsw_ccm_coeff2_r = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff2_r, awb_cfg_v35->ccm_coeff2_r);
    fprintf(fp, "\t\tsw_ccm_coeff0_g = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff0_g, awb_cfg_v35->ccm_coeff0_g);
    fprintf(fp, "\t\tsw_ccm_coeff1_g = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff1_g, awb_cfg_v35->ccm_coeff1_g);
    fprintf(fp, "\t\tsw_ccm_coeff2_g = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff2_g, awb_cfg_v35->ccm_coeff2_g);
    fprintf(fp, "\t\tsw_ccm_coeff0_b = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff0_b, awb_cfg_v35->ccm_coeff0_b);
    fprintf(fp, "\t\tsw_ccm_coeff1_b = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff1_b, awb_cfg_v35->ccm_coeff1_b);
    fprintf(fp, "\t\tsw_ccm_coeff2_b = 0x%0x (%d)\n",                             awb_cfg_v35->ccm_coeff2_b, awb_cfg_v35->ccm_coeff2_b);

    fprintf(fp, "\t\tsw_rawawb_pre_wbgain_inv_r = 0x%0x (%d)\n",                  awb_cfg_v35->pre_wbgain_inv_r, awb_cfg_v35->pre_wbgain_inv_r);
    fprintf(fp, "\t\tsw_rawawb_pre_wbgain_inv_g = 0x%0x (%d)\n",                  awb_cfg_v35->pre_wbgain_inv_g, awb_cfg_v35->pre_wbgain_inv_g);
    fprintf(fp, "\t\tsw_rawawb_pre_wbgain_inv_b = 0x%0x (%d)\n",                  awb_cfg_v35->pre_wbgain_inv_b, awb_cfg_v35->pre_wbgain_inv_b);
    fprintf(fp, "\t\tsw_rawawb_vertex0_u_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_u_0, awb_cfg_v35->vertex0_u_0);
    fprintf(fp, "\t\tsw_rawawb_vertex0_v_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_v_0, awb_cfg_v35->vertex0_v_0);
    fprintf(fp, "\t\tsw_rawawb_vertex1_u_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_u_0, awb_cfg_v35->vertex1_u_0);
    fprintf(fp, "\t\tsw_rawawb_vertex1_v_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_v_0, awb_cfg_v35->vertex1_v_0);
    fprintf(fp, "\t\tsw_rawawb_vertex2_u_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_u_0, awb_cfg_v35->vertex2_u_0);
    fprintf(fp, "\t\tsw_rawawb_vertex2_v_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_v_0, awb_cfg_v35->vertex2_v_0);
    fprintf(fp, "\t\tsw_rawawb_vertex3_u_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_u_0, awb_cfg_v35->vertex3_u_0);
    fprintf(fp, "\t\tsw_rawawb_vertex3_v_0 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_v_0, awb_cfg_v35->vertex3_v_0);
    fprintf(fp, "\t\tsw_rawawb_islope01_0 = 0x%0x (%d)\n",                        awb_cfg_v35->islope01_0, awb_cfg_v35->islope01_0);
    fprintf(fp, "\t\tsw_rawawb_islope12_0 = 0x%0x (%d)\n",                        awb_cfg_v35->islope12_0, awb_cfg_v35->islope12_0);
    fprintf(fp, "\t\tsw_rawawb_islope23_0 = 0x%0x (%d)\n",                        awb_cfg_v35->islope23_0, awb_cfg_v35->islope23_0);
    fprintf(fp, "\t\tsw_rawawb_islope30_0 = 0x%0x (%d)\n",                        awb_cfg_v35->islope30_0, awb_cfg_v35->islope30_0);
    fprintf(fp, "\t\tsw_rawawb_vertex0_u_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_u_1, awb_cfg_v35->vertex0_u_1);
    fprintf(fp, "\t\tsw_rawawb_vertex0_v_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_v_1, awb_cfg_v35->vertex0_v_1);
    fprintf(fp, "\t\tsw_rawawb_vertex1_u_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_u_1, awb_cfg_v35->vertex1_u_1);
    fprintf(fp, "\t\tsw_rawawb_vertex1_v_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_v_1, awb_cfg_v35->vertex1_v_1);
    fprintf(fp, "\t\tsw_rawawb_vertex2_u_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_u_1, awb_cfg_v35->vertex2_u_1);
    fprintf(fp, "\t\tsw_rawawb_vertex2_v_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_v_1, awb_cfg_v35->vertex2_v_1);
    fprintf(fp, "\t\tsw_rawawb_vertex3_u_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_u_1, awb_cfg_v35->vertex3_u_1);
    fprintf(fp, "\t\tsw_rawawb_vertex3_v_1 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_v_1, awb_cfg_v35->vertex3_v_1);
    fprintf(fp, "\t\tsw_rawawb_islope01_1 = 0x%0x (%d)\n",                        awb_cfg_v35->islope01_1, awb_cfg_v35->islope01_1);
    fprintf(fp, "\t\tsw_rawawb_islope12_1 = 0x%0x (%d)\n",                        awb_cfg_v35->islope12_1, awb_cfg_v35->islope12_1);
    fprintf(fp, "\t\tsw_rawawb_islope23_1 = 0x%0x (%d)\n",                        awb_cfg_v35->islope23_1, awb_cfg_v35->islope23_1);
    fprintf(fp, "\t\tsw_rawawb_islope30_1 = 0x%0x (%d)\n",                        awb_cfg_v35->islope30_1, awb_cfg_v35->islope30_1);
    fprintf(fp, "\t\tsw_rawawb_vertex0_u_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_u_2, awb_cfg_v35->vertex0_u_2);
    fprintf(fp, "\t\tsw_rawawb_vertex0_v_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_v_2, awb_cfg_v35->vertex0_v_2);
    fprintf(fp, "\t\tsw_rawawb_vertex1_u_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_u_2, awb_cfg_v35->vertex1_u_2);
    fprintf(fp, "\t\tsw_rawawb_vertex1_v_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_v_2, awb_cfg_v35->vertex1_v_2);
    fprintf(fp, "\t\tsw_rawawb_vertex2_u_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_u_2, awb_cfg_v35->vertex2_u_2);
    fprintf(fp, "\t\tsw_rawawb_vertex2_v_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_v_2, awb_cfg_v35->vertex2_v_2);
    fprintf(fp, "\t\tsw_rawawb_vertex3_u_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_u_2, awb_cfg_v35->vertex3_u_2);
    fprintf(fp, "\t\tsw_rawawb_vertex3_v_2 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_v_2, awb_cfg_v35->vertex3_v_2);
    fprintf(fp, "\t\tsw_rawawb_islope01_2 = 0x%0x (%d)\n",                        awb_cfg_v35->islope01_2, awb_cfg_v35->islope01_2);
    fprintf(fp, "\t\tsw_rawawb_islope12_2 = 0x%0x (%d)\n",                        awb_cfg_v35->islope12_2, awb_cfg_v35->islope12_2);
    fprintf(fp, "\t\tsw_rawawb_islope23_2 = 0x%0x (%d)\n",                        awb_cfg_v35->islope23_2, awb_cfg_v35->islope23_2);
    fprintf(fp, "\t\tsw_rawawb_islope30_2 = 0x%0x (%d)\n",                        awb_cfg_v35->islope30_2, awb_cfg_v35->islope30_2);
    fprintf(fp, "\t\tsw_rawawb_vertex0_u_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_u_3, awb_cfg_v35->vertex0_u_3);
    fprintf(fp, "\t\tsw_rawawb_vertex0_v_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex0_v_3, awb_cfg_v35->vertex0_v_3);
    fprintf(fp, "\t\tsw_rawawb_vertex1_u_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_u_3, awb_cfg_v35->vertex1_u_3);
    fprintf(fp, "\t\tsw_rawawb_vertex1_v_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex1_v_3, awb_cfg_v35->vertex1_v_3);
    fprintf(fp, "\t\tsw_rawawb_vertex2_u_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_u_3, awb_cfg_v35->vertex2_u_3);
    fprintf(fp, "\t\tsw_rawawb_vertex2_v_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex2_v_3, awb_cfg_v35->vertex2_v_3);
    fprintf(fp, "\t\tsw_rawawb_vertex3_u_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_u_3, awb_cfg_v35->vertex3_u_3);
    fprintf(fp, "\t\tsw_rawawb_vertex3_v_3 = 0x%0x (%d)\n",                       awb_cfg_v35->vertex3_v_3, awb_cfg_v35->vertex3_v_3);
    fprintf(fp, "\t\tsw_rawawb_islope01_3 = 0x%0x (%d)\n",                        awb_cfg_v35->islope01_3, awb_cfg_v35->islope01_3);
    fprintf(fp, "\t\tsw_rawawb_islope12_3 = 0x%0x (%d)\n",                        awb_cfg_v35->islope12_3, awb_cfg_v35->islope12_3);
    fprintf(fp, "\t\tsw_rawawb_islope23_3 = 0x%0x (%d)\n",                        awb_cfg_v35->islope23_3, awb_cfg_v35->islope23_3);
    fprintf(fp, "\t\tsw_rawawb_islope30_3 = 0x%0x (%d)\n",                        awb_cfg_v35->islope30_3, awb_cfg_v35->islope30_3);
    fprintf(fp, "\t\tsw_rawawb_wt0 = 0x%0x (%d)\n",                               awb_cfg_v35->wt0, awb_cfg_v35->wt0);
    fprintf(fp, "\t\tsw_rawawb_wt1 = 0x%0x (%d)\n",                               awb_cfg_v35->wt1, awb_cfg_v35->wt1);
    fprintf(fp, "\t\tsw_rawawb_wt2 = 0x%0x (%d)\n",                               awb_cfg_v35->wt2, awb_cfg_v35->wt2);
    fprintf(fp, "\t\tsw_rawawb_mat0_x = 0x%0x (%d)\n",                            awb_cfg_v35->mat0_x, awb_cfg_v35->mat0_x);
    fprintf(fp, "\t\tsw_rawawb_mat0_y = 0x%0x (%d)\n",                            awb_cfg_v35->mat0_y, awb_cfg_v35->mat0_y);
    fprintf(fp, "\t\tsw_rawawb_mat1_x = 0x%0x (%d)\n",                            awb_cfg_v35->mat1_x, awb_cfg_v35->mat1_x);
    fprintf(fp, "\t\tsw_rawawb_mat1_y = 0x%0x (%d)\n",                            awb_cfg_v35->mat1_y, awb_cfg_v35->mat1_y);
    fprintf(fp, "\t\tsw_rawawb_mat2_x = 0x%0x (%d)\n",                            awb_cfg_v35->mat2_x, awb_cfg_v35->mat2_x);
    fprintf(fp, "\t\tsw_rawawb_mat2_y = 0x%0x (%d)\n",                            awb_cfg_v35->mat2_y, awb_cfg_v35->mat2_y);
    fprintf(fp, "\t\tsw_rawawb_nor_x0_0 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x0_0, awb_cfg_v35->nor_x0_0);
    fprintf(fp, "\t\tsw_rawawb_nor_x1_0 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x1_0, awb_cfg_v35->nor_x1_0);
    fprintf(fp, "\t\tsw_rawawb_nor_y0_0 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y0_0, awb_cfg_v35->nor_y0_0);
    fprintf(fp, "\t\tsw_rawawb_nor_y1_0 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y1_0, awb_cfg_v35->nor_y1_0);
    fprintf(fp, "\t\tsw_rawawb_big_x0_0 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x0_0, awb_cfg_v35->big_x0_0);
    fprintf(fp, "\t\tsw_rawawb_big_x1_0 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x1_0, awb_cfg_v35->big_x1_0);
    fprintf(fp, "\t\tsw_rawawb_big_y0_0 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y0_0, awb_cfg_v35->big_y0_0);
    fprintf(fp, "\t\tsw_rawawb_big_y1_0 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y1_0, awb_cfg_v35->big_y1_0);
    fprintf(fp, "\t\tsw_rawawb_nor_x0_1 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x0_1, awb_cfg_v35->nor_x0_1);
    fprintf(fp, "\t\tsw_rawawb_nor_x1_1 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x1_1, awb_cfg_v35->nor_x1_1);
    fprintf(fp, "\t\tsw_rawawb_nor_y0_1 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y0_1, awb_cfg_v35->nor_y0_1);
    fprintf(fp, "\t\tsw_rawawb_nor_y1_1 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y1_1, awb_cfg_v35->nor_y1_1);
    fprintf(fp, "\t\tsw_rawawb_big_x0_1 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x0_1, awb_cfg_v35->big_x0_1);
    fprintf(fp, "\t\tsw_rawawb_big_x1_1 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x1_1, awb_cfg_v35->big_x1_1);
    fprintf(fp, "\t\tsw_rawawb_big_y0_1 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y0_1, awb_cfg_v35->big_y0_1);
    fprintf(fp, "\t\tsw_rawawb_big_y1_1 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y1_1, awb_cfg_v35->big_y1_1);
    fprintf(fp, "\t\tsw_rawawb_nor_x0_2 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x0_2, awb_cfg_v35->nor_x0_2);
    fprintf(fp, "\t\tsw_rawawb_nor_x1_2 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x1_2, awb_cfg_v35->nor_x1_2);
    fprintf(fp, "\t\tsw_rawawb_nor_y0_2 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y0_2, awb_cfg_v35->nor_y0_2);
    fprintf(fp, "\t\tsw_rawawb_nor_y1_2 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y1_2, awb_cfg_v35->nor_y1_2);
    fprintf(fp, "\t\tsw_rawawb_big_x0_2 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x0_2, awb_cfg_v35->big_x0_2);
    fprintf(fp, "\t\tsw_rawawb_big_x1_2 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x1_2, awb_cfg_v35->big_x1_2);
    fprintf(fp, "\t\tsw_rawawb_big_y0_2 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y0_2, awb_cfg_v35->big_y0_2);
    fprintf(fp, "\t\tsw_rawawb_big_y1_2 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y1_2, awb_cfg_v35->big_y1_2);
    fprintf(fp, "\t\tsw_rawawb_nor_x0_3 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x0_3, awb_cfg_v35->nor_x0_3);
    fprintf(fp, "\t\tsw_rawawb_nor_x1_3 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_x1_3, awb_cfg_v35->nor_x1_3);
    fprintf(fp, "\t\tsw_rawawb_nor_y0_3 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y0_3, awb_cfg_v35->nor_y0_3);
    fprintf(fp, "\t\tsw_rawawb_nor_y1_3 = 0x%0x (%d)\n",                          awb_cfg_v35->nor_y1_3, awb_cfg_v35->nor_y1_3);
    fprintf(fp, "\t\tsw_rawawb_big_x0_3 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x0_3, awb_cfg_v35->big_x0_3);
    fprintf(fp, "\t\tsw_rawawb_big_x1_3 = 0x%0x (%d)\n",                          awb_cfg_v35->big_x1_3, awb_cfg_v35->big_x1_3);
    fprintf(fp, "\t\tsw_rawawb_big_y0_3 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y0_3, awb_cfg_v35->big_y0_3);
    fprintf(fp, "\t\tsw_rawawb_big_y1_3 = 0x%0x (%d)\n",                          awb_cfg_v35->big_y1_3, awb_cfg_v35->big_y1_3);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region0_excen, awb_cfg_v35->exc_wp_region0_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_measen = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region0_measen, awb_cfg_v35->exc_wp_region0_measen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region0_domain, awb_cfg_v35->exc_wp_region0_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region0_weight, awb_cfg_v35->exc_wp_region0_weight);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region1_excen, awb_cfg_v35->exc_wp_region1_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_measen = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region1_measen, awb_cfg_v35->exc_wp_region1_measen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region1_domain, awb_cfg_v35->exc_wp_region1_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region1_weight, awb_cfg_v35->exc_wp_region1_weight);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region2_excen, awb_cfg_v35->exc_wp_region2_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_measen = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region2_measen, awb_cfg_v35->exc_wp_region2_measen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region2_domain, awb_cfg_v35->exc_wp_region2_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region3_weight, awb_cfg_v35->exc_wp_region2_weight);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region3_excen, awb_cfg_v35->exc_wp_region3_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_measen = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region3_measen, awb_cfg_v35->exc_wp_region3_measen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region3_domain, awb_cfg_v35->exc_wp_region3_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region3_weight, awb_cfg_v35->exc_wp_region3_weight);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region4_excen, awb_cfg_v35->exc_wp_region4_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region4_domain, awb_cfg_v35->exc_wp_region4_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region4_weight, awb_cfg_v35->exc_wp_region4_weight);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region5_excen, awb_cfg_v35->exc_wp_region5_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region5_domain, awb_cfg_v35->exc_wp_region5_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region5_weight, awb_cfg_v35->exc_wp_region5_weight);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_excen = 0x%0x (%d)\n",              awb_cfg_v35->exc_wp_region6_excen, awb_cfg_v35->exc_wp_region6_excen);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_domain = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region6_domain, awb_cfg_v35->exc_wp_region6_domain);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_weight = 0x%0x (%d)\n",             awb_cfg_v35->exc_wp_region6_weight, awb_cfg_v35->exc_wp_region6_weight);
    fprintf(fp, "\t\tsw_rawawb_multiwindow_en = 0x%0x (%d)\n",                    awb_cfg_v35->multiwindow_en, awb_cfg_v35->multiwindow_en);
    fprintf(fp, "\t\tsw_rawawb_multiwindow0_h_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow0_h_offs, awb_cfg_v35->multiwindow0_h_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow0_v_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow0_v_offs, awb_cfg_v35->multiwindow0_v_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow0_h_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow0_h_size, awb_cfg_v35->multiwindow0_h_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow1_v_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow1_v_size, awb_cfg_v35->multiwindow1_v_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow1_h_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow1_h_offs, awb_cfg_v35->multiwindow1_h_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow1_v_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow1_v_offs, awb_cfg_v35->multiwindow1_v_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow1_h_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow1_h_size, awb_cfg_v35->multiwindow1_h_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow1_v_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow1_v_size, awb_cfg_v35->multiwindow1_v_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow2_h_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow2_h_offs, awb_cfg_v35->multiwindow2_h_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow2_v_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow2_v_offs, awb_cfg_v35->multiwindow2_v_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow2_h_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow2_h_size, awb_cfg_v35->multiwindow2_h_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow2_v_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow2_v_size, awb_cfg_v35->multiwindow2_v_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow3_h_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow3_h_offs, awb_cfg_v35->multiwindow3_h_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow3_v_offs = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow3_v_offs, awb_cfg_v35->multiwindow3_v_offs);
    fprintf(fp, "\t\tsw_rawawb_multiwindow3_h_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow3_h_size, awb_cfg_v35->multiwindow3_h_size);
    fprintf(fp, "\t\tsw_rawawb_multiwindow3_v_size = 0x%0x (%d)\n",               awb_cfg_v35->multiwindow3_v_size, awb_cfg_v35->multiwindow3_v_size);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region0_xu0, awb_cfg_v35->exc_wp_region0_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region0_xu1, awb_cfg_v35->exc_wp_region0_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region0_yv0, awb_cfg_v35->exc_wp_region0_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region0_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region0_yv1, awb_cfg_v35->exc_wp_region0_yv1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region1_xu0, awb_cfg_v35->exc_wp_region1_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region1_xu1, awb_cfg_v35->exc_wp_region1_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region1_yv0, awb_cfg_v35->exc_wp_region1_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region1_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region1_yv1, awb_cfg_v35->exc_wp_region1_yv1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region2_xu0, awb_cfg_v35->exc_wp_region2_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region2_xu1, awb_cfg_v35->exc_wp_region2_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region2_yv0, awb_cfg_v35->exc_wp_region2_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region2_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region2_yv1, awb_cfg_v35->exc_wp_region2_yv1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region3_xu0, awb_cfg_v35->exc_wp_region3_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region3_xu1, awb_cfg_v35->exc_wp_region3_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region3_yv0, awb_cfg_v35->exc_wp_region3_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region3_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region3_yv1, awb_cfg_v35->exc_wp_region3_yv1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region4_xu0, awb_cfg_v35->exc_wp_region4_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region4_xu1, awb_cfg_v35->exc_wp_region4_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region4_yv0, awb_cfg_v35->exc_wp_region4_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region4_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region4_yv1, awb_cfg_v35->exc_wp_region4_yv1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region5_xu0, awb_cfg_v35->exc_wp_region5_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region5_xu1, awb_cfg_v35->exc_wp_region5_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region5_yv0, awb_cfg_v35->exc_wp_region5_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region5_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region5_yv1, awb_cfg_v35->exc_wp_region5_yv1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_xu0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region6_xu0, awb_cfg_v35->exc_wp_region6_xu0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_xu1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region6_xu1, awb_cfg_v35->exc_wp_region6_xu1);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_yv0 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region6_yv0, awb_cfg_v35->exc_wp_region6_yv0);
    fprintf(fp, "\t\tsw_rawawb_exc_wp_region6_yv1 = 0x%0x (%d)\n",                awb_cfg_v35->exc_wp_region6_yv1, awb_cfg_v35->exc_wp_region6_yv1);
    fprintf(fp, "\t\tbls2 en(%d),val_rggb(%d,%d,%d,%d))\n",             awb_cfg_v35->bls2_en, awb_cfg_v35->bls2_val.r,
            awb_cfg_v35->bls2_val.gr, awb_cfg_v35->bls2_val.gb, awb_cfg_v35->bls2_val.b);

    fprintf(fp, "awb_cfg_v35->wp_blk_wei_w:");
    for(int i = 0; i < 225; i++) {
        if (i % 15 == 0) {
            fprintf(fp, "\n");
        }
        fprintf(fp, "0x%03x (%3d),", awb_cfg_v35->wp_blk_wei_w[i], awb_cfg_v35->wp_blk_wei_w[i]);
...
static void calcInputBitIs12Bit(uint8_t* inputBitIs12Bit, const awbStats_cfg_t* awb_meas,
    ...
    bool selectBayerNrData = (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode || awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrfeOut_mode);
...
static void ConfigBlc2(const blc_res_cvt_t* ablc, const awbStats_cfg_t* awb_meas, float isp_dgain,
                       ...
                       struct isp35_rawawb_meas_cfg* awb_cfg_v35) {
    awb_cfg_v35->bls2_en     = false;
    awb_cfg_v35->bls2_val.r  = 0;
    awb_cfg_v35->bls2_val.gr = 0;
    awb_cfg_v35->bls2_val.gb = 0;
    awb_cfg_v35->bls2_val.b  = 0;
    if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode ||
            ((awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode || awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrfeOut_mode) &&
    ...
    if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode || awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrfeOut_mode) {
        // update by (offset +blc1)*dgain
        // working_mode = normal,so   applyPosition = IN_AWBGAIN1
        awb_cfg_v35->bls2_val.r =
            (awbBls2.hw_awbCfg_obR_val + blc1[AWB_CHANNEL_R]) * isp_dgain * dgain2 + 0.5;
        awb_cfg_v35->bls2_val.b =
            (awbBls2.hw_awbCfg_obB_val + blc1[AWB_CHANNEL_B]) * isp_dgain * dgain2 + 0.5;
        awb_cfg_v35->bls2_val.gr =
            (awbBls2.hw_awbCfg_obGr_val + blc1[AWB_CHANNEL_GR]) * isp_dgain * dgain2 + 0.5;
        awb_cfg_v35->bls2_val.gb =
            (awbBls2.hw_awbCfg_obGb_val + blc1[AWB_CHANNEL_GB]) * isp_dgain * dgain2 + 0.5;
    } else {  // select raw
        // update by offset +blc1
        awb_cfg_v35->bls2_val.r  = (awbBls2.hw_awbCfg_obR_val + blc1[AWB_CHANNEL_R]) + 0.5;
        awb_cfg_v35->bls2_val.b  = (awbBls2.hw_awbCfg_obB_val + blc1[AWB_CHANNEL_B]) + 0.5;
        awb_cfg_v35->bls2_val.gr = (awbBls2.hw_awbCfg_obGr_val + blc1[AWB_CHANNEL_GR]) + 0.5;
        awb_cfg_v35->bls2_val.gb = (awbBls2.hw_awbCfg_obGb_val + blc1[AWB_CHANNEL_GB]) + 0.5;
    }
}
...
static XCamReturn ConfigOverexposureValue(const blc_res_cvt_t* ablc, float hdrmge_gain0_1,
        ...
        struct isp35_rawawb_meas_cfg* awb_cfg_v35) {
    XCamReturn ret                        = XCAM_RETURN_NO_ERROR;
    awb_cfg_v35->in_overexposure_check_en = true;
    float dgain                           = 1.0;
    int max_blc                           = 0;
    short ob                              = 0;
    ...
    if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode) {
        awb_cfg_v35->in_overexposure_threshold = overexposure_value;
    } else if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode || awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrfeOut_mode) {
        ...
        if (working_mode != RK_AIQ_WORKING_MODE_NORMAL) {
            ...
            awb_cfg_v35->in_overexposure_threshold = overexposure_value * hdrmge_gain0_1 * dgain;
        } else {
            // need check
            ...
            blc[AWB_CHANNEL_B] += awb_cfg_v35->bls2_val.b + ob;
            blc[AWB_CHANNEL_R] += awb_cfg_v35->bls2_val.r + ob;
            blc[AWB_CHANNEL_GB] += awb_cfg_v35->bls2_val.gb + ob;
            blc[AWB_CHANNEL_GR] += awb_cfg_v35->bls2_val.gr + ob;
            for (int i = 0; i < AWB_CHANNEL_MAX; i++) {
                if (blc[i] > max_blc) {
                    max_blc = blc[i];
                }
            ...
            awb_cfg_v35->in_overexposure_threshold = overexposure_value * dgain - max_blc / 16;
        }
        ...
        awb_cfg_v35->in_overexposure_threshold = overexposure_value - max_blc / 16;
    }
    return (ret);
}
...
static void ConfigPreWbgain3(AiqIspParamsCvt_t* pCvt, struct isp35_rawawb_meas_cfg* awb_cfg_v35,
    ...
    if ((awb_gain->rgain * awb_gain->grgain * awb_gain->bgain > 0.0001) &&
        ...
        awb_cfg_v35->pre_wbgain_inv_g =
            (1 << RK_AIQ_AWB_PRE_WBGAIN_FRAC_BIT) / (awb_gain->grgain + awb_gain->gbgain) * 2 * preWbgainSw[1] + 0.5;
        awb_cfg_v35->pre_wbgain_inv_b =
...
static void convertAiqAwbToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    struct isp35_rawawb_meas_cfg* awb_cfg_v35 = &pCvt->isp_params.isp_cfg->meas.rawawb;
    awb_cfg_v35->ovexp_2ddr_dis = 0;
    ...
    float isp_ob_predgain = pCvt->mCommonCvtInfo.preDGain_drc;
    ConfigBlc2(blc, awb_meas, isp_dgain, isp_ob_predgain, pCvt->_working_mode, awb_cfg_v35);
    LOGD_AWB("blc2_cfg %d %d,%d,%d,%d", awb_cfg_v35->bls2_en, awb_cfg_v35->bls2_val.r,
             awb_cfg_v35->bls2_val.gr, awb_cfg_v35->bls2_val.gb, awb_cfg_v35->bls2_val.b);

    awb_cfg_v35->rawawb_sel = 0;
    awb_cfg_v35->bnr_be_sel = 0;
    if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode) {
        awb_cfg_v35->drc2awb_sel = 1;
    } else {
        awb_cfg_v35->drc2awb_sel = 0;
        if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode ||
                awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrfeOut_mode) {
            awb_cfg_v35->bnr2awb_sel = 1;
            if(awb_meas->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode) {
                awb_cfg_v35->bnr_be_sel = 1;
            }
        } else {
            awb_cfg_v35->bnr2awb_sel = 0;
            awb_cfg_v35->rawawb_sel  = awb_meas->hw_awbCfg_statsSrc_mode;
            if (pCvt->_working_mode == RK_AIQ_WORKING_MODE_NORMAL && awb_cfg_v35->rawawb_sel > 0) {
                awb_cfg_v35->rawawb_sel = 0;
            }
        }
    }
    bool ablc_en = blc && blc->en;
    calcInputBitIs12Bit(&awb_cfg_v35->low12bit_val, awb_meas, pCvt->_working_mode, ablc_en,
                        isp_ob_predgain);
    awb_cfg_v35->in_rshift_to_12bit_en = !awb_cfg_v35->low12bit_val;
    // awb_cfg_v35->ddr_path_en = awb_meas->write2ddrEnable;
    // awb_cfg_v35->ddr_path_sel = awb_meas->write2ddrSelc;
    ConfigOverexposureValue(blc, hdrmge_gain0_1, isp_ob_predgain, pCvt->_working_mode, awb_meas,
                            awb_cfg_v35);
    awb_cfg_v35->xy_en0           = awb_meas->wpEngine.norWpStatsCfg.hw_awbCfg_xyDct_en;
    awb_cfg_v35->uv_en0           = awb_meas->wpEngine.norWpStatsCfg.hw_awbCfg_uvDct_en;
    awb_cfg_v35->xy_en1           = awb_meas->wpEngine.bigWpStatsCfg.hw_awbCfg_xyDct_en;
    awb_cfg_v35->uv_en1           = awb_meas->wpEngine.bigWpStatsCfg.hw_awbCfg_uvDct_en;
    awb_cfg_v35->wp_blk_wei_en0   = awb_meas->wpEngine.norWpStatsCfg.hw_awbCfg_zoneWgt_en;
    awb_cfg_v35->wp_blk_wei_en1   = awb_meas->wpEngine.bigWpStatsCfg.hw_awbCfg_zoneWgt_en;
    awb_cfg_v35->rawlsc_bypass_en = awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode
                                    ? true
                                    : (!awb_meas->hw_awbCfg_lsc_en);
    awb_cfg_v35->blk_measure_enable = awb_meas->pixEngine.hw_awbCfg_stats_en;
    awb_cfg_v35->blk_measure_mode =
        awb_meas->pixEngine.hw_awbCfg_zoneStatsSrc_mode > awbStats_pixAll_mode;
    awb_cfg_v35->blk_measure_xytype =
        awb_meas->pixEngine.hw_awbCfg_zoneStatsSrc_mode >= awbStats_bigWpLs0_mode;
    awb_cfg_v35->blk_measure_illu_idx = awb_meas->pixEngine.hw_awbCfg_zoneStatsSrc_mode % 0x10;
    awb_cfg_v35->blk_with_luma_wei_en = awb_meas->pixEngine.hw_awbCfg_lumaWgt_en;
    awb_cfg_v35->wp_luma_wei_en0      = awb_meas->wpEngine.norWpStatsCfg.hw_awbCfg_lumaWgt_en;
    awb_cfg_v35->wp_luma_wei_en1      = awb_meas->wpEngine.bigWpStatsCfg.hw_awbCfg_lumaWgt_en;
    awb_cfg_v35->wp_hist_xytype       = awb_meas->wpEngine.hw_awbCfg_wpHistSrc_mode;
    awb_cfg_v35->light_num            = awb_meas->wpEngine.hw_awbCfg_lightSrcNum_val;
    awb_cfg_v35->h_offs               = awb_meas->mainWin.hw_awbCfg_win_x;
    awb_cfg_v35->v_offs               = awb_meas->mainWin.hw_awbCfg_win_y;
    awb_cfg_v35->h_size               = awb_meas->mainWin.hw_awbCfg_win_width;
    awb_cfg_v35->v_size               = awb_meas->mainWin.hw_awbCfg_win_height;

    if (awb_meas->hw_awbCfg_ds_mode == awbStats_ds_4x4 ||
            awb_meas->hw_awbCfg_ds_mode == awbStats_ds_8x8) {
        awb_cfg_v35->wind_size      = awb_meas->hw_awbCfg_ds_mode;
        awb_cfg_v35->ds16x8_mode_en = 0;
    } else if (awb_meas->hw_awbCfg_ds_mode == awbStats_ds_16x8) {
        awb_cfg_v35->ds16x8_mode_en = 1;
        awb_cfg_v35->wind_size      = awbStats_ds_8x8;
    }
    awb_cfg_v35->r_max = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMaxR_thred + 0.5;
    awb_cfg_v35->g_max = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMaxG_thred + 0.5;
    awb_cfg_v35->b_max = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMaxB_thred + 0.5;
    awb_cfg_v35->y_max = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMaxY_thred + 0.5;
    awb_cfg_v35->r_min = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMinR_thred + 0.5;
    awb_cfg_v35->g_min = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMinG_thred + 0.5;
    awb_cfg_v35->b_min = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMinB_thred + 0.5;
    awb_cfg_v35->y_min = 16 * awb_meas->wpEngine.wpDct_rgbySpace.hw_awbT_wpMinY_thred + 0.5;

    unsigned short pu_region[5];
    unsigned short pv_region[5];
    int slope_inv[4];
    UvParaFixed32(&awb_meas->wpEngine.wpDct_uvSpace.wpRegion[0], pu_region, pv_region, slope_inv);
    awb_cfg_v35->vertex0_u_0 = pu_region[0];
    awb_cfg_v35->vertex0_v_0 = pv_region[0];
    awb_cfg_v35->vertex1_u_0 = pu_region[1];
    awb_cfg_v35->vertex1_v_0 = pv_region[1];
    awb_cfg_v35->vertex2_u_0 = pu_region[2];
    awb_cfg_v35->vertex2_v_0 = pv_region[2];
    awb_cfg_v35->vertex3_u_0 = pu_region[3];
    awb_cfg_v35->vertex3_v_0 = pv_region[3];
    awb_cfg_v35->islope01_0  = slope_inv[0];
    awb_cfg_v35->islope12_0  = slope_inv[1];
    awb_cfg_v35->islope23_0  = slope_inv[2];
    awb_cfg_v35->islope30_0  = slope_inv[3];
    UvParaFixed32(&awb_meas->wpEngine.wpDct_uvSpace.wpRegion[1], pu_region, pv_region, slope_inv);
    awb_cfg_v35->vertex0_u_1 = pu_region[0];
    awb_cfg_v35->vertex0_v_1 = pv_region[0];
    awb_cfg_v35->vertex1_u_1 = pu_region[1];
    awb_cfg_v35->vertex1_v_1 = pv_region[1];
    awb_cfg_v35->vertex2_u_1 = pu_region[2];
    awb_cfg_v35->vertex2_v_1 = pv_region[2];
    awb_cfg_v35->vertex3_u_1 = pu_region[3];
    awb_cfg_v35->vertex3_v_1 = pv_region[3];
    awb_cfg_v35->islope01_1  = slope_inv[0];
    awb_cfg_v35->islope12_1  = slope_inv[1];
    awb_cfg_v35->islope23_1  = slope_inv[2];
    awb_cfg_v35->islope30_1  = slope_inv[3];
    UvParaFixed32(&awb_meas->wpEngine.wpDct_uvSpace.wpRegion[2], pu_region, pv_region, slope_inv);
    awb_cfg_v35->vertex0_u_2 = pu_region[0];
    awb_cfg_v35->vertex0_v_2 = pv_region[0];
    awb_cfg_v35->vertex1_u_2 = pu_region[1];
    awb_cfg_v35->vertex1_v_2 = pv_region[1];
    awb_cfg_v35->vertex2_u_2 = pu_region[2];
    awb_cfg_v35->vertex2_v_2 = pv_region[2];
    awb_cfg_v35->vertex3_u_2 = pu_region[3];
    awb_cfg_v35->vertex3_v_2 = pv_region[3];
    awb_cfg_v35->islope01_2  = slope_inv[0];
    awb_cfg_v35->islope12_2  = slope_inv[1];
    awb_cfg_v35->islope23_2  = slope_inv[2];
    awb_cfg_v35->islope30_2  = slope_inv[3];
    UvParaFixed32(&awb_meas->wpEngine.wpDct_uvSpace.wpRegion[3], pu_region, pv_region, slope_inv);
    awb_cfg_v35->vertex0_u_3 = pu_region[0];
    awb_cfg_v35->vertex0_v_3 = pv_region[0];
    awb_cfg_v35->vertex1_u_3 = pu_region[1];
    awb_cfg_v35->vertex1_v_3 = pv_region[1];
    awb_cfg_v35->vertex2_u_3 = pu_region[2];
    awb_cfg_v35->vertex2_v_3 = pv_region[2];
    awb_cfg_v35->vertex3_u_3 = pu_region[3];
    awb_cfg_v35->vertex3_v_3 = pv_region[3];
    awb_cfg_v35->islope01_3  = slope_inv[0];
    awb_cfg_v35->islope12_3  = slope_inv[1];
    awb_cfg_v35->islope23_3  = slope_inv[2];
    awb_cfg_v35->islope30_3  = slope_inv[3];
    ...
    awb_cfg_v35->nor_x0_0 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_x1_0 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_y0_0 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->nor_y1_0 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_x0_0 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->big_x1_0 = UtlFloatToFix_S0310(bigWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->big_y0_0 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_y1_0 = UtlFloatToFix_S0310(bigWpRegion->rbVtx.hw_awbT_vtxY_val);
    norWpRegion           = &awb_meas->wpEngine.wpDct_xySpace.norWpRegion[1];
    bigWpRegion           = &awb_meas->wpEngine.wpDct_xySpace.bigWpRegion[1];
    awb_cfg_v35->nor_x0_1 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_x1_1 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_y0_1 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->nor_y1_1 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_x0_1 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->big_x1_1 = UtlFloatToFix_S0310(bigWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->big_y0_1 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_y1_1 = UtlFloatToFix_S0310(bigWpRegion->rbVtx.hw_awbT_vtxY_val);
    norWpRegion           = &awb_meas->wpEngine.wpDct_xySpace.norWpRegion[2];
    bigWpRegion           = &awb_meas->wpEngine.wpDct_xySpace.bigWpRegion[2];
    awb_cfg_v35->nor_x0_2 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_x1_2 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_y0_2 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->nor_y1_2 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_x0_2 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->big_x1_2 = UtlFloatToFix_S0310(bigWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->big_y0_2 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_y1_2 = UtlFloatToFix_S0310(bigWpRegion->rbVtx.hw_awbT_vtxY_val);
    norWpRegion           = &awb_meas->wpEngine.wpDct_xySpace.norWpRegion[3];
    bigWpRegion           = &awb_meas->wpEngine.wpDct_xySpace.bigWpRegion[3];
    awb_cfg_v35->nor_x0_3 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_x1_3 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxX_val);
    awb_cfg_v35->nor_y0_3 = UtlFloatToFix_S0310(norWpRegion->ltVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->nor_y1_3 = UtlFloatToFix_S0310(norWpRegion->rbVtx.hw_awbT_vtxY_val);
    awb_cfg_v35->big_x0_3 = UtlFloatToFix_S0310(bigWpRegion->ltVtx.hw_awbT_vtxX_val);
    ...
    awb_cfg_v35->multiwindow0_v_offs = awb_meas->mainWin.nonROI[0].hw_awbCfg_nonROI_y;
    //note  multiwindow0_h_size means multiwindow0_h_end in chip
    awb_cfg_v35->multiwindow0_h_size = awb_meas->mainWin.nonROI[0].hw_awbCfg_nonROI_x +
                                       awb_meas->mainWin.nonROI[0].hw_awbCfg_nonROI_width;
    awb_cfg_v35->multiwindow0_v_size = awb_meas->mainWin.nonROI[0].hw_awbCfg_nonROI_y +
                                       awb_meas->mainWin.nonROI[0].hw_awbCfg_nonROI_height;
    awb_cfg_v35->multiwindow1_h_offs = awb_meas->mainWin.nonROI[1].hw_awbCfg_nonROI_x;
    awb_cfg_v35->multiwindow1_v_offs = awb_meas->mainWin.nonROI[1].hw_awbCfg_nonROI_y;
    awb_cfg_v35->multiwindow1_h_size = awb_meas->mainWin.nonROI[1].hw_awbCfg_nonROI_x +
                                       awb_meas->mainWin.nonROI[1].hw_awbCfg_nonROI_width;
    awb_cfg_v35->multiwindow1_v_size = awb_meas->mainWin.nonROI[1].hw_awbCfg_nonROI_y +
                                       awb_meas->mainWin.nonROI[1].hw_awbCfg_nonROI_height;
    awb_cfg_v35->multiwindow2_h_offs = awb_meas->mainWin.nonROI[2].hw_awbCfg_nonROI_x;
    awb_cfg_v35->multiwindow2_v_offs = awb_meas->mainWin.nonROI[2].hw_awbCfg_nonROI_y;
    awb_cfg_v35->multiwindow2_h_size = awb_meas->mainWin.nonROI[2].hw_awbCfg_nonROI_x +
                                       awb_meas->mainWin.nonROI[2].hw_awbCfg_nonROI_width;
    awb_cfg_v35->multiwindow2_v_size = awb_meas->mainWin.nonROI[2].hw_awbCfg_nonROI_y +
                                       awb_meas->mainWin.nonROI[2].hw_awbCfg_nonROI_height;
    awb_cfg_v35->multiwindow3_h_offs = awb_meas->mainWin.nonROI[3].hw_awbCfg_nonROI_x;
    awb_cfg_v35->multiwindow3_v_offs = awb_meas->mainWin.nonROI[3].hw_awbCfg_nonROI_y;
    awb_cfg_v35->multiwindow3_h_size = awb_meas->mainWin.nonROI[3].hw_awbCfg_nonROI_x +
                                       awb_meas->mainWin.nonROI[3].hw_awbCfg_nonROI_width;
    awb_cfg_v35->multiwindow3_v_size = awb_meas->mainWin.nonROI[3].hw_awbCfg_nonROI_y +
    ...
    awb_cfg_v35->exc_wp_region0_xu1 =
    ...
    awb_cfg_v35->exc_wp_region0_yv0 =
    ...
    awb_cfg_v35->exc_wp_region0_yv1 =
    ...
    awb_cfg_v35->exc_wp_region0_weight =
    ...
    awb_cfg_v35->exc_wp_region1_xu1 =
    ...
    awb_cfg_v35->exc_wp_region1_yv0 =
    ...
    awb_cfg_v35->exc_wp_region1_yv1 =
    ...
    awb_cfg_v35->exc_wp_region1_weight =
    ...
    awb_cfg_v35->exc_wp_region2_xu1 =
    ...
    awb_cfg_v35->exc_wp_region2_yv0 =
    ...
    awb_cfg_v35->exc_wp_region2_yv1 =
    ...
    awb_cfg_v35->exc_wp_region2_weight =
    ...
    awb_cfg_v35->exc_wp_region3_xu1 =
    ...
    awb_cfg_v35->exc_wp_region3_yv0 =
    ...
    awb_cfg_v35->exc_wp_region3_yv1 =
    ...
    awb_cfg_v35->exc_wp_region3_weight =
    ...
    awb_cfg_v35->exc_wp_region4_xu1 =
    ...
    awb_cfg_v35->exc_wp_region4_yv0 =
    ...
    awb_cfg_v35->exc_wp_region4_yv1 =
    ...
    awb_cfg_v35->exc_wp_region4_weight =
    ...
    awb_cfg_v35->exc_wp_region5_xu1 =
    ...
    awb_cfg_v35->exc_wp_region5_yv0 =
    ...
    awb_cfg_v35->exc_wp_region5_yv1 =
    ...
    awb_cfg_v35->exc_wp_region5_weight =
    ...
    awb_cfg_v35->exc_wp_region6_xu1 =
    ...
    awb_cfg_v35->exc_wp_region6_yv0 =
    ...
    awb_cfg_v35->exc_wp_region6_yv1 =
    ...
    awb_cfg_v35->exc_wp_region6_weight =
        (uint8_t)(awb_meas->wpEngine.wpFiltOut_smpEntity[2].hw_awbT_stats_wgt *
                  ((1 << RK_AIQ_WP_INCLUDE_BIS) - 1) +
                  0.5);

    LOGV_AWB("hw_awbT_luma2WpWgt_ccm(%f,%f,%f,%f,%f,%f,%f,%f,%f)",
             awb_cfg_v35->ccm_coeff0_r,
             awb_cfg_v35->ccm_coeff1_r,
             awb_cfg_v35->ccm_coeff2_r,
             awb_cfg_v35->ccm_coeff0_g,
             awb_cfg_v35->ccm_coeff1_g,
             awb_cfg_v35->ccm_coeff2_g,
             awb_cfg_v35->ccm_coeff0_b,
             awb_cfg_v35->ccm_coeff1_b,
             awb_cfg_v35->ccm_coeff2_b);

    awb_cfg_v35->ccm_coeff0_r = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[0]);
    awb_cfg_v35->ccm_coeff1_r = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[1]);
    awb_cfg_v35->ccm_coeff2_r = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[2]);
    awb_cfg_v35->ccm_coeff0_g = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[3]);
    awb_cfg_v35->ccm_coeff1_g = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[4]);
    awb_cfg_v35->ccm_coeff2_g = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[5]);
    awb_cfg_v35->ccm_coeff0_b = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[6]);
    awb_cfg_v35->ccm_coeff1_b = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[7]);
    awb_cfg_v35->ccm_coeff2_b = UtlFloatToFix_S0406(awb_meas->wpEngine.hw_awbT_luma2WpWgt_ccm[8]);

    awb_cfg_v35->wp_luma_weicurve_y0 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[0];
    awb_cfg_v35->wp_luma_weicurve_y1 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[1];
    awb_cfg_v35->wp_luma_weicurve_y2 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[2];
    awb_cfg_v35->wp_luma_weicurve_y3 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[3];
    awb_cfg_v35->wp_luma_weicurve_y4 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[4];
    awb_cfg_v35->wp_luma_weicurve_y5 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[5];
    awb_cfg_v35->wp_luma_weicurve_y6 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[6];
    awb_cfg_v35->wp_luma_weicurve_y7 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[7];
    awb_cfg_v35->wp_luma_weicurve_y8 = awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.idx[8];
    awb_cfg_v35->wp_luma_weicurve_w0 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[0] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w1 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[1] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w2 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[2] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w3 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[3] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w4 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[4] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w5 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[5] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w6 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[6] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w7 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[7] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    awb_cfg_v35->wp_luma_weicurve_w8 = (uint8_t)(awb_meas->wpEngine.hw_awbT_luma2WpWgt_curve.val[8] *
                                       ((1 << RK_AIQ_AWB_WP_WEIGHT_BIS_V201) - 1) +
                                       0.5);
    ...
    for (int i = 0; i < RK_AIQ_AWB_GRID_NUM_TOTAL; i++) {
        awb_cfg_v35->wp_blk_wei_w[i] = awb_meas->wpEngine.hw_awbCfg_zone_wgt[i];
    }
    awb_cfg_v35->blk_rtdw_measure_en = 0;
    WriteDataForIcCmodel(awb_cfg_v35);
    WriteAwbReg(awb_cfg_v35);
    pCvt->mLatestMeasCfg.rawawb = pCvt->isp_params.isp_cfg->meas.rawawb;
}

#if RKAIQ_HAVE_CAC_V30
static void convertAiqCacToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase,
                                       struct isp35_isp_params_cfg* isp_cfg,
                                       struct isp35_isp_params_cfg* isp_cfg_right,
                                       bool is_multi_isp) {
    if (pBase->en) {
...
static void convertAiqBtnrToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
#if 1
    btnr_api_attrib_t *btnr_attrib = pCvt->btnr_attrib;
    btnr_params_static_t* psta = NULL;
    if (btnr_attrib->opMode == RK_AIQ_OP_MODE_AUTO) {
        psta = &btnr_attrib->stAuto.sta;
    } else {
        psta = &btnr_attrib->stMan.sta;
    }
    btnr_dloMd_mode_t tmpDLoMode = psta->dLoMd_mode;
    if (tmpDLoMode == btnr_dLoMdCurOrPre_mode) {
        // bayer3dnr enable  bayer2dnr must enable at the same time
    ...
    pCvt->isp_params.isp_cfg->others.bay3d_l2_cfg.bypass_en = 0;
    pCvt->mBtnrInfo.btnr_attrib = pCvt->btnr_attrib;
...
static void convertAiqYnrToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
    ...
    if(pBase->bypass) {
        ...
        LOGW_ANR("Bypass ynr instead by ynr.dyn.hiNr.hw_ynrT_hiNr_en, ynr.dyn.midLoNr.mf.hw_ynrT_midNr_en and ynr.dyn.midLoNr.lf.hw_ynrT_loNr_en.");
    }

    rk_aiq_ynr41_params_cvt(pBase->_data, &pCvt->isp_params, &pCvt->mCommonCvtInfo);
}
#endif

#if (RKAIQ_HAVE_CNR_V36)
static void convertAiqCnrToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
static void convertAiqDmToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
    ...
    rk_aiq_dm25_params_cvt(pBase->_data, &pCvt->isp_params, &pCvt->mCommonCvtInfo);
}
#endif

#if (RKAIQ_HAVE_SHARP_V41)
static void convertAiqTexEstToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    memcpy(&pCvt->mCommonCvtInfo.texEst_param, pBase->_data, sizeof(texEst_param_t));
}

static void convertAiqSharpToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
        pCvt->isp_params.isp_cfg->module_ens |= ISP35_MODULE_SHARP;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_SHARP;
        pCvt->isp_params.isp_cfg->module_cfg_update |= ISP35_MODULE_SHARP;
    }
    else {
        pCvt->isp_params.isp_cfg->module_ens &= ~ISP35_MODULE_SHARP;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_SHARP;
    ...
    rk_aiq_sharp41_params_cvt(pBase->_data, &pCvt->isp_params, &pCvt->mCommonCvtInfo, &pCvt->mBtnrInfo);
}
#endif
...
static void convertAiqGicToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    rk_aiq_gic30_params_cvt(pBase->_data, &pCvt->isp_params.isp_cfg->others.gic_cfg, &pCvt->mBtnrInfo);
}
#endif
...
static void convertAiqHisteqToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
        pCvt->isp_params.isp_cfg->module_ens |= ISP35_MODULE_HIST;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_HIST;
        pCvt->isp_params.isp_cfg->module_cfg_update |= ISP35_MODULE_HIST;
    } else {
        pCvt->isp_params.isp_cfg->module_ens &= ~ISP35_MODULE_HIST;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_HIST;
    }
...
static void convertAiqEnhToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
        pCvt->isp_params.isp_cfg->module_ens |= ISP35_MODULE_ENH;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_ENH;
        pCvt->isp_params.isp_cfg->module_cfg_update |= ISP35_MODULE_ENH;
    }
    else {
        pCvt->isp_params.isp_cfg->module_ens &= ~ISP35_MODULE_ENH;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_ENH;
...
static void convertAiqHsvToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
        pCvt->isp_params.isp_cfg->module_ens |= ISP35_MODULE_HSV;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_HSV;
        pCvt->isp_params.isp_cfg->module_cfg_update |= ISP35_MODULE_HSV;
    } else {
        pCvt->isp_params.isp_cfg->module_ens &= ~ISP35_MODULE_HSV;
        pCvt->isp_params.isp_cfg->module_en_update |= ISP35_MODULE_HSV;
    }

    rk_aiq_hsv11_params_cvt(pBase->_data, &pCvt->isp_params, &pCvt->mCommonCvtInfo);
}
#endif

#if RKAIQ_HAVE_MERGE_V13
void convertAiqMergeToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
    ...
    rk_aiq_merge23_params_cvt(pBase->_data, &pCvt->isp_params, &pCvt->mCommonCvtInfo,  &pCvt->mergeLuma2Wgt);
}
#endif

#if RKAIQ_HAVE_DRC_V21
void convertAiqDrcToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    rk_aiq_drc41_params_cvt(pBase->_data, &pCvt->isp_params, &pCvt->mCommonCvtInfo, pBase->en);
}
#endif
...
struct params_cvt_info_isp35 {
    int32_t type;
    const char* const name;
    void (*func)(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase);
};

static const struct params_cvt_info_isp35 params_cvts_isp35[] = {
...
static bool params_cvt_is_known(int32_t type) {
    ...
    return params_cvts_isp35[type].type == type;
}

bool Convert3aResultsToIsp35Cfg(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase,
                                void* isp_cfg_p, bool is_multi_isp) {
    struct isp35_isp_params_cfg* isp_cfg = (struct isp35_isp_params_cfg*)isp_cfg_p;
...
#if RKAIQ_HAVE_CAC_V30
        struct isp35_isp_params_cfg* isp_cfg = (struct isp35_isp_params_cfg*)isp_cfg_p;
        struct isp35_isp_params_cfg* isp_cfg_right = isp_cfg + 1;
        convertAiqCacToIsp35Params(pCvt, pBase, isp_cfg, isp_cfg_right, is_multi_isp);
#endif
    }
    break;
    ...
    default:
        if (params_cvt_is_known(pBase->type)) {
            const struct params_cvt_info_isp35* info = &params_cvts_isp35[pBase->type];
...
static void UpdateModEn(struct isp35_isp_params_cfg* src, struct isp35_isp_params_cfg* dst) {
...
static void UpdateMeasModCfg(struct isp35_isp_params_cfg* src, struct isp35_isp_params_cfg* dst) {
...
static void UpdateOthersModCfg(struct isp35_isp_params_cfg* src, struct isp35_isp_params_cfg* dst) {
    u64 module_cfg_update = src->module_cfg_update;
...
void AiqIspParamsCvt_updIsp35Params(void* src, void* dst) {
    if (!src || !dst) return;

    struct isp35_isp_params_cfg* pSrc = (struct isp35_isp_params_cfg*)src;
    struct isp35_isp_params_cfg* pDst = (struct isp35_isp_params_cfg*)dst;
