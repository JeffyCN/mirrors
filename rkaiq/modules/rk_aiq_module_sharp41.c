...
void rk_aiq_sharp41_params_cvt(void* attr, isp_params_t* isp_params, common_cvt_info_t *cvtinfo, btnr_cvt_info_t* pBtnrInfo)
{
    ...
    struct isp35_sharp_cfg *pCfg = &isp_params->isp_cfg->others.sharp_cfg;
    ...
    // REG: TEXTURE0
    tmp = ROUND_F(noise_norm_scale * (pTexDyn->texEst.hw_texEstT_nsEstDf1_scale) * (1 << 6));
    pCfg->fst_noise_scale = CLIP(tmp, 0, 0xffff);
    tmp = ROUND_F(noise_norm_scale * (pTexDyn->texEst.hw_texEstT_sigmaDf1_scale) * (1 << 6));
    ...
    tmp = ROUND_F((pTexDyn->texEst.hw_texEstT_texEstDf1_scale) * (1 << 10));
    ...
    // REG: TEXTURE3
    tmp = ROUND_F(noise_norm_scale * (pTexDyn->texEst.hw_texEstT_nsEstDf2_scale) * (1 << 6));
    pCfg->sec_noise_scale = CLIP(tmp, 0, 0xffff);
    tmp = ROUND_F(noise_norm_scale * (pTexDyn->texEst.hw_texEstT_sigmaDf2_scale) * (1 << 6));
    ...
    tmp = ROUND_F((pTexDyn->texEst.hw_texEstT_texEstDf2_scale) * (1 << 10));
...

    {
        ...
        if (pdyn->eHfDetailShp.detailExtra_hpf.sw_shpT_filtCfg_mode == shp_cfgByFiltStrg_mode) {
            ...
            int imgHpf_radius0 = pdyn->eHfDetailShp.detailExtra_hpf.sw_shpT_loFiltRadius_mode;
            int imgHpf_radius1 = pdyn->eHfDetailShp.detailExtra_hpf.sw_shpT_upFiltRadius_mode;
            float rsigma0 = pdyn->eHfDetailShp.detailExtra_hpf.sw_shpT_loFiltSpatial_strg;
            float rsigma1 = pdyn->eHfDetailShp.detailExtra_hpf.sw_shpT_upFiltSpatial_strg;
            SharpCreateKernelCoeffs(imgHpf_radius0, 5 / 2, rsigma0, &tmp_hpf_coeff0[0], 7, 2);
            SharpCreateKernelCoeffs(imgHpf_radius1, 5 / 2, rsigma1, &tmp_hpf_coeff1[0], 7, 2);
            for (int i = 0; i < 6; i++) {
                coeff[i] = tmp_hpf_coeff0[i] - tmp_hpf_coeff1[i];
            }
        } else {
            for (i = 0; i < 6; i++)
...

    {
        ...
        if (pdyn->detailShp.midDetailExtra_lpf.sw_shpT_filtCfg_mode == shp_cfgByFiltStrg_mode) {
            float rsigma = pdyn->detailShp.midDetailExtra_lpf.sw_shpT_filtSpatial_strg;
            SharpCreateKernelCoeffs(radius, 5 / 2, rsigma, coeff, 7, 2);
        } else {
            for (i = 0; i < 6; i++)
    ...
    // REG: GAIN
    tmp = ROUND_F((pdyn->locShpStrg.motionStrg.hw_shpT_glbSgmStrg_val) * (1 << 4));
    pCfg->global_gain = CLIP(tmp, 0, 0x3ff);
    tmp = ROUND_F((pdyn->locShpStrg.motionStrg.hw_shpT_glbSgmStrg_alpha) * (1 << 3));
    pCfg->gain_merge_alpha = CLIP(tmp, 0, 0xf);
    tmp = ROUND_F((pdyn->locShpStrg.motionStrg.hw_shpT_localSgmStrg_scale) * (1 << 7));
    ...
    // REG: GAIN_ADJ4
    tmp = ROUND_F((pdyn->edgeShp.locShpStrg_motionStrg2.hw_shpT_motionStrg_sigma) * (1 << 11));
    pCfg->edge_gain_sigma = CLIP(tmp, 0, 0xfff);
    tmp = ROUND_F((pdyn->detailShp.locShpStrg_motionStrg2.hw_shpT_motionStrg_sigma) * (1 << 11));
    pCfg->detail_gain_sigma = CLIP(tmp, 0, 0xfff);
    // REG: EDGE0
    tmp = ROUND_F((pdyn->edgeShp.locShpStrg_edge.hw_shpT_edgePosIdx_scale) * (1 << 2));
    pCfg->pos_edge_wgt_scale = CLIP(tmp, 0, 0xfff);
    tmp = ROUND_F((pdyn->edgeShp.locShpStrg_edge.hw_shpT_edgeNegIdx_scale) * (1 << 2));
    pCfg->neg_edge_wgt_scale = CLIP(tmp, 0, 0xfff);
    // REG: EDGE1
    tmp = ROUND_F((pdyn->edgeShp.glbShpStrg.hw_shpT_edgePos_strg) * (1 << 2));
    pCfg->pos_edge_strg = CLIP(tmp, 0, 0x3f);
    tmp = ROUND_F((pdyn->edgeShp.glbShpStrg.hw_shpT_edgeNeg_strg) * (1 << 2));
    pCfg->neg_edge_strg = CLIP(tmp, 0, 0x3f);
    tmp = ROUND_F((pdyn->edgeShp.shootReduction.sw_shpT_overShoot_strg) * (1 << 5));
    pCfg->overshoot_alpha = CLIP(tmp, 0, 0x3f);
    tmp = ROUND_F((pdyn->edgeShp.shootReduction.sw_shpT_underShoot_strg) * (1 << 5));
...

    {
        ...
        int coeff1[10];
        int coeff[10];
        int radius0 = pdyn->edgeShp.edgeExtra_bpf.sw_shpT_loFiltRadius_mode;
        int radius1 = pdyn->edgeShp.edgeExtra_bpf.sw_shpT_upFiltRadius_mode;
        if (pdyn->edgeShp.edgeExtra_bpf.sw_shpT_filtCfg_mode == shp_cfgByFiltStrg_mode) {
            float rsigma0 = pdyn->edgeShp.edgeExtra_bpf.sw_shpT_loFiltSpatial_strg;
            float rsigma1 = pdyn->edgeShp.edgeExtra_bpf.sw_shpT_upFiltSpatial_strg;
            SharpCreateKernelCoeffs(radius0, 7 / 2, rsigma0, coeff0, 7, 2);
            SharpCreateKernelCoeffs(radius1, 7 / 2, rsigma1, coeff1, 7, 2);

            for (i = 0; i < 10; i++) {
                coeff[i] = coeff0[i] - coeff1[i];
            }
        } else {
            for (i = 0; i < 10; i++)
                coeff[i] = ROUND_F(pdyn->edgeShp.edgeExtra_bpf.hw_shpT_bpfSpatial_wgt[i] * (1 << 7));
        }
        ...
        SharpKernelCoeffsNormalization(coeff, 7 / 2, 0, 2);
        for(int k = 1; k < 3; k++) {
            if(coeff[k] < 0) {
                tmp = coeff[k];
                coeff[k] =  tmp & ((1 << 6) - 1);
        ...
        // REG: EDGE_KERNEL0
        pCfg->edge_bpf_coeff[0] = CLIP(coeff[0], 0, 0x7f);
        pCfg->edge_bpf_coeff[1] = CLIP(coeff[1], 0, 0x3f);
        pCfg->edge_bpf_coeff[2] = CLIP(coeff[2], 0, 0x3f);
        pCfg->edge_bpf_coeff[3] = CLIP(coeff[3], 0, 0x1f);
        // REG: EDGE_KERNEL1
        pCfg->edge_bpf_coeff[4] = CLIP(coeff[4], 0, 0x1f);
        pCfg->edge_bpf_coeff[5] = CLIP(coeff[5], 0, 0x1f);
        pCfg->edge_bpf_coeff[6] = CLIP(coeff[6], 0, 0x1f);
        pCfg->edge_bpf_coeff[7] = CLIP(coeff[7], 0, 0x1f);
        // REG: EDGE_KERNEL2
        pCfg->edge_bpf_coeff[8] = CLIP(coeff[8], 0, 0x1f);
        pCfg->edge_bpf_coeff[9] = CLIP(coeff[9], 0, 0x1f);
    }
...

    {
        ...
        tmp = ROUND_F((pdyn->detailShp.glbShpStrg.hw_shpT_detailNeg_strg) * (1 << 2));
...

    {
        int tex_adj_strg[9];
        for (i = 0; i < 9; i++) {
            tex_adj_strg[i] = ROUND_F(pdyn->detailShp.locShpStrg_tex.hw_shpT_tex2Shp_strg[i] * (1 << 10));
        }
        pCfg->tex2detail_strg[0] = CLIP(tex_adj_strg[0], 0, 0x3ff);
        pCfg->tex2detail_strg[1] = CLIP(tex_adj_strg[1], 0, 0x3ff);
        pCfg->tex2detail_strg[2] = CLIP(tex_adj_strg[2], 0, 0x3ff);
        pCfg->tex2detail_strg[3] = CLIP(tex_adj_strg[3], 0, 0x3ff);
        pCfg->tex2detail_strg[4] = CLIP(tex_adj_strg[4], 0, 0x3ff);
        pCfg->tex2detail_strg[5] = CLIP(tex_adj_strg[5], 0, 0x3ff);
        pCfg->tex2detail_strg[6] = CLIP(tex_adj_strg[6], 0, 0x3ff);
        pCfg->tex2detail_strg[7] = CLIP(tex_adj_strg[7], 0, 0x3ff);
        pCfg->tex2detail_strg[8] = CLIP(tex_adj_strg[8], 0, 0x3ff);
    }
    ...
    int noise_curve_ext[17] = {0};
    {
        ...
        for (i = 0; i < 17; i++) {
            int ynr_lo_noise_sigma = cvtinfo->ynr_sigma[i] * (1 << 3);
    ...
    {
        int tex_adj_mf_strg[9];
        for (i = 0; i < 9; i++) {
            tex_adj_mf_strg[i] = ROUND_F(pdyn->mfDetailShp.locShpStrg_tex.hw_shpT_tex2Shp_strg[i] * (1 << 10));
        }
        pCfg->tex2mf_detail_strg[0] = CLIP(tex_adj_mf_strg[0], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[1] = CLIP(tex_adj_mf_strg[1], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[2] = CLIP(tex_adj_mf_strg[2], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[3] = CLIP(tex_adj_mf_strg[3], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[4] = CLIP(tex_adj_mf_strg[4], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[5] = CLIP(tex_adj_mf_strg[5], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[6] = CLIP(tex_adj_mf_strg[6], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[7] = CLIP(tex_adj_mf_strg[7], 0, 0x3ff);
        pCfg->tex2mf_detail_strg[8] = CLIP(tex_adj_mf_strg[8], 0, 0x3ff);
    }
    ...
    {
        int coeff[3];
        if (pdyn->edgeShp.locShpStrg_edge.sw_shpT_edgeWgtFltCfg_mode == shp_cfgByFiltStrg_mode) {
            float rsigma = pdyn->edgeShp.locShpStrg_edge.sw_shpT_edgeWgtFlt_strg;
            SharpCreateKernelCoeffs(1, 1, rsigma, coeff, 6, 2);
        } else {
            for (i = 0; i < 3; i++) {
                coeff[i] = ROUND_F(pdyn->edgeShp.locShpStrg_edge.sw_shpT_edgeWgtFlt_wgt[i] * (1 << 6));
            }
        }

        SharpKernelCoeffsNormalization(coeff, 1, (1 << 6), 2);

        pCfg->edge_wgt_flt_coeff0 = CLIP(coeff[0], 0, 0x3f);
        pCfg->edge_wgt_flt_coeff1 = CLIP(coeff[1], 0, 0x3f);
        pCfg->edge_wgt_flt_coeff2 = CLIP(coeff[2], 0, 0x3f);
    }

    // edge global clip
    tmp = pdyn->edgeShp.shootReduction.hw_shpT_glbClip_thred;
    pCfg->edge_glb_clip_thred = CLIP(tmp, 0, 0x3ff);
    tmp = pdyn->edgeShp.shootReduction.hw_shpT_edgePosClip_val;
    pCfg->pos_edge_clip       = CLIP(tmp, 0, 0x3ff);
    tmp =  pdyn->edgeShp.shootReduction.hw_shpT_edgeNegClip_val;
    pCfg->neg_edge_clip       = CLIP(tmp, 0, 0x3ff);

    // global mi-freq strg
    tmp = ROUND_F(pdyn->mfDetailShp.glbShpStrg.hw_shpT_detailPos_strg * (1 << 2));
    pCfg->pos_mf_detail_strg    = CLIP(tmp, 0, 0x3f);
    tmp = ROUND_F(pdyn->mfDetailShp.glbShpStrg.hw_shpT_detailNeg_strg * (1 << 2));
    pCfg->neg_mf_detail_strg    = CLIP(tmp, 0, 0x3f);
    tmp = ROUND_F(pdyn->mfDetailShp.detailExtra.hw_shpT_detailSrcMf_alpha * (1 << 5));
    pCfg->mf_detail_data_alpha  = CLIP(tmp, 0, 0x20);
    // mifreq clip
    tmp = pdyn->mfDetailShp.shootReduction.hw_shpT_detailPosClip_val;
    pCfg->mf_detail_pos_clip    = CLIP(tmp, 0, 0x3ff);
    tmp = pdyn->mfDetailShp.shootReduction.hw_shpT_detailNegClip_val;
    pCfg->sharp_mf_detail_neg_clip = CLIP(tmp, 0, 0x3ff);

    {
        int sat_adj_strg[9];
        if(pdyn->locShpStrg.sat.sw_shpT_sat_en) {
            for (i = 0; i < 9; i++) {
                sat_adj_strg[i] = ROUND_F(pdyn->locShpStrg.sat.hw_shpT_sat2ShpStrg_alpha[i] * (1 << 4));
            }
        } else {
            for (i = 0; i < 9; i++) {
                sat_adj_strg[i] = ROUND_F(1 * (1 << 4));
            }
        }
        pCfg->staturation2strg[0] = CLIP(sat_adj_strg[0], 0, 0x1f);
        pCfg->staturation2strg[1] = CLIP(sat_adj_strg[1], 0, 0x1f);
        pCfg->staturation2strg[2] = CLIP(sat_adj_strg[2], 0, 0x1f);
        pCfg->staturation2strg[3] = CLIP(sat_adj_strg[3], 0, 0x1f);
        pCfg->staturation2strg[4] = CLIP(sat_adj_strg[4], 0, 0x1f);
        pCfg->staturation2strg[5] = CLIP(sat_adj_strg[5], 0, 0x1f);
        pCfg->staturation2strg[6] = CLIP(sat_adj_strg[6], 0, 0x1f);
        pCfg->staturation2strg[7] = CLIP(sat_adj_strg[7], 0, 0x1f);
        pCfg->staturation2strg[8] = CLIP(sat_adj_strg[8], 0, 0x1f);
    }
    // saturation strength

    tmp = ROUND_F(pdyn->locShpStrg.sat.hw_shpT_loSatRegionShp_strg * (1 << 9));
    pCfg->lo_saturation_strg = CLIP(tmp, 0, 0x3ff);
