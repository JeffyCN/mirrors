...
{
    struct isp35_ynr_cfg *pCfg = &isp_params->isp_cfg->others.ynr_cfg;
    ...
    pCfg->dsfilt_bypass = !pdyn->midLoNr.lf.dsFlt.hw_ynrT_dsFlt_en;
    pCfg->tex2wgt_en = pdyn->midLoNr.hw_ynrT_tex2NrOutAlpha_en;
...
#else // fix -Werror=strict-aliasing compiled by arm-rockchip1240-linux-gnueabihf-gcc (v2.38) 12.4.0
        int number_ptr = 0;
        memcpy(&number_ptr, (int*)(&r_sq_inv), sizeof(int));
        int EE = ((number_ptr) >> 23) & (0x0ff);
        EE = -(EE - 127 + 1);
        int MM = (number_ptr) & 0x7fffff;
#endif
...

    {
        ...
        if (pdyn->hiNr.epf.sw_ynrT_filtCfg_mode == ynr_cfgByFiltStrg_mode) {
            ...
            float w_sum_f = 1.0f + w01_f * 4 + w02_f * 4 + w11_f * 4 + w12_f * 8;
...

    {
        ...
        if (pdyn->midLoNr.mf.sw_ynrT_filtCfg_mode == ynr_cfgByFiltStrg_mode) {
            ...
            float mid_w01_f = exp(-(0 + 1) / (2.0 * ynr_mi_gauss_sigma * ynr_mi_gauss_sigma));
            float mid_w02_f = exp(-(0 + 4) / (2.0 * ynr_mi_gauss_sigma * ynr_mi_gauss_sigma));
            float mid_w11_f = exp(-(1 + 1) / (2.0 * ynr_mi_gauss_sigma * ynr_mi_gauss_sigma));
            float mid_w12_f = exp(-(1 + 4) / (2.0 * ynr_mi_gauss_sigma * ynr_mi_gauss_sigma));
            float mid_w_sum_f = 1.0f + mid_w01_f * 4 + mid_w02_f * 4 + mid_w11_f * 4 + mid_w12_f * 8;

            int mid_rem_sum = 128;

            int mid_w12_int = (int)(mid_w12_f / mid_w_sum_f * 128); 
            mid_rem_sum = mid_rem_sum - mid_w12_int * 8;
            mid_w_sum_f = mid_w_sum_f - mid_w12_f * 8;

            int mid_w11_int = (int)(mid_w11_f / mid_w_sum_f * mid_rem_sum); 
            mid_rem_sum = mid_rem_sum - mid_w11_int * 4;
            mid_w_sum_f = mid_w_sum_f - mid_w11_f * 4;

            int mid_w02_int = (int)(mid_w02_f / mid_w_sum_f * mid_rem_sum); 
            mid_rem_sum = mid_rem_sum - mid_w02_int * 4;
            mid_w_sum_f = mid_w_sum_f - mid_w02_f * 4;

            int mid_w01_int = (int)(mid_w01_f / mid_w_sum_f * mid_rem_sum); 
            mid_rem_sum = mid_rem_sum - mid_w01_int * 4;
            mid_w_sum_f = mid_w_sum_f - mid_w01_f * 4;

            int mid_w00_int = mid_rem_sum;
            coeff[0] = mid_w00_int;
            coeff[1] = mid_w01_int;
            coeff[2] = mid_w02_int;
            coeff[3] = mid_w11_int;
        ...
        // REG: MI_GAUS_COE
        pCfg->mi_spnr_filt_coeff0 = CLIP(coeff[0], 0, 0x80);
        pCfg->mi_spnr_filt_coeff1 = CLIP(coeff[1], 0, 0x1f);
        pCfg->mi_spnr_filt_coeff2 = CLIP(coeff[2], 0, 0x1f);
        pCfg->mi_spnr_filt_coeff3 = CLIP(coeff[3], 0, 0xf);
        pCfg->mi_spnr_filt_coeff4 = CLIP(coeff[4], 0, 0x7);
    }
    // REG: MI_STRG_DETAIL
    ...
    tmp = (pdyn->midLoNr.lf.epf.hw_ynrT_alphaLfTex_scale) * 8;
    pCfg->lo_enhance_scale = CLIP(tmp, 0, 0xff);
	...
	for (i = 0; i < 9; i++){
		pCfg->lo_spnr_tex2wgt_scale[i] = MIN(pdyn->midLoNr.lf.epf.tex2NrOutAlpha[i] * 64, 64);
	}
	for (i = 0; i < 9; i++){
		pCfg->mi_spnr_tex2wgt_scale[i] = MIN(pdyn->midLoNr.mf.tex2NrOutAlpha[i] * 64, 64);
	}

	pCfg->dsfilt_strg = MIN((int)(pdyn->midLoNr.lf.dsFlt.hw_ynrT_dsFlt_strg  * (1 << 6)), 1023);
	pCfg->dsfilt_diff_offset = MIN((int)(pdyn->midLoNr.lf.dsFlt.hw_ynrT_diff_offset  * (1 << 10)), 1023);
	pCfg->dsfilt_center_wgt = MIN(MAX((int)(pdyn->midLoNr.lf.dsFlt.hw_ynrT_centerPix_wgt  * 1024), 1), 2047); // clip to [1, 2047]
    return;
}
