...
#ifndef __CALIBDBV2_LDC_HEAD_H__
#define __CALIBDBV2_LDC_HEAD_H__

#include "common/rk_aiq_comm.h"

RKAIQ_BEGIN_DECLARE

typedef struct CalibDbV2_Ldc_Param_s {
    // clang-format off
    // M4_BOOL_DESC("ldch_en", "0");
    bool ldch_en;
    // M4_BOOL_DESC("ldcv_en", "0");
    bool ldcv_en;
    // M4_STRING_DESC("meshfile", M4_SIZE(1,1), M4_RANGE(0, 256), "default_meshfile", M4_DYNAMIC(0))
    char ldch_meshfile[256];
    // M4_STRING_DESC("meshfile", M4_SIZE(1,1), M4_RANGE(0, 256), "default_meshfile", M4_DYNAMIC(0))
    char ldcv_meshfile[256];
    ...
    // clang-format on
} CalibDbV2_Ldc_Param_t;

typedef struct CalibDbV2_LDC_s {
    // M4_STRUCT_DESC("param", "normal_ui_style")
    CalibDbV2_Ldc_Param_t param;
} CalibDbV2_LDC_t;
