 ...
 *
 */
#ifndef _RK_AIQ_PARAM_DPCC20_H_
#define _RK_AIQ_PARAM_DPCC20_H_

#define DPCC_PDAF_POINT_NUM            (16)
typedef struct Dpcc_bpt_params_s {
    // bpt_ctrl 0x005c
...
typedef struct Dpcc_pdaf_params_s {
    // pdaf_en 0x0070
...
typedef struct Dpcc_Sensor_s {
    float level_single;
    float level_multiple;
} Dpcc_Sensor_t;

typedef struct Dpcc_Fast_Mode_s {
...
typedef struct Dpcc_basic_params_select_s {
    // mode 0x0000
...
typedef struct {
    ...
    float Sensor_dpcc_en;
    float Sensor_dpcc_max_level;
    Dpcc_bpt_params_t stBptParams;
    Dpcc_pdaf_params_t stPdafParams;
} dpcc_params_static_t;

typedef struct {
    // M4_STRUCT_DESC("arBasic", "normal_ui_style")
    Dpcc_basic_params_select_t arBasic;
    // M4_STRUCT_DESC("stFastMode", "normal_ui_style")
    Dpcc_Fast_Mode_t stFastMode;
    Dpcc_Sensor_t stSensorDpcc;
} dpcc_params_dyn_t;
