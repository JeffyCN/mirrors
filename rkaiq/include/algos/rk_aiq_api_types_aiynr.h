...
#ifndef _RK_AIQ_API_TYPES_AIYNR_H_
#define _RK_AIQ_API_TYPES_AIYNR_H_

#include "common/rk_aiq_comm.h"

#define AIYNR_ISO_STEP_MAX          13

RKAIQ_BEGIN_DECLARE

typedef struct aiynr_param_auto_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(sta),
        M4_TYPE(struct),
    ...
    aiynr_param_dyn_t dyn[AIYNR_ISO_STEP_MAX];
} aiynr_param_auto_t;

typedef struct aiynr_api_attrib_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(opMode),
        M4_TYPE(enum),
    ...
    /* M4_GENERIC_DESC(
        ...
        M4_RO(0),
        M4_ORDER(4),
        M4_NOTES(The params of manual mode, be only valid for manual mode))  */
    aiynr_param_t stMan;
} aiynr_api_attrib_t;

typedef struct aiynr_status_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(opMode),
        M4_TYPE(enum),
        M4_ENUM_DEF(rk_aiq_op_mode_t),
