...
typedef struct dehaze_param_auto_s {
    ...
    dehaze_params_dyn_t dyn[DEHAZE_ISO_STEP_MAX];
} dehaze_param_auto_t;

typedef struct dehaze_api_attrib_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(opMode),
        M4_TYPE(enum),
    ...
    dehaze_param_t stMan;
} dehaze_api_attrib_t;

typedef struct dehaze_status_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(opMode),
        M4_TYPE(enum),
        M4_ENUM_DEF(rk_aiq_op_mode_t),
