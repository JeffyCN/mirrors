...
#ifndef _RK_AIQ_API_TYPES_GIC_H_
#define _RK_AIQ_API_TYPES_GIC_H_

#include "common/rk_aiq_comm.h"

#define GIC_ISO_STEP_MAX 13

RKAIQ_BEGIN_DECLARE

typedef struct gic_param_auto_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(sta),
        M4_TYPE(struct),
        ...
        M4_RO(0),
        M4_ORDER(1),
        M4_NOTES(The static params of gic module))  */
    ...
    gic_params_dyn_t dyn[GIC_ISO_STEP_MAX];
} gic_param_auto_t;

typedef struct gic_api_attrib_s {
    /* M4_GENERIC_DESC(
        M4_ALIAS(opMode),
        M4_TYPE(enum),
