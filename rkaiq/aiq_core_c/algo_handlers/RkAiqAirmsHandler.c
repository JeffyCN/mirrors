...
#include "aiq_base.h"
#include "RkAiqAirmsHandler.h"
#include "aiq_core.h"
#include "RkAiqGlobalParamsManager_c.h"

static void _handlerAirms_init(AiqAlgoHandler_t* pHdl) {
    ...
    pHdl->mResultType = RESULT_TYPE_AIRMS_PARAM;
    pHdl->mResultSize = sizeof(airms_param_t);

    EXIT_ANALYZER_FUNCTION();
}

static XCamReturn _handlerAirms_prepare(AiqAlgoHandler_t* pAlgoHandler) {
    ENTER_ANALYZER_FUNCTION();

    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    ret = AiqAlgoHandler_prepare(pAlgoHandler);
    RKAIQCORE_CHECK_RET(ret, "Airms handle prepare failed");
    ...
    RKAIQCORE_CHECK_RET(ret, "Airms algo prepare failed");

    EXIT_ANALYZER_FUNCTION();
    return XCAM_RETURN_NO_ERROR;
}

static XCamReturn _handlerAirms_processing(AiqAlgoHandler_t* pAlgoHandler) {
    ENTER_ANALYZER_FUNCTION();
    ...
    if (ret) {
        RKAIQCORE_CHECK_RET(ret, "Airms handle processing failed");
    }

    AiqAlgoHandler_do_processing_common(pAlgoHandler);

    RKAIQCORE_CHECK_RET(ret, "Airms algo processing failed");

    EXIT_ANALYZER_FUNCTION();
    return ret;
}

AiqAlgoHandler_t* AiqAlgoHandlerAirms_constructor(RkAiqAlgoDesComm* des, AiqCore_t* aiqCore) {
    AiqAlgoHandler_t* pHdl = (AiqAlgoHandler_t*)aiq_mallocz(sizeof(AiqAlgoHandler_t));
    if (!pHdl)
        return NULL;
    AiqAlgoHandler_constructor(pHdl, des, aiqCore);
    pHdl->processing   = _handlerAirms_processing;
    pHdl->genIspResult = AiqAlgoHandler_genIspResult_common;
    pHdl->prepare      = _handlerAirms_prepare;
    pHdl->init         = _handlerAirms_init;
    return pHdl;
}
