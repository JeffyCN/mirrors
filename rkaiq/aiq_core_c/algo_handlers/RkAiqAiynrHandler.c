...
#include "aiq_base.h"
#include "RkAiqAiynrHandler.h"
#include "aiq_core.h"
#include "RkAiqGlobalParamsManager_c.h"

static void _handlerAiynr_init(AiqAlgoHandler_t* pHdl) {
    ...
    pHdl->mResultType = RESULT_TYPE_AIYNR_PARAM;
    pHdl->mResultSize = sizeof(aiynr_param_t);

    EXIT_ANALYZER_FUNCTION();
}

static XCamReturn _handlerAiynr_prepare(AiqAlgoHandler_t* pAlgoHandler) {
    ENTER_ANALYZER_FUNCTION();

    XCamReturn ret = XCAM_RETURN_NO_ERROR;

    ret = AiqAlgoHandler_prepare(pAlgoHandler);
    RKAIQCORE_CHECK_RET(ret, "Aiynr handle prepare failed");
    ...
    RKAIQCORE_CHECK_RET(ret, "Aiynr algo prepare failed");

    EXIT_ANALYZER_FUNCTION();
    return XCAM_RETURN_NO_ERROR;
}

static XCamReturn _handlerAiynr_processing(AiqAlgoHandler_t* pAlgoHandler) {
    ENTER_ANALYZER_FUNCTION();
    ...
    if (ret) {
        RKAIQCORE_CHECK_RET(ret, "Aiynr handle processing failed");
    }

    AiqAlgoHandler_do_processing_common(pAlgoHandler);

    RKAIQCORE_CHECK_RET(ret, "Aiynr algo processing failed");

    EXIT_ANALYZER_FUNCTION();
    return ret;
}

AiqAlgoHandler_t* AiqAlgoHandlerAiynr_constructor(RkAiqAlgoDesComm* des, AiqCore_t* aiqCore) {
    AiqAlgoHandler_t* pHdl = (AiqAlgoHandler_t*)aiq_mallocz(sizeof(AiqAlgoHandler_t));
    if (!pHdl)
        return NULL;
    AiqAlgoHandler_constructor(pHdl, des, aiqCore);
    pHdl->processing   = _handlerAiynr_processing;
    pHdl->genIspResult = AiqAlgoHandler_genIspResult_common;
    pHdl->prepare      = _handlerAiynr_prepare;
    pHdl->init         = _handlerAiynr_init;
    return pHdl;
}
