...
#include "aiq_core_c/algo_handlers/RkAiqCnrHandler.h"
...
static XCamReturn
_cnr_SetStrength(const rk_aiq_sys_ctx_t* sys_ctx, acnr_strength_t *strg)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    AiqCnrHandler_t* algo_handle =
        (AiqCnrHandler_t*)sys_ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_ACNR];
    if (algo_handle) {
        AiqCnrHandler_setStrength(algo_handle, strg);
    }
    return ret;
}
...
rk_aiq_user_api2_cnr_SetStrength(const rk_aiq_sys_ctx_t* sys_ctx, acnr_strength_t *strg)
{
    ...
    for (int i = 0; i < ctx_array.num; i++) {
        ret = _cnr_SetStrength(ctx_array.ctx[i], strg);
    }
    return ret;
}

static XCamReturn
_cnr_GetStrength(const rk_aiq_sys_ctx_t* sys_ctx, acnr_strength_t *strg)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    AiqCnrHandler_t* algo_handle =
        (AiqCnrHandler_t*)sys_ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_ACNR];
    if (algo_handle) {
        AiqCnrHandler_getStrength(algo_handle, strg);
    }
    return ret;
}
...
rk_aiq_user_api2_cnr_GetStrength(const rk_aiq_sys_ctx_t* sys_ctx, acnr_strength_t *strg)
{
    ...
    if (_ctx) {
        return _cnr_GetStrength(_ctx, strg);
    }
...
rk_aiq_user_api2_cnr_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, cnr_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_UVNR_PARAM;
	int man_param_size = sizeof(cnr_param_t);
	int aut_param_size = sizeof(cnr_param_auto_t);
...
rk_aiq_user_api2_cnr_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, cnr_status_t* status)
{
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_ACNR];
	if (algo_handle) {
