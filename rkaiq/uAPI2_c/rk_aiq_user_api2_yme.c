...
rk_aiq_user_api2_yme_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, yme_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_MOTION_PARAM;
	int man_param_size = sizeof(yme_param_t);
	int aut_param_size = sizeof(yme_param_auto_t);
...
rk_aiq_user_api2_yme_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, yme_status_t* status)
{
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_AMD];
