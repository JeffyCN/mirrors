...
#include "rk_aiq_api_private_c.h"
...
__RKAIQUAPI_SET_WRAPPER(rk_aiq_user_api2_ae_setAttr, ae_param_t);
...
#endif
__RKAIQUAPI_CALLER(histeq_api_attrib_t);
__RKAIQUAPI_CALLER(histeq_status_t);
...
#endif
__RKAIQUAPI_CALLER(ccm_api_attrib_t);
__RKAIQUAPI_CALLER(ccm_status_t);
#else
...
RkAiqUapiDesc_t rkaiq_uapidesc_list[] = {
    ...
    __RKAIQUAPI_DESC_DEF("/uapi/0/ae_uapi/attr", ae_param_t, __RKAIQUAPI_SET_WRAPPER_NAME(rk_aiq_user_api2_ae_setAttr), rk_aiq_user_api2_ae_getAttr),
...
#endif
    __RKAIQUAPI_DESC_DEF("/uapi/0/histeq_uapi/attr", histeq_api_attrib_t, rk_aiq_user_api2_histeq_SetAttrib, rk_aiq_user_api2_histeq_GetAttrib),
    __RKAIQUAPI_DESC_DEF("/uapi/0/histeq_uapi/info", histeq_status_t, NULL, rk_aiq_user_api2_histeq_QueryStatus),
#if RKAIQ_HAVE_ENHANCE
    __RKAIQUAPI_DESC_DEF("/uapi/0/enh_uapi/attr", enh_api_attrib_t, rk_aiq_user_api2_enh_SetAttrib, rk_aiq_user_api2_enh_GetAttrib),
    __RKAIQUAPI_DESC_DEF("/uapi/0/enh_uapi/info", enh_status_t, NULL, rk_aiq_user_api2_enh_QueryStatus),
#endif
...
#if RKAIQ_HAVE_ASD
    __RKAIQUAPI_DESC_DEF("/uapi/0/asd_uapi/attr", asd_api_attrib_t, rk_aiq_user_api2_asd_SetAttrib, rk_aiq_user_api2_asd_GetAttrib),
    __RKAIQUAPI_DESC_DEF("/uapi/0/asd_uapi/info", asd_status_t, NULL, rk_aiq_user_api2_asd_QueryStatus),
#endif
#endif
    ...
    __RKAIQUAPI_DESC_DEF("/uapi/0/aibnr_uapi/info", aibnr_status_t, NULL, rk_aiq_user_api2_aibnr_QueryStatus),
#endif
#if RKAIQ_HAVE_AIRMS
...
int rkaiq_uapi_best_match(const char* cmd_path_str) {
    ...
    for (i = 0; i < list_len; i++) {
        ...
        if (strstr(cmd_path_str, temp_uapi_desc->arg_path)) {
            int path_length = strlen(temp_uapi_desc->arg_path);
...
int rkaiq_uapi_unified_ctl(rk_aiq_sys_ctx_t* sys_ctx, const char* js_str,
    ...
    *ret_str = NULL;
    ...
    if (change_sum <= 0) {
        ...
        socket_client_setNote(sys_ctx->_socket, IPC_RET_JSON_ERROR, "can't find json patch operation\n");
        return -1;
    }
    ...
    for (i = 0; i <= (change_sum - 1); ++i) {
        if (arr_item) {
            if (RkCam_cJSON_GetObjectItem(arr_item, JSON_PATCH_PATH)->valuestring) {
                cmd_path_str = RkCam_cJSON_GetObjectItem(arr_item, JSON_PATCH_PATH)->valuestring;
                strcpy(orig_path_str, cmd_path_str);
                int desc_i = rkaiq_uapi_best_match(cmd_path_str);
                if (desc_i >= 0) {
                    uapi_desc = &rkaiq_uapidesc_list[desc_i];
                    if (0 == strcmp(uapi_desc->arg_path, cmd_path_str)) {
    ...
    if (!uapi_desc) {
        ...
        socket_client_setNote(sys_ctx->_socket, IPC_RET_UAPI_ERROR, "can't find uapi\n");
        return -1;
    }
    ...
    if (!msys_ctx) {
        ...
        socket_client_setNote(sys_ctx->_socket, IPC_RET_AIQ_ERROR, "can't find valid ctx\n");
        return -1;
    }
#else
    msys_ctx = sys_ctx;
    ...
    if (op_mode == RKAIQUAPI_OPMODE_SET) {
        *ret_str = NULL;
        ...
        if (ret_js) {
            *ret_str = rkaiq_uapi_rpc_response(orig_path_str, ret_js, final_path);
