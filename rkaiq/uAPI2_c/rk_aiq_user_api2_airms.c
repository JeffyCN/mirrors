...
#include "aiq_core_c/algo_handlers/RkAiqAirmsHandler.h"
...
XCamReturn
rk_aiq_user_api2_airms_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, airms_api_attrib_t* attr)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_AIRMS);
    RKAIQ_API_SMART_LOCK(sys_ctx);
    rk_aiq_sys_ctx_array_t ctx_array = rk_aiq_user_api2_common_getSysCtxArray(sys_ctx);
    int type = RESULT_TYPE_AIRMS_PARAM;
    int man_param_size = sizeof(airms_param_t);
    int aut_param_size = sizeof(airms_param_t);
    ...
    for (int i = 0; i < ctx_array.num; i++) {
        if (attr->opMode == RK_AIQ_OP_MODE_MANUAL || attr->opMode == RK_AIQ_OP_MODE_AUTO) {
            ...
            LOGE_AIRMS("wrong mode %d !", attr->opMode);
        }
    }
...
XCamReturn
rk_aiq_user_api2_airms_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, airms_api_attrib_t* attr)
{
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_AIRMS);
    RKAIQ_API_SMART_LOCK(sys_ctx);
...
XCamReturn
rk_aiq_user_api2_airms_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, airms_status_t* status)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_AIRMS);
    ...
    AiqAlgoHandler_t* algo_handle =
        ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_AIRMS];
