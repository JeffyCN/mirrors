...
static XCamReturn
_btnr2_SetStrength(const rk_aiq_sys_ctx_t* sys_ctx, abtnr_strength_t *strg)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    AiqBtnrHandler_t* algo_handle =
        (AiqBtnrHandler_t*)sys_ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_BAYERTNR2];
    if (algo_handle) {
...
XCamReturn
rk_aiq_user_api2_btnr2_SetStrength(const rk_aiq_sys_ctx_t* sys_ctx, abtnr_strength_t *strg)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ...
    for (int i = 0; i < ctx_array.num; i++) {
        ret = _btnr2_SetStrength(ctx_array.ctx[i], strg);
    }
    return ret;
}

static XCamReturn
_btnr2_GetStrength(const rk_aiq_sys_ctx_t* sys_ctx, abtnr_strength_t *strg)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    AiqBtnrHandler_t* algo_handle =
        (AiqBtnrHandler_t*)sys_ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_BAYERTNR2];
...
XCamReturn
rk_aiq_user_api2_btnr2_GetStrength(const rk_aiq_sys_ctx_t* sys_ctx, abtnr_strength_t *strg)
{
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_AMFNR);
    RKAIQ_API_SMART_LOCK(sys_ctx);
    ...
    if (_ctx) {
        return _btnr2_GetStrength(_ctx, strg);
    }
    return XCAM_RETURN_ERROR_UNKNOWN;
}

XCamReturn
rk_aiq_user_api2_btnr2_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, btnr_api_attrib_t* attr)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_BAYERTNR2);
    RKAIQ_API_SMART_LOCK(sys_ctx);

	rk_aiq_sys_ctx_array_t ctx_array = rk_aiq_user_api2_common_getSysCtxArray(sys_ctx);

	int type = RESULT_TYPE_TNR2_PARAM;
...
XCamReturn
rk_aiq_user_api2_btnr2_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, btnr_api_attrib_t* attr)
{
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_BAYERTNR2);
    RKAIQ_API_SMART_LOCK(sys_ctx);
...
XCamReturn
rk_aiq_user_api2_btnr2_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, btnr_status_t* status)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_BAYERTNR2);
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_BAYERTNR2];
