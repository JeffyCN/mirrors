...
#include "aiq_core_c/algo_handlers/RkAiqGicHandler.h"
...
rk_aiq_user_api2_gic_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, gic_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_GIC_PARAM;
	int man_param_size = sizeof(gic_param_t);
	int aut_param_size = sizeof(gic_param_auto_t);
...
rk_aiq_user_api2_gic_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, gic_status_t* status)
{
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_AGIC];
