...
XCamReturn
rk_aiq_user_api2_fpnSw_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, fpnSw_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_FPNSW_PARAM;
	int man_param_size = sizeof(fpnSw_param_t);
	int aut_param_size = sizeof(fpnSw_param_auto_t);
    ...
    for (int i = 0; i < ctx_array.num; i++) {
        if (attr->opMode == RK_AIQ_OP_MODE_MANUAL || attr->opMode == RK_AIQ_OP_MODE_AUTO) {
            ret = rk_aiq_user_api2_common_processParams(ctx_array.ctx[i], true,
                    ...
                    type, man_param_size, NULL, aut_param_size,  &attr->stAuto);
        } else {
            ret = XCAM_RETURN_ERROR_FAILED;
            LOGE_ACP("wrong mode %d !", attr->opMode);
        }
    }
...
XCamReturn
rk_aiq_user_api2_fpnSw_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, fpnSw_api_attrib_t* attr)
{
    CHECK_USER_API_ENABLE2(sys_ctx);
    RKAIQ_API_SMART_LOCK(sys_ctx);
...
XCamReturn
rk_aiq_user_api2_fpnSw_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, fpnSw_status_t* status)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
	...
	// 	ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_ACP];
	// if (algo_handle) {
