...
#include "aiq_core_c/algo_handlers/RkAiqAibnrHandler.h"
...
XCamReturn
rk_aiq_user_api2_aibnr_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, aibnr_api_attrib_t* attr)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_AIBNR);
    RKAIQ_API_SMART_LOCK(sys_ctx);
    rk_aiq_sys_ctx_array_t ctx_array = rk_aiq_user_api2_common_getSysCtxArray(sys_ctx);
    int type = RESULT_TYPE_AIBNR_PARAM;
    int man_param_size = sizeof(aibnr_param_t);
    int aut_param_size = sizeof(aibnr_param_auto_t);
    ...
    for (int i = 0; i < ctx_array.num; i++) {
        if (attr->opMode == RK_AIQ_OP_MODE_MANUAL || attr->opMode == RK_AIQ_OP_MODE_AUTO) {
            ...
            LOGE_AIBNR("wrong mode %d !", attr->opMode);
        }
    }
...
XCamReturn
rk_aiq_user_api2_aibnr_GetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, aibnr_api_attrib_t* attr)
{
    CHECK_USER_API_ENABLE2(sys_ctx);
    CHECK_USER_API_ENABLE(RK_AIQ_ALGO_TYPE_AIBNR);
    RKAIQ_API_SMART_LOCK(sys_ctx);
...
rk_aiq_user_api2_aibnr_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, aibnr_status_t* status)
{
    ...
    AiqAlgoHandler_t* algo_handle =
        ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_AIBNR];
