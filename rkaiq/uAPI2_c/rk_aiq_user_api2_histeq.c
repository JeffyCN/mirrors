...
#include "aiq_core_c/algo_handlers/RkAiqHisteqHandler.h"
...
rk_aiq_user_api2_histeq_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, histeq_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_HISTEQ_PARAM;
	int man_param_size = sizeof(histeq_param_t);
	int aut_param_size = sizeof(histeq_param_auto_t);
...
rk_aiq_user_api2_histeq_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, histeq_status_t* status)
{
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_AHISTEQ];
