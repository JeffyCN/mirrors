...
#include "aiq_core_c/algo_handlers/RkAiqDmHandler.h"
...
rk_aiq_user_api2_dm_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, dm_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_DEBAYER_PARAM;
	int man_param_size = sizeof(dm_param_t);
	int aut_param_size = sizeof(dm_param_auto_t);
    ...
    for (int i = 0; i < ctx_array.num; i++) {
        if (attr->opMode == RK_AIQ_OP_MODE_MANUAL || attr->opMode == RK_AIQ_OP_MODE_AUTO) {
            ...
            LOGE_ADEBAYER("wrong mode %d !", attr->opMode);
        }
    }
...
rk_aiq_user_api2_dm_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, dm_status_t* status)
{
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_ADEBAYER];
	if (algo_handle) {
