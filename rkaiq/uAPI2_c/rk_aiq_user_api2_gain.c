...
#include "aiq_core_c/algo_handlers/RkAiqGainHandler.h"
...
rk_aiq_user_api2_gain_SetAttrib(const rk_aiq_sys_ctx_t* sys_ctx, gain_api_attrib_t* attr)
{
	...
	int type = RESULT_TYPE_GAIN_PARAM;
	int man_param_size = sizeof(gain_param_t);
	int aut_param_size = sizeof(gain_param_auto_t);
...
rk_aiq_user_api2_gain_QueryStatus(const rk_aiq_sys_ctx_t* sys_ctx, gain_status_t* status)
{
	...
	AiqAlgoHandler_t* algo_handle =
		ctx->_analyzer->mAlgoHandleMaps[RK_AIQ_ALGO_TYPE_AGAIN];
