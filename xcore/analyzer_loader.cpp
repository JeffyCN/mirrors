...
{
    XCAM_LOG_INFO ("destroy analyzer loader.");
    close_handle ();
    if (_path)
        xcam_free (_path);
    if (_symbol)
...
void *
AnalyzerLoader::load_library (const char *lib_path, const int type)
{
    void *desc = NULL;
...
void *
AnalyzerLoader::get_symbol (void* handle, const char *symbol)
{
    void *desc = NULL;
    ...
    if(symbol == NULL) {
        ...
        if (!desc) {
            XCAM_LOG_ERROR ("get symbol(%s) failed from lib(%s), reason:%s", _symbol, XCAM_STR (_path), dlerror ());
            return NULL;
        }
    } else {
        desc = (void *)dlsym (handle, symbol);
        if (!desc) {
            XCAM_LOG_ERROR ("get symbol(%s) failed from lib(%s), reason:%s", symbol, XCAM_STR (_path), dlerror ());
            return NULL;
        }
    }
