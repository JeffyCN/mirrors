    ...
    int num_plane)
{
    ...
    if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == buf_type) {
        ...
        v4l2_buf.m.planes[0].m.fd = prime.fd;
    ...
    XCAM_LOG_DEBUG ("create drm buffer size:%lld, fd: %d, sizeimage: %d, fmt: %d\n",
		gem.size, prime.fd, format.fmt.pix.sizeimage, format.fmt.pix.pixelformat);
