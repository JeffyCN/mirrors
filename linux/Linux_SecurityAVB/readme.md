...
There is already a set of test certificates and keys in avb_keys directory. If you need new Keys and Certificates, you can generate them by the following steps:
~~~shell
    ./avb_user_tool.sh -n <Product ID> #The size of Product ID is 16 bytes.

~~~
This operation will remove avb_keys first, so please backup your previous keys properly if necessary.
...
~~~
The PrivateKey.pem which is the key of Base SecureBoot is requried.
This operation will generate .setting configuration file, it set device type to efuse.
If you want to reconfig device type, **DO NOT FORGET to chanage the type**. You can set "efuse" or "otp" in .setting.

## 5. Download Process

1.    Sign firmwares and generate vbmeta.img in "out" folder.
~~~shell
...
- vbmeta.img and boot.img/recovery.img generated with step 1.
...
Store user password for fastboot which run with supper user.
~~~shell
    ./avb_user_tool.sh --su_pswd < /user/password >
~~~
It would generate .su_pswd in scripts, it can help us run fastboot without input user password manually.
...
Lock the device:
~~~shell
    ./avb_user_tool.sh -l # reboot device after lock finished
~~~

Unlock the device:
~~~shell
    ./avb_user_tool.sh -u # reboot device after unlock finished
~~~
## 7. Verity
