...
public:
    XCam3AAiqContext ();
    ~XCam3AAiqContext ();
    bool setup_analyzer (struct isp_supplemental_sensor_mode_data &sensor_mode_data, const char *cpf);
    bool setup_analyzer (SmartPtr<IspController> &ispCtrl, const char *cpf);
...
bool
XCam3AAiqContext::setup_analyzer (struct isp_supplemental_sensor_mode_data &sensor_mode_data, const char *cpf)
{
    XCAM_ASSERT (!_analyzer.ptr ());
    SmartPtr<X3aAnalyzer> aiq_analyzer = new X3aAnalyzerRKiq (sensor_mode_data, cpf);
...
bool
XCam3AAiqContext::setup_analyzer (SmartPtr<IspController> &ispCtrl, const char *cpf)
{
    XCAM_ASSERT (!_analyzer.ptr ());
    SmartPtr<X3aAnalyzer> aiq_analyzer = new X3aAnalyzerRKiq (ispCtrl, cpf);
    XCAM_ASSERT (aiq_analyzer.ptr ());
...
XCam3AAiqContext::x3a_calculation_done (XAnalyzer *analyzer, X3aResultList &results)
{
    ...
    XCAM_LOG_INFO ("aiq x3a_calculation_done size: %d", _results.size());
}

void
XCam3AAiqContext::x3a_calculation_failed (XAnalyzer *analyzer, int64_t timestamp, const char *msg)
{
    XCAM_UNUSED (analyzer);
...
xcam_configure_3a (XCam3AContext *context, void* dev, void* isp_stats_dev, void* isp_params_dev, uint32_t width, uint32_t height, double framerate)
{
    ...
    XCAM_LOG_INFO ("---xcam_configure_3a");
    /*
    ...
    */
    ...
    // TODO:: here use isp controller to get sensor mode data(stat node for rkisp10, sensor subdev for rkisp1)
    SmartPtr<IspController> isp_controller = new IspController ();
    //isp_controller->set_video_device(dev);
    //isp_controller->set_sensor_subdev(dev);
    XCAM_FAIL_RETURN (
        WARNING,
        aiq_context->setup_analyzer (isp_controller, path_cpf == NULL ? DEFAULT_AIQ_CPF_FILE : path_cpf),
        XCAM_RETURN_ERROR_UNKNOWN,
        "setup aiq 3a analyzer failed");

    SmartPtr<X3aAnalyzeTuner> analyzer = aiq_context->get_analyzer ();

    analyzer->get_analyzer()->set_video_device(dev);
    analyzer->get_analyzer()->set_isp_stats_device(isp_stats_dev);
    analyzer->get_analyzer()->set_isp_params_device(isp_params_dev);
    ...
    XCAM_LOG_INFO ("-aiq init analyzer");
    ...
    XCAM_LOG_INFO ("-aiq start analyzer");
    ret = analyzer->start ();
    XCAM_FAIL_RETURN (
        WARNING,
        ret == XCAM_RETURN_NO_ERROR,
...
xcam_set_3a_stats (XCam3AContext *context, XCam3AStats *stats, int64_t timestamp)
{
    ...
    struct cifisp_stat_buffer *raw_stats = (struct cifisp_stat_buffer*)isp_stats->get_isp_stats ();
