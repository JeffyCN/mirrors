...
XCamReturn RkAiqCamGroupAmergeHandleInt::updateConfig(bool needSync) {
    ...
    if (updateAtt) {
        mCurAtt   = mNewAtt;
        rk_aiq_uapi_amerge_SetAttrib(mAlgoCtx, mCurAtt, false);
        updateAtt = false;
...
XCamReturn RkAiqCamGroupAmergeHandleInt::setAttrib(mergeAttr_t att) {
    ENTER_ANALYZER_FUNCTION();
...
XCamReturn RkAiqCamGroupAmergeHandleInt::getAttrib(mergeAttr_t* att) {
    ENTER_ANALYZER_FUNCTION();
