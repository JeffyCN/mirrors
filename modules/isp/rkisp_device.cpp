...
RKispDevice::RKispDevice (const char *name)
    : V4l2Device (name)
{
}

RKispDevice::~RKispDevice ()
{
}

XCamReturn
RKispDevice::pre_set_format (struct v4l2_format &format)
{
    ...
    const uint32_t index)
{
    ...
    if (get_mem_type () == V4L2_MEMORY_DMABUF && _drm_disp.ptr () != NULL) {
        buf = _drm_disp->create_drm_buf (format, index, get_buf_type (), &_planes[index], FMT_NUM_PLANES);
        if (!buf.ptr()) {
