...
{
public:
    explicit X3aIspStatsData (struct cifisp_stat_buffer *isp_data, XCam3AStats *data);
    ~X3aIspStatsData ();
    void *get_isp_stats () {
        return _isp_data;
    }
...
private:
    struct cifisp_stat_buffer *_isp_data;
};
...
public:
    ...
    void set_grid_info (const struct rkisp_grid_info &info);
...
private:
    struct cifisp_stat_buffer *alloc_isp_statsictics ();
    XCAM_DEAD_COPY (X3aStatisticsQueue);

private:
    struct rkisp_grid_info  _grid_info;
};
