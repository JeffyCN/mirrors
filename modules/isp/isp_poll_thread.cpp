...
XCamReturn
IspPollThread::notify_sof (int64_t time, int frameid)
{
#if RKISP
    _isp_controller->handle_sof (time, frameid);
#endif
    return XCAM_RETURN_NO_ERROR;
}
...
IspPollThread::handle_events (struct v4l2_event &event)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    switch (event.type) {
    case V4L2_EVENT_RKISP_3A_STATS_READY:
