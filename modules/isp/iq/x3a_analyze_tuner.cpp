...
{
    XCAM_LOG_INFO("---running internal init... \n"); 
...
X3aAnalyzeTuner::configure_3a ()
{
    ...
    ret = _analyzer->start ();
    notify_calculation_done (_results);
...
X3aAnalyzeTuner::x3a_calculation_done (XAnalyzer *analyzer, X3aResultList &results)
{
    ...
    XCAM_LOG_INFO ("tunner x3a_calculation_done result size: %d", _results.size());

#if 0
    for (X3aResultList::iterator iter = _results.begin (); iter != _results.end (); ++iter) {
        ...
        if (isp_result->get_type() == X3aIspConfig::IspAllParameters) {
            ...
            XCAM_LOG_ERROR ("tunner active_configs: %x", rkisp_params.active_configs);
        }
    }
#endif
