...
AtomIspConfigContent::copy (const struct rkisp_parameters &isp_params)
{
    ...
    isp_config.active_configs = isp_params.active_configs;

    dpcc_config = isp_params.dpcc_config;
    isp_config.dpcc_config = dpcc_config;

    bls_config = isp_params.bls_config;
    isp_config.bls_config = bls_config;

    sdg_config = isp_params.sdg_config;
    isp_config.sdg_config = sdg_config;

    hst_config = isp_params.hst_config;
    isp_config.hst_config = hst_config;

    lsc_config = isp_params.lsc_config;
    isp_config.lsc_config = lsc_config;

    awb_gain_config = isp_params.awb_gain_config;
    isp_config.awb_gain_config = awb_gain_config;

    awb_meas_config = isp_params.awb_meas_config;
    isp_config.awb_meas_config = awb_meas_config;

    flt_config = isp_params.flt_config;
    isp_config.flt_config = flt_config;

    bdm_config = isp_params.bdm_config;
    isp_config.bdm_config = bdm_config;

    ctk_config = isp_params.ctk_config;
    isp_config.ctk_config = ctk_config;

    goc_config = isp_params.goc_config;
    isp_config.goc_config = goc_config;

    cproc_config = isp_params.cproc_config;
    isp_config.cproc_config = cproc_config;

    aec_config = isp_params.aec_config;
    isp_config.aec_config = aec_config;

    afc_config = isp_params.afc_config;
    isp_config.afc_config = afc_config;

    ie_config = isp_params.ie_config;
    isp_config.ie_config = ie_config;

    dpf_config = isp_params.dpf_config;
    isp_config.dpf_config = dpf_config;

    dpf_strength_config = isp_params.dpf_strength_config;
    isp_config.dpf_strength_config = dpf_strength_config;

    for (int i=0; i < HAL_ISP_MODULE_MAX_ID_ID + 1; i++) {
        isp_config.enabled[i] = isp_params.enabled[i];
    }

    isp_config.flt_denoise_level = isp_params.flt_denoise_level;
    isp_config.flt_sharp_level = isp_params.flt_sharp_level;
