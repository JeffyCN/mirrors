...
static inline int v4l_queue_queue_all(const struct v4l_queue *q, struct v4l_fd *f)
{
    struct v4l_buffer buf;
	for (int i = 0; i < v4l_queue_g_buffers(q); i++) {
        v4l_queue_buffer_init(q, &buf, i);
        int ret = v4l_buffer_qbuf(f, &buf);
        if (ret)
            return ret;
    }
    return 0;
