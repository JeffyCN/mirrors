...
public:
	...
	int subdev_g_fmt(v4l2_subdev_format &fmt)
	{
		return cv4l_ioctl(VIDIOC_SUBDEV_G_FMT, &fmt);
	}

	int subdev_s_fmt(v4l2_subdev_format &fmt)
	{
		return cv4l_ioctl(VIDIOC_SUBDEV_S_FMT, &fmt);
	}
	...
	int subdev_s_selection(v4l2_subdev_selection &sel)
	{
		return cv4l_ioctl(VIDIOC_SUBDEV_S_SELECTION, &sel);
	}
